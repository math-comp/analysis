...
Proof.
split=> [Fy eps|Fy P [eps /filterS]]; last exact.
exact/Fy/locally_ball.
Qed.
...
(* Lemma filterlim_locally_cauchy {F} {FF : ProperFilter F} (f : T -> U) : *)
(*   cauchy (f @ F) -> [cvg f @ F in U]. *)
(* Proof. *)

(* move=> fF_cauchy; rewrite cvgE; apply/flim_ballP => eps /=. *)
(* near x. *)
(*   have_near F y. *)
(* apply/ *)
(* intros F FF f. *)
