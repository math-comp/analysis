...
by have [x Pfx] := filter_ex FP; exists (f x).
Qed.
...
split.
- intros P FP.
  destruct (filter_ex FP) as [x [y [_ Hy]]].
  now exists y.
- apply filtermapi_filter.
  exact HF.
  apply filter_filter.
...
Proof.
  ...
  case: (filter_ex H1) => x1 Hx1.
  case: (filter_ex H2) => x2 Hx2.
  exists (x1,x2).
  by apply HP.
  apply filter_prod_filter.
  apply FF.
...
Proof. by apply Uniform.ax1. Qed.
