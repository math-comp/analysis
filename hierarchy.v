...
Proof.
move=> cxy P /= /locallyP /= [eps _ epsP].
...
by apply/Dx/D_close; apply/locallyP => -[eps _ /subD].
Qed.
...
have NDfx: ~ D (f x) by move: fxD => /locallyP [e _]; apply.
...
Proof.
apply: Build_ProperFilter' => -[d _ /(_ (x + pos d / 2%:R))].
...
Proof.
by move: x => [x P [eps _ HP] |x P|x P] //=; exists eps => // ???; apply: HP.
Qed.

Lemma Rbar_locally'_le_finite (x : R) : Rbar_locally' x --> locally x.
Proof. by move=> P [eps _ HP] //=; exists eps => // ???; apply: HP. Qed.
...
case: x => /= [x [delta _ Hp] |[delta Hp] |[delta Hp]].
- (* x \in R *)
...
- move: (H1 eps) => {H1} [x0 _ Hx0].
  ...
  by rewrite mul1r absrB.
- apply H2 => eps; move: (H eps) => {H} [x0 _ Hx0].
...
Lemma funaddo (F : filterType T) e (df dg :'o_F e) (f g: T -> V) :
  ...
  @the_littleo F e (@the_littleo F e (@little0 F e) (df \+ dg))
                   (@the_littleo F e df f \+ @the_littleo F e dg g).
Proof. Admitted.
...
Lemma addo (F : filterType T) e (df dg :'o_F e) (f g: T -> V) x :
  ...
  @the_littleo F e (@the_littleo F e (@little0 F e) (df \+ dg))
                   (@the_littleo F e df f \+ @the_littleo F e dg g) x.
Proof. Admitted.

Lemma congro (F : filterType T) e (df dg :'o_F e) (f hf hg : T -> V) :
  f \+ @the_littleo F e df hf =1 f \+ @the_littleo F e dg hg.
Proof. Admitted.
...
Proof.
by move=> -> ->; rewrite funeqE=> x; rewrite /= -addrA addo; apply: congro.
Qed.
