...
(*                                                                            *)
(* `(x \in P)`                                                                *)
(*     ~ boolean membership predicate from `ssrbool` for set `P`,             *)
(*       available thanks to a canonical `predType T` structure on sets       *)
(*       on `T`                                                               *)
(*                                                                            *)
(* `[set x : T | P]`                                                          *)
(*     ~ set of points `x : T` such that `P` holds                            *)
(*                                                                            *)
(* `[set x | P]`                                                              *)
(*     ~ same as before with `T` left implicit                                *)
(*                                                                            *)
(* `[set E | x in A]`                                                         *)
(*     ~ set defined by the expression `E` for `x` in set `A`                 *)
(*                                                                            *)
(* `[set E | x in A & y in B]`                                                *)
(*     ~ same as before for `E` depending on 2 variables `x` and `y` in       *)
(*       sets `A` and `B`                                                     *)
(*                                                                            *)
(* `setT`                                                                     *)
(*     ~ full set                                                             *)
...
(*                                                                            *)
(* `range f`                                                                  *)
(*     ~ the range of `f`, i.e., `[set f x | x in setT]`                      *)
(*                                                                            *)
(* `[set a]`                                                                  *)
(*     ~ set containing only `a`                                              *)
(*                                                                            *)
(* `[set a : T]`                                                              *)
(*     ~ same as before with the type of `a` made explicit                    *)
(*                                                                            *)
(* ``A `|` B``                                                                *)
(*     ~ union of `A` and `B`                                                 *)
(*                                                                            *)
...
(*                                                                            *)
(* `[set a1; a2; ..; an]`                                                     *)
(*     ~ set containing only the `n` elements `ai`                            *)
(*                                                                            *)
(* ``A `&` B``                                                                *)
(*     ~ intersection of `A` and `B`                                          *)
(*                                                                            *)
(* ``A `*` B``                                                                *)
(*     ~ product of `A` and `B`, i.e., set of pairs `(a,b)` such that         *)
(*       `A a` and `B b`                                                      *)
(*                                                                            *)
(* ``A.`1``                                                                   *)
(*     ~ set of points `a` such that there exists `b` so that                 *)
(*       `A (a, b)`                                                           *)
(*                                                                            *)
(* ``A.`2``                                                                   *)
(*     ~ set of points `a` such that there exists `b` so that                 *)
...
(*                                                                            *)
(* `\bigcup_(i in P) F`                                                       *)
(*     ~ union of the elements of the family `F` whose index satisfies        *)
(*       `P`                                                                  *)
(*                                                                            *)
(* `\bigcup_(i : T) F`                                                        *)
(*     ~ union of the family `F` indexed on `T`                               *)
(*                                                                            *)
(* `\bigcup_(i < n) F`                                                        *)
(*     ~ ``\bigcup_(i in `I_n) F``                                            *)
(*                                                                            *)
(* `\bigcup_(i >= n) F`                                                       *)
(*     ~ `\bigcup_(i in [set i | i >= n]) F`                                  *)
(*                                                                            *)
(* `\bigcup_i F`                                                              *)
(*     ~ same as before with `T` left implicit                                *)
(*                                                                            *)
(* `\bigcap_(i in P) F`                                                       *)
(*     ~ intersection of the elements of the family `F` whose index           *)
(*       satisfies `P`                                                        *)
(*                                                                            *)
(* `\bigcap_(i : T) F`                                                        *)
(*     ~ union of the family `F` indexed on `T`                               *)
...
(*                                                                            *)
(* `setI_closed G`                                                            *)
(*     ~ the set of sets `G` is closed under finite intersection              *)
(*                                                                            *)
(* `setU_closed G`                                                            *)
(*     ~ the set of sets `G` is closed under finite union                     *)
(*                                                                            *)
(* `rectangle X Y`                                                            *)
(*     ~ ``[set U `*` V | U in X & V in Y]``                                  *)
(*                                                                            *)
(* `preimage_set_system D f G`                                                *)
(*     ~ set system of the preimages by `f` of sets in `G`                    *)
(*                                                                            *)
(* `image_set_system D f G`                                                   *)
(*     ~ set system of the sets with a preimage by `f` in `G`                 *)
(*                                                                            *)
(* `cross f g X Y`                                                            *)
(*     ~ ``preimage_set_system setT f X `|` preimage_set_system setT g Y``    *)
(*                                                                            *)
(* ``X `x` Y``                                                                *)
(*     ~ `cross fst snd X Y`                                                  *)
(*                                                                            *)
(* `R ^nat`                                                                   *)
(*     ~ notation for the type of sequences, i.e., functions of type          *)
(*       `nat -> R`                                                           *)
(*                                                                            *)
(* `bigcup2 A B`                                                              *)
(*     ~ the sequence `A`, `B`, `0`, `0`, ...                                 *)
(*                                                                            *)
(* `bigcup2 A B`                                                              *)
(*     ~ the sequence `A`, `B`, `T`, `T`, ...                                 *)
(*                                                                            *)
(* `smallest C G`                                                             *)
(*     ~ ``\bigcap_(A in [set M | C M /\ G `<=` M]) A``                       *)
(*                                                                            *)
(* ``A `<=` B``                                                               *)
(*     ~ `A` is included in `B`                                               *)
(*                                                                            *)
...
(*                                                                            *)
(* ``A `<=>` B``                                                              *)
(*     ~ double inclusion ``A `<=` B`` and ``B `<=` A``                       *)
(*                                                                            *)
(* ``f @^-1` A``                                                              *)
(*     ~ preimage of `A` by `f`                                               *)
(*                                                                            *)
(* ``f @` A``                                                                 *)
(*     ~ image of `A` by `f`. This is a notation for `image A f`              *)
(*                                                                            *)
(* `A !=set0`                                                                 *)
(*     ~ `exists x, A x`                                                      *)
(*                                                                            *)
(* ``[set` p]``                                                               *)
(*     ~ a classical set corresponding to the `predType p`                    *)
(*                                                                            *)
(* `` `[a, b] ``                                                              *)
(*     ~ ``[set` `[a, b]]``, i.e., a classical set corresponding to           *)
(*       the interval `` `[a, b] ``                                           *)
(*                                                                            *)
(* `` `]a, b] ``                                                              *)
(*     ~ ``[set` `]a, b]]``                                                   *)
...
(*                                                                            *)
(* `is_subset1 A`                                                             *)
(*     ~ `A` contains only 1 element                                          *)
(*                                                                            *)
(* `is_fun f`                                                                 *)
(*     ~ for each `a`, `f a` contains only 1 element                          *)
(*                                                                            *)
(* `is_total f`                                                               *)
(*     ~ for each `a`, `f a` is non empty                                     *)
(*                                                                            *)
(* `is_totalfun f`                                                            *)
(*     ~ conjunction of `is_fun` and `is_total`                               *)
(*                                                                            *)
(* `xget x0 P`                                                                *)
(*     ~ point `x` in `P` if it exists, `x0` otherwise; `P` must be a         *)
(*       set on a `choiceType`                                                *)
(*                                                                            *)
(* `fun_of_rel f0 f`                                                          *)
(*     ~ function that maps `x` to an element of `f x` if there is            *)
(*       one, to `f0 x` otherwise                                             *)
(*                                                                            *)
(* ``F `#` G``                                                                *)
(*     ~ intersections beween elements of `F` an `G` are all non              *)
(*       empty                                                                *)
(*                                                                            *)
(* `seqDU F`                                                                  *)
(*     ~ sequence `F_0`, `F_1\F_0`, `F_2\(F_0 U F_1)`,...                     *)
(*                                                                            *)
(* `seqD F`                                                                   *)
(*     ~ the sequence `F_0`, `F_1 \ F_0`, `F_2 \ F_1`,...                     *)
(*                                                                            *)
(* ## Pointed types                                                           *)
(* `pointedType`                                                              *)
(*     ~ interface type for types equipped with a canonical inhabitant.       *)
(*       The HB class is `Pointed`.                                           *)
(*                                                                            *)
(* `point`                                                                    *)
(*     ~ canonical inhabitant of a `pointedType`                              *)
(*                                                                            *)
(* `get P`                                                                    *)
(*     ~ point `x` in `P` if it exists, `point` otherwise. `P` must           *)
(*       be a set on a `pointedType`                                          *)
(*                                                                            *)
(* ## squash/unsquash                                                         *)
(* `$| T |`                                                                   *)
(*     ~ the type `T : Type` is inhabited. `$| T |` has type `Prop`.          *)
(*       `$| T |` is a notation for `squashed T`.                             *)
(*                                                                            *)
(* `squash x`                                                                 *)
(*     ~ object of type `$| T |` (with `x : T`)                               *)
(*                                                                            *)
(* `unsquash s`                                                               *)
(*     ~ extract an inhabitant of type `T` (with `s : $| T |`)                *)
(* Tactic:                                                                    *)
(*   - squash x:                                                              *)
...
(* `trivIset D F`                                                             *)
(*     ~ the sets `F i`, where `i` ranges over `D : set I`, are               *)
(*       pairwise-disjoint                                                    *)
(*                                                                            *)
(* `cover D F`                                                                *)
(*     ~ `\bigcup_(i in D) F i`                                               *)
(*                                                                            *)
(* `partition D F A`                                                          *)
(*     ~ the non-empty sets `F i`,where `i` ranges over `D : set I`,          *)
(*       form a partition of `A`                                              *)
(*                                                                            *)
(* `pblock_index D F x`                                                       *)
(*     ~ index `i` such that `i \in D` and `x \in F i`                        *)
(*                                                                            *)
(* `pblock D F x`                                                             *)
(*     ~ `F (pblock_index D F x)`                                             *)
(*                                                                            *)
(* `maximal_disjoint_subcollection F A B`                                     *)
(*     ~ `A` is a maximal (for inclusion) disjoint subcollection of           *)
(*       the collection `B` of elements in `F : I -> set T`                   *)
(*                                                                            *)
(* ## Upper and lower bounds                                                  *)
(* `ubound A`                                                                 *)
(*     ~ the set of upper bounds of the set `A`                               *)
(*                                                                            *)
(* `lbound A`                                                                 *)
(*     ~ the set of lower bounds of the set `A`                               *)
(*                                                                            *)
...
(*                                                                            *)
(* `isLub A m`                                                                *)
(*     ~ `m` is a least upper bound of the set `A`                            *)
(*                                                                            *)
(* `supremums A`                                                              *)
(*     ~ set of supremums of the set `A`                                      *)
(*                                                                            *)
(* `supremum x0 A`                                                            *)
(*     ~ supremum of `A` or `x0` if `A` is empty                              *)
(*                                                                            *)
(* `infimums A`                                                               *)
(*     ~ set of infimums of the set `A`                                       *)
(*                                                                            *)
(* `infimum x0 A`                                                             *)
(*     ~ infimum of `A` or `x0` if `A` is empty                               *)
(*                                                                            *)
(* ``F `#` G``                                                                *)
(*     ~ the classes of sets `F` and `G` intersect                            *)
(*                                                                            *)
(* ## Sections                                                                *)
(* `xsection A x`                                                             *)
(*     ~ with `A : set (T1 * T2)` and `x : T1` is the `x`-section             *)
(*       of `A`                                                               *)
(*                                                                            *)
(* `ysection A y`                                                             *)
(*     ~ with `A : set (T1 * T2)` and `y : T2` is the `y`-section             *)
(*       of `A`                                                               *)
(*                                                                            *)
(* ## Relations                                                               *)
(* Notations for composition and inverse (scope: relation_scope):             *)
(* `B \; A`                                                                   *)
(*     ~ `[set x | exists z, A (x.1, z) & B (z, x.2)]`                        *)
(*                                                                            *)
(* `A^-1`                                                                     *)
(*     ~ `[set xy | A (xy.2, xy.1)]`                                          *)
(*                                                                            *)
