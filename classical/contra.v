...
(* ## Tactics                                                                 *)
(* `assume_not`                                                               *)
(*     ~ add a goal negation assumption. The tactic also works for goals      *)
(*       in `Type`, simplifies the added assumption, and exposes its          *)
(*       top-level constructive content.                                      *)
(*                                                                            *)
(* `absurd_not`                                                               *)
(*     ~ proof by contradiction. Same as `assume_not`, but the goal is        *)
(*       erased and replaced by `False`. Caveat: `absurd_not` cannot be       *)
(*       used as a `move/` view because its conclusion is indeterminate.      *)
(*       The more general `notP` can be used instead.                         *)
(*                                                                            *)
(* `contra`                                                                   *)
(*     ~ proof by contraposition. Change a goal of the form                   *)
(*       `assumption -> conclusion` to `~ conclusion -> ~ assumption`. As     *)
(*       with `assume_not`, `contra` allows both assumption and conclusion    *)
(*       to be in `Type`, simplifies the negation of both assumption and      *)
(*       conclusion, and exposes the constructive contents of the negated     *)
(*       conclusion. The `contra` tactic also supports a limited form of      *)
(*       the `':'` discharge pseudo tactical, whereby `contra: <d-items>`     *)
(*       means `move: <d-items>; contra`. The only `<d-items>` allowed are    *)
(*       one term, possibly preceded by a clear switch.                       *)
(*                                                                            *)
(* `absurd`                                                                   *)
(*     ~ proof by contradiction. The defective form of the tactic simply      *)
(*       replaces the entire goal with `False` (just as the Ltac              *)
(*       `exfalso`), leaving the user to derive a contradiction from the      *)
(*       assumptions. The `':'` form `absurd: <d-items>` replaces the goal    *)
(*       with the negation of the (single) `<d-item>` (as with `contra:`,     *)
(*       a clear switch is also allowed. Finally the `Ltac absurd term`       *)
