...
(* ## Structure of filter                                                     *)
(* `filteredType U`                                                           *)
(*     ~ interface type for types whose elements represent sets of sets       *)
(*       on `U` These sets are intended to be filters on `U` but this is      *)
(*       not enforced yet. The HB class is called `Filtered`. It extends      *)
(*       `Pointed`.                                                           *)
(*                                                                            *)
(* `nbhs p`                                                                   *)
(*     ~ set of sets associated to `p` (in a filtered type)                   *)
(*                                                                            *)
(* `pfilteredType U`                                                          *)
(*     ~ a pointed and filtered type                                          *)
(*                                                                            *)
(* `hasNbhs`                                                                  *)
(*     ~ factory for `filteredType`                                           *)
(*                                                                            *)
(* `nbhsType`                                                                 *)
(*     ~ type of a structure that has a set system of neighborhoods           *)
(*       associated to each point                                             *)
(*                                                                            *)
(* `pnbhsType`                                                                *)
(*     ~ same has `nbhsType` for pointed types                                *)
(*                                                                            *)
(* `continuous f`                                                             *)
(*     ~ `f` is continuous w.r.t the topology                                 *)
(*                                                                            *)
(* `isSubNbhs V S U`                                                          *)
(*     ~ interface that states the continuity of val for `U` which has a      *)
(*       `subChoiceType` and a `nbhsType`                                     *)
(*                                                                            *)
(* `subNbhsType V S`                                                          *)
(*     ~ structure that extends a `subChoiceType`/`nbhsType` with the         *)
(*       `isSubNbhs` interface The HB class is `SubNbhs`.                     *)
(*                                                                            *)
(* `filterI_iter F n`                                                         *)
(*     ~ nth stage of recursively building the filter of finite               *)
(*       intersections of `F`                                                 *)
(*                                                                            *)
(* `finI_from D f`                                                            *)
(*     ~ set of `\bigcap_(i in E) f i` where `E` is a a finite subset of      *)
(*       `D`                                                                  *)
(*                                                                            *)
...
(* ## Theory of filters                                                       *)
(* `filter_from D B`                                                          *)
(*     ~ set of the supersets of the elements of the family of sets `B`       *)
(*       whose indices are in the domain `D` This is a filter if              *)
(*       `(B_i)_(i in D)` forms a filter base.                                *)
(*                                                                            *)
(* `filter_prod F G`                                                          *)
(*     ~ product of the filters `F` and `G`                                   *)
(*                                                                            *)
(* ``F `=>` G``                                                               *)
(*     ~ `G` is included in `F` `F` and `G` are sets of sets.                 *)
(*                                                                            *)
(* `\oo`                                                                      *)
(*     ~ "eventually" filter on `nat`: set of predicates on natural           *)
(*       numbers that are eventually true                                     *)
(*                                                                            *)
(* `F --> G`                                                                  *)
(*     ~ the canonical filter associated to `G` is included in the            *)
(*       canonical filter associated to `F`                                   *)
(*                                                                            *)
(* `lim F`                                                                    *)
(*     ~ limit of the canonical filter associated with `F` if there is        *)
(*       such a limit, i.e., an element `l` such that the canonical           *)
(*       filter associated to `l` is a subset of `F`                          *)
(*                                                                            *)
(* `[lim F in T]`                                                             *)
(*     ~ limit of the canonical filter associated to `F` in `T` where         *)
(*       `T` has type `filteredType U`                                        *)
(*                                                                            *)
(* `[cvg F in T]`                                                             *)
(*     ~ the canonical filter associated to `F` converges in `T`              *)
(*                                                                            *)
(* `cvg F`                                                                    *)
(*     ~ same as `[cvg F in T]` where `T` is inferred from the type of        *)
(*       the canonical filter associated to `F`                               *)
(*                                                                            *)
(* `Filter F`                                                                 *)
(*     ~ type class proving that the set of sets `F` is a filter              *)
(*                                                                            *)
(* `ProperFilter F`                                                           *)
(*     ~ type class proving that the set of sets `F` is a proper filter       *)
(*                                                                            *)
(* `UltraFilter F`                                                            *)
(*     ~ type class proving that the set of sets `F` is an ultrafilter        *)
(*                                                                            *)
(* `filter_on T`                                                              *)
(*     ~ interface type for sets of sets on `T` that are filters              *)
(*                                                                            *)
(* `FilterType F FF`                                                          *)
(*     ~ packs the set of sets `F` with the proof `FF` of `Filter F` to       *)
(*       build a `filter_on T` structure                                      *)
(*                                                                            *)
(* `pfilter_on T`                                                             *)
(*     ~ interface type for sets of sets on `T` that are proper filters       *)
(*                                                                            *)
(* `PFilterPack F FF`                                                         *)
(*     ~ packs the set of sets `F` with the proof `FF` of                     *)
(*       `ProperFilter F` to build a `pfilter_on T` structure                 *)
(*                                                                            *)
(* `fmap f F`                                                                 *)
(*     ~ image of the filter `F` by the function `f`                          *)
(*                                                                            *)
(* `E @[x --> F]`                                                             *)
(*     ~ image of the canonical filter associated to `F` by the function      *)
(*       `(fun x => E)`                                                       *)
(*                                                                            *)
(* `f @ F`                                                                    *)
(*     ~ image of the canonical filter associated to `F` by the function      *)
...
(*                                                                            *)
(* `fmapi f F`                                                                *)
(*     ~ image of the filter `F` by the relation `f`                          *)
(*                                                                            *)
(* ``E `@[x --> F]``                                                          *)
(*     ~ image of the canonical filter associated to `F` by the relation      *)
(*       `(fun x => E)`                                                       *)
(*                                                                            *)
(* ``f `@ F``                                                                 *)
(*     ~ image of the canonical filter associated to `F` by the relation      *)
...
(*                                                                            *)
(* `globally A`                                                               *)
(*     ~ filter of the sets containing `A`                                    *)
(*                                                                            *)
(* `@frechet_filter T`                                                        *)
(*     ~ ``[set S : set T | finite_set (~` S)]`` a.k.a. cofinite              *)
(*       filter                                                               *)
(*                                                                            *)
(* `within D F`                                                               *)
(*     ~ restriction of the filter `F` to the domain `D`                      *)
(*                                                                            *)
(* `principal_filter x`                                                       *)
(*     ~ filter containing every superset of `x`                              *)
(*                                                                            *)
(* `principal_filter_type`                                                    *)
(*     ~ alias for choice types with principal filters                        *)
(*                                                                            *)
(* `subset_filter F D`                                                        *)
(*     ~ similar to `within D F`, but with dependent types                    *)
(*                                                                            *)
(* `powerset_filter_from F`                                                   *)
(*     ~ the filter of downward closed subsets of `F`. Enables use of         *)
(*       near notation to pick suitably small members of `F`                  *)
(*                                                                            *)
(* `in_filter F`                                                              *)
(*     ~ interface type for the sets that belong to the set of sets `F`       *)
(*                                                                            *)
(* `InFilter FP`                                                              *)
(*     ~ packs a set `P` with a proof of `F P` to build a `in_filter F`       *)
(*       structure                                                            *)
(*                                                                            *)
...
(* ### Notations                                                              *)
(* `{near F, P}`                                                              *)
(*     ~ the property `P` holds near the canonical filter associated to       *)
(*       `F` `P` must have the form `forall x, Q x`. Equivalent to            *)
(*       `F Q`. Prefer this notation when `P` is an existing statement        *)
(*       to be relativised (i.e., has its own definition).                    *)
(*                                                                            *)
(* `\forall x \near F, P x`                                                   *)
(*     ~ `F (fun x => P x)`. Prefer this notation when the statement          *)
...
(*                                                                            *)
(* `{near F & G, P}`                                                          *)
(*     ~ same as `{near H, P}`, where `H` is the product of the filters       *)
(*       `F` and `G`                                                          *)
(*                                                                            *)
(* `\forall x \near F & y \near G, P x y`                                     *)
...
(*                                                                            *)
(* `\forall x & y \near F, P x y`                                             *)
(*     ~ same as before, with `G = F`                                         *)
(*                                                                            *)
(* `\near x & y, P x y`                                                       *)
(*     ~ `\forall z \near x & t \near y, P x y`                               *)
(*                                                                            *)
(* `x \is_near F`                                                             *)
(*     ~ `x` belongs to a set `P : in_filter F`                               *)
(*                                                                            *)
(* ### Tactics                                                                *)
(* - `near=> x` introduces x.                                                 *)
