...
(*                                                                            *)
(* `set_fun A B f`                                                            *)
(*     ~ `f : aT -> rT` is a function with domain `A : set aT` and            *)
(*       codomain `B : set rT`                                                *)
(*                                                                            *)
(* `set_surj A B f`                                                           *)
(*     ~ `f` is surjective                                                    *)
...
(*                                                                            *)
(* `{fun A >-> B}`                                                            *)
(*     ~ type of functions `f : aT -> rT` from `A : set aT` to `B : set rT`   *)
(*       `funS f` is a proof of `set_fun A B f`                               *)
(*                                                                            *)
(* `{oinv aT >-> rT}`                                                         *)
(*     ~ type of functions with a partial inverse                             *)
(*                                                                            *)
(* `{oinvfun A >-> B}`                                                        *)
(*     ~ combination of `{fun A >-> B}` and `{oinv aT >-> rT}`                *)
(*                                                                            *)
(* `{inv aT >-> rT}`                                                          *)
(*     ~ type of functions with an inverse                                    *)
(*                                                                            *)
(* `f ^-1`                                                                    *)
(*     ~ inverse of `f : {inv aT >-> rT}`                                     *)
(*                                                                            *)
(* `{invfun A >-> B}`                                                         *)
(*     ~ combination of `{fun A >-> B}` and `{inv aT >-> rT}`                 *)
(*                                                                            *)
(* `{surj A >-> B}`                                                           *)
(*     ~ type of surjective functions                                         *)
(*                                                                            *)
(* `{surjfun A >-> B}`                                                        *)
(*     ~ combination of `{fun A >-> B}` and `{surj A >-> B}`                  *)
(*                                                                            *)
(* `{splitsurj A >-> B}`                                                      *)
(*     ~ type of surjective functions with an inverse                         *)
(*                                                                            *)
(* `{splitsurjfun A >-> B}`                                                   *)
(*     ~ combination of `{fun A >-> B}` and `{splitsurj A >-> B}`             *)
(*                                                                            *)
(* `{inj A >-> rT}`                                                           *)
(*     ~ type of injective functions                                          *)
(*                                                                            *)
(* `{injfun A >-> B}`                                                         *)
(*     ~ combination of `{fun A >-> B}` and `{inj A >-> rT}`                  *)
(*                                                                            *)
(* `{splitinj A >-> B}`                                                       *)
(*     ~ type of injective functions with an inverse                          *)
(*                                                                            *)
(* `{splitinjfun A >-> B}`                                                    *)
(*     ~ combination of `{fun A >-> B}` and `{splitinj A >-> B}`              *)
(*                                                                            *)
(* `{bij A >-> B}`                                                            *)
(*     ~ combination of `{injfun A >-> B}` and `{surjfun A >-> B}`            *)
(*                                                                            *)
(* `{splitbij A >-> B}`                                                       *)
(*     ~ combination of `{splitinj A >-> B}` and `{splitsurj A >-> B}`        *)
(*                                                                            *)
(* `'inj_ f`                                                                  *)
(*     ~ proof of `{in A &, injective f}` where `f` has type                  *)
(*       `{splitinj A >-> _}`                                                 *)
(*                                                                            *)
(* `[fun f in A]`                                                             *)
(*     ~ the function `f` from the set `A` to the set ``f @` A`` with         *)
(*       `f : aT -> rT` and `A : set aT`                                      *)
(*       This is a notation for `funin A f`, which is an HB alias.            *)
(*                                                                            *)
(* `'split_ d f`                                                              *)
(*     ~ partial injection from `aT : Type` to `rT : Type`, `f : aT -> rT`,   *)
(*       and `d : rT -> aT`                                                   *)
(*       This is a notation for `split_ d f` which is an HB alias.            *)
(*                                                                            *)
(* `split`                                                                    *)
(*     ~ `'split_(fun=> point)`                                               *)
(*                                                                            *)
(* `@to_setT T`                                                               *)
(*     ~ function that associates to `x : T` a dependent pair of `x` with     *)
(*       a proof that `x` belongs to `setT` (i.e., the type                   *)
(*       `set_type [set: T]`)                                                 *)
(*                                                                            *)
(* `incl AB`                                                                  *)
(*     ~ identity function from `T` to `T`, where `AB` is a proof of          *)
(*       ``A `<=` B``, with `A, B : set T`                                    *)
(*                                                                            *)
(* `inclT A`                                                                  *)
(*     ~ `incl (@subsetT _ _)`                                                *)
...
(*                                                                            *)
(* `mkfun fAB`                                                                *)
(*     ~ builds a function `{fun A >-> B}` given a function `f : aT -> rT`    *)
(*       and a proof `fAB` that `{homo f : x / A x >-> B x}`                  *)
(*                                                                            *)
(* `@set_val T A`                                                             *)
(*     ~ injection from `set_type A` to `T`, where `A` has type `set T`       *)
(*                                                                            *)
(* `@ssquash T`                                                               *)
(*     ~ function of type `{splitsurj [set: T] >-> [set: $| T |]}`            *)
(*                                                                            *)
(* `@finset_val T X`                                                          *)
(*     ~ function that turns an element `x : X` (with `X : {fset T}`) into    *)
(*       a dependent pair of `x` with a proof that `x` belongs to `X`         *)
(*       (i.e., the type ``set_type [set` X]``)                               *)
(*                                                                            *)
(* `@val_finset T X`                                                          *)
(*     ~ function of type ``[set` X] -> X`` with `X : {fset T}` that          *)
(*       cancels `finset_val`                                                 *)
(*                                                                            *)
(* `glue XY AB f g`                                                           *)
(*     ~ function that behaves as `f` over `X`, as `g` over `Y`               *)
(*       `XY` is a proof that sets `X` and `Y` are disjoint, `AB` is a        *)
(*       proof that sets `A` and `B` are disjoint, `A` and `B` are            *)
(*       intended to be the ranges of `f` and `g`                             *)
(*                                                                            *)
(* `'pinv_ d A f`                                                             *)
(*     ~ inverse of the function `[fun f in A]` over ``f @` A``, function     *)
(*       `d` outside of ``f @` A``                                            *)
(*       This is a notation for `pinv_` which is defined as the inverse       *)
(*       of a partial injection (`'split_`).                                  *)
(*                                                                            *)
(* `pinv`                                                                     *)
(*     ~ `'pinv_(fun=> point)`                                                *)
(*                                                                            *)
...
(* `patch d A f`                                                              *)
(*     ~ "partial function" that behaves as the function `f` over the         *)
(*       set `A` and as the function `d` otherwise                            *)
(*                                                                            *)
(* `restrict D f`                                                             *)
(*     ~ `patch (fun=> point) D f`                                            *)
(*                                                                            *)
...
(*                                                                            *)
(* `sigL A f`                                                                 *)
(*     ~ "left restriction"; given a set `A : set U` and a function           *)
(*       `f : U -> V`, returns the corresponding function of type             *)
(*       `set_type A -> V`                                                    *)
(*                                                                            *)
(* `sigR A f`                                                                 *)
(*     ~ "right restriction"; given a set `B : set V` and a function          *)
(*       `f : {fun [set: U] >-> B}`, returns the corresponding function       *)
(*       of type `U -> set_type B`                                            *)
(*                                                                            *)
(* `sigLR A B f`                                                              *)
(*     ~ the function of type `set_type A -> set_type B` corresponding to     *)
(*       `f : {fun A >-> B}`                                                  *)
(*                                                                            *)
(* `valL_ v`                                                                  *)
(*     ~ function cancelled by `sigL A`, with `A : set U` and `v : V`         *)
(*                                                                            *)
(* `valR f`                                                                   *)
(*     ~ the function of type `U -> V` corresponding to                       *)
(*       `f : U -> set_type B`, with `B : set V`                              *)
(*                                                                            *)
(* `valR_fun`                                                                 *)
(*     ~ the function of type `{fun [set: U] >-> B}` corresponding to         *)
(*       `f : U -> set_type B`, with `B : set V`                              *)
(*                                                                            *)
(* `valLR v f`                                                                *)
(*     ~ the function of type `U -> V` corresponding to                       *)
(*       `f : set_type A -> set_type B` (where `v : V`), i.e.,                *)
(*       `'valL_ v \o valR_fun`                                               *)
(*                                                                            *)
(* `valLfun_ v A B f`                                                         *)
(*     ~ `[fun of valL_ f]` with `f : {fun [set: A] >-> B}`                   *)
(*                                                                            *)
(* `valL`                                                                     *)
(*     ~ `'valL_ point`                                                       *)
(*                                                                            *)
(* `valLRfun v`                                                               *)
(*     ~ `'valLfun_ v \o valR_fun`                                            *)
(*                                                                            *)
(* `Section function_space`                                                   *)
(*     ~ canonical `nmodType`, `zmodType`, `ringType`, `comRingType`, and     *)
(*       `lmodType` structures for functions whose range is `nmodType`,       *)
(*       `zmodType`, `ringType`, `comRingType`, and `lmodType` resp.          *)
(*                                                                            *)
(* `fctE`                                                                     *)
(*     ~ multi-rule for `fct`                                                 *)
(*                                                                            *)
(* `linfun E F s`                                                             *)
(*     ~ membership predicate for linear functions of type `E -> F` with      *)
...
(*                                                                            *)
(* `linfun_spec f`                                                            *)
(*     ~ specification for membership of the linear function `f`              *)
(*                                                                            *)
(*                                                                            *)
(*                                                                            *)
(******************************************************************************)
