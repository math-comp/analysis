...
(*                                                                            *)
(* `neitv i`                                                                  *)
(*     ~ the interval `i` is non-empty                                        *)
(*       when the support type is a `numFieldType`, this is equivalent        *)
(*       to `(i.1 < i.2)%O` (lemma `neitvE`)                                  *)
(*                                                                            *)
(* `set_itv_infty_set0`                                                       *)
(*     ~ multirule to simplify empty intervals                                *)
(*                                                                            *)
(* `line_path a b t`                                                          *)
(*     ~ `(1 - t) * a + t * b`, convexity operator over a `numDomainType`     *)
(*                                                                            *)
(* `ndline_path`                                                              *)
(*     ~ `line_path a b` with the constraint that `a < b`                     *)
(*                                                                            *)
(* `factor a b x`                                                             *)
(*     ~ `(x - a) / (b - a)`                                                  *)
(*                                                                            *)
(* `set_itvE`                                                                 *)
(*     ~ multirule to turn intervals into inequalities                        *)
(*                                                                            *)
(* `disjoint_itv i j`                                                         *)
(*     ~ intervals `i` and `j` are disjoint                                   *)
(*                                                                            *)
(* `itv_is_open_unbounded i`                                                  *)
(*     ~ `i` is either `` `]x, +oo[ ``, `` `]-oo, x[ `` or `` `]-oo, +oo[ ``  *)
(*                                                                            *)
(* `itv_is_oo i`                                                              *)
(*     ~ `i` is `` `]x, y[ ``                                                 *)
(*                                                                            *)
(* `itv_open_ends i`                                                          *)
(*     ~ `i` has open endpoints, i.e., it is one of the two above             *)
(*                                                                            *)
(* `itv_is_closed_unbounded i`                                                *)
(*     ~ `i` is either `` `[x, +oo[ ``, `` `]-oo, x] `` or `` `]-oo, +oo[ ``  *)
(*                                                                            *)
(* `itv_is_cc i`                                                              *)
(*     ~ `i` is `` `[x, y] ``                                                 *)
(*                                                                            *)
(* `itv_closed_ends i`                                                        *)
(*     ~ `i` has closed endpoints, i.e., it is one of the two above           *)
(*                                                                            *)
(* `is_open_itv A`                                                            *)
(*     ~ the set `A` can be written as an open interval                       *)
(*                                                                            *)
(* `open_itv_cover A`                                                         *)
(*     ~ the set `A` can be covered by open intervals                         *)
(*                                                                            *)
