...
Fact floorR_subproof x : {m | ZtoR m <= x < ZtoR (m + 1)}.
Proof.
without loss x_ge0: x / x >= 0.
have [x_ge0 | /ltrW x_le0] := ger0P x; first exact.
  ...
  case: eqP => [-> _ | _ /and3P[lt_x_m _ le_m_x]].
    by exists (- m); rewrite lerr rmorphD ltr_addl ltr01.
  by exists (- m - 1); rewrite le_m_x subrK.
have /ex_minnP[n lt_x_n1 min_n]: exists n, x < n.+1%:R.
  exists (Num.bound x).
  by apply: ltr_trans (archi_boundP x_ge0) _; rewrite ltr_nat.
exists n%:Z. rewrite addrC -intS lt_x_n1 andbT.
case Dn: n => // [n1]; rewrite -Dn.
have [//|] := @lerP _ n%:R x; rewrite ?rpred_nat //.
by rewrite Dn => /min_n; rewrite Dn ltnn.
Qed.


Definition ifloor x : int := sval (@floorR_subproof x).
Definition floor x : R := ZtoR (ifloor x).

Lemma ifloorP x : ZtoR (ifloor x) <= x < ZtoR ((ifloor x) + 1).
Proof. exact: (svalP (floorR_subproof x)). Qed.

Lemma floorP x : floor x <= x < floor x + 1.
Proof.
have -> : floor x + 1 = intr (ifloor x + 1) by rewrite rmorphD.
exact: ifloorP.
Qed.

Definition Rint := [qualify a x : R | floor x == x].
Fact Rint_key : pred_key Rint. Proof. by []. Qed.
Canonical Rint_keyed := KeyedQualifier Rint_key.

Lemma Rint_def x : (x \is a Rint) = (floor x == x).
Proof. by []. Qed.

Lemma ifloor_intr z : ifloor z%:~R = z.
Proof.
by apply/eqP; have:= ifloorP z%:~R; rewrite  ler_int ltr_int ltz_addr1 -eqr_le.
Qed.

Lemma RintC z : z%:~R \is a Rint.
Proof. by rewrite Rint_def /floor ifloor_intr. Qed.
...
Lemma RintP x : reflect (exists z : int, x = z%:~R) (x \in Rint).
Proof.
apply: (iffP idP); last by case=> z ->; exact: RintC.
by rewrite Rint_def=> /eqP e; exists (ifloor x); rewrite -[LHS]e.
Qed.
...
Definition Rtoint (x : R) : int := ifloor x.

Lemma RtointK (x : R): x \is a Rint -> (Rtoint x)%:~R = x.
Proof. by case/RintP=> z ->; rewrite /Rtoint ifloor_intr. Qed.

Lemma Rtointz (z : int): Rtoint z%:~R = z.
Proof. by rewrite /Rtoint ifloor_intr. Qed.
...
by rewrite has_supP; split; first by exists x.
Qed.
...
Proof.
rewrite /has_ub not_exists -propeqE; apply: eq_forall => x.
rewrite inE asboolE; apply: notLR; rewrite not_exists2. (* still some missing infra *)
by apply: eq_forall => y; rewrite notb lerNgt. (* missing eq_imply *)
Qed.
...
Proof.
move=> neE.
suff -> : has_sup E = has_ub E by exact: has_ubPn.
by rewrite /has_sup propeqE; intuition.
Qed.
...
Proof.
rewrite has_infP has_supP.
suff -> : nonempty (lb E) = nonempty (ub (-` E)) by rewrite nonemptyN.
rewrite /nonempty propeqE; split; case=> x hx; exists (- x); apply/lb_ubN=> //.
by rewrite opprK.
Qed.
...
Proof.
split; rewrite has_lbP; first by case=> x /lb_ubN hx; exists (- x).
by case=> x /ub_lbN; rewrite setNK => hx; exists (- x).
Qed.
...
by move/has_lbPn; rewrite has_infP; intuition.
Qed.
