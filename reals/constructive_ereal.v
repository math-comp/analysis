...
(* `\bar R`                                                                   *)
(*     ~ coproduct of `R` and `{+oo, -oo}`                                    *)
(*     ~ notation for `extended (R:Type)`                                     *)
(*                                                                            *)
(* `r%:E`                                                                     *)
(*     ~ injects real numbers into `\bar R`                                   *)
(*                                                                            *)
(* `- x`                                                                      *)
(*     ~ opposite of `x : \bar R`                                             *)
(*                                                                            *)
...
(*                                                                            *)
(* `<number>`                                                                 *)
(*     ~ `<number>%:R%:E` with `<number>` a sequence of digits                *)
(*                                                                            *)
(* `-<number>`                                                                *)
(*     ~ `- <number>%:R%:E` with `<number>` a sequence of digits              *)
(*                                                                            *)
(* `+%E, -%E, *%E`                                                            *)
(*     ~ addition/opposite/multiplication for extended reals                  *)
(*                                                                            *)
(* `x^-1, x / y`                                                              *)
(*     ~ inverse and division for extended reals                              *)
(*                                                                            *)
(* `er_map (f : T -> T')`                                                     *)
(*     ~ the `\bar T -> \bar T'` lifting of `f`                               *)
(*                                                                            *)
(* `sqrte`                                                                    *)
(*     ~ square root for extended reals                                       *)
(*                                                                            *)
(* `` `| x |%E ``                                                             *)
...
(*                                                                            *)
(* `+%dE, (x *+ n)%dE`                                                        *)
(*     ~ dual addition/dual iterated addition for extended reals              *)
(*       (`-oo + +oo = +oo` instead of `-oo`)                                 *)
(*     ~ Import `DualAddTheory` for related lemmas                            *)
(*                                                                            *)
(* `x +? y`                                                                   *)
(*     ~ the addition of the extended real numbers `x` and `y` is             *)
(*       defined, i.e., it is neither `+oo - oo` nor `-oo + oo`               *)
(*                                                                            *)
(* `x *? y`                                                                   *)
(*     ~ the multiplication of the extended real numbers `x` and `y`          *)
(*       is not of the form `0 * +oo` or `0 * -oo`                            *)
(*                                                                            *)
(* `x *^-1? y`                                                                *)
(*     ~ the inverse commutes with the product                                *)
(*                                                                            *)
(* `(_ <= _)%E, (_ < _)%E, (_ >= _)%E, (_ > _)%E`                             *)
(*     ~ comparison relations for extended reals                              *)
(*                                                                            *)
(* `(\sum_(i in A) f i)%E`                                                    *)
(*     ~ bigop-like notation in scope `%E`                                    *)
(*                                                                            *)
(* `(\prod_(i in A) f i)%E`                                                   *)
(*     ~ bigop-like notation in scope `%E`                                    *)
(*                                                                            *)
(* `maxe x y, mine x y`                                                       *)
(*     ~ notation for the maximum/minimum of two extended real                *)
(*       numbers                                                              *)
(*                                                                            *)
(* `esg x`                                                                    *)
(*     ~ sign of `x` (`0`, `-1`, or `+1`, as an extended real)                *)
(*                                                                            *)
(* ## Signed extended real numbers                                            *)
(* `{posnum \bar R}`                                                          *)
(*     ~ interface type for elements in `\bar R` that are positive,           *)
(*       c.f., `interval_inference.v`, notation in scope `%E`                 *)
(*                                                                            *)
(* `{nonneg \bar R}`                                                          *)
(*     ~ interface types for elements in `\bar R` that are                    *)
(*       non-negative, c.f. `interval_inference.v`, notation in               *)
(*       scope `%E`                                                           *)
(*                                                                            *)
(* `x%:pos`                                                                   *)
(*     ~ explicitly casts `x` to `{posnum \bar R}`, in scope `%E`             *)
(*                                                                            *)
(* `x%:nng`                                                                   *)
(*     ~ explicitly casts `x` to `{nonneg \bar R}`, in scope `%E`             *)
(*                                                                            *)
(* ## Topology of extended real numbers                                       *)
(* `contract`                                                                 *)
(*     ~ order-preserving bijective function from extended real               *)
(*       numbers to `[-1; 1]`                                                 *)
(*                                                                            *)
(* `expand`                                                                   *)
(*     ~ function from real numbers to extended real numbers that             *)
(*       cancels `contract` in `[-1; 1]`                                      *)
(******************************************************************************)
