...
have [x [Ax /andP[sAex xsA]]] : exists x, A x /\ sup A - e%:num < x < sup A.
  ...
  have /(sup_le_ub A0) : classical_sets.ubound A (sup A - e%:num).
...
Local Coercion mixin : class_of >-> mixin_of.
Local Coercion lalg_mixin : class_of >-> GRing.Lalgebra.axiom.
Local Coercion alg_mixin : class_of >-> GRing.Algebra.axiom.
...
Definition RComplex_lmodClass 
    {V} (M : GRing.Lmodule.class_of C V) :=
  (@GRing.Lmodule.Class R V M (RComplex_lmodMixin M)). 

Canonical asR_lmodType (M : lmodType C) := 
  GRing.Lmodule.Pack (Phant R)  
    (RComplex_lmodClass  (GRing.Lmodule.class M)).

Lemma RComplex_scaleE {V : lmodType C} (r : R) (x : asR_lmodType V) :
  r *: x = (r +i* 0)%C *: x.
Proof. by rewrite /GRing.scale /=. Qed.


Lemma RComplex_lalg_mixin 
    {V} m (M : @GRing.Lalgebra.axiom C V m) : 
  (@GRing.Lalgebra.axiom R (asR_lmodType V) m).
Proof.
...
Lemma RComplex_alg_mixin 
    {V} (M : @GRing.Algebra.axiom C V) : 
  (@GRing.Algebra.axiom R (asR_lalgType V)).
Proof.
  by move=> r x y; rewrite RComplex_scaleE M -RComplex_scaleE.
Qed.

Definition RComplex_BanachAlg_Mixin 
    {V } (M : @BanachAlgebra.class_of C V) : 
@BanachAlgebra.mixin_of R V
  (@normr R
     (@CompleteNormedModule.Pack R (Phant R) V
        (@RComplex_CompleteNormedModClass V M)))
  (@GRing.Ring.mul
     (@GRing.Zmodule.Pack V (@RComplex_CompleteNormedModClass V M)) M)
  (@GRing.Ring.one
     (@GRing.Zmodule.Pack V (@RComplex_CompleteNormedModClass V M)) M).
Proof.
  constructor.
  - move=> x y; case: (BanachAlgebra.mixin M) => [/(_ x y) + _]; rewrite lecE.
    move=> /andP [_].
    set l := (x in (Re (x * _))).
    set r := (x in (Re (_ * x))).
    have : 0 <= l by [].
    have : 0 <= r by [].
    rewrite {}/l {}/r.
    rewrite /normr /= ?normc_Re/= ?lecE /= =>
      /andP [/eqP xI _] /andP [/eqP yI _].
    rewrite [x in x * _]complexE [x in _ * x]complexE. 
    by rewrite xI yI /= ?mul0r ?mulr0 ?subr0 ?addr0 ?mulr0 subr0.
  - rewrite /banach_alg_ax2.
    case: (BanachAlgebra.mixin M) => _ .
    rewrite /banach_alg_ax2 /= /normr /= => ->.
    by rewrite /= expr0n addr0 expr1n sqrtr1.
Qed.

Definition RComplex_BanachAlgClass 
    {V} ( M : BanachAlgebra.class_of C V) :=
  @BanachAlgebra.Class R V (RComplex_CompleteNormedModClass M)
  M M (RComplex_lalg_mixin (@BanachAlgebra.lalg_mixin _ _ M))
  (RComplex_alg_mixin (@BanachAlgebra.alg_mixin _ _ M))
