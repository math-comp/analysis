...
Proof.
apply: (bij_inj (@to_wedge_bij i)).
by rewrite /from_wedge_sub /= [LHS]invK ?path_zero ?in_setT.
Qed.

Local Lemma from_wedge_one : from_wedge_sub one = one.
Proof.
apply: (bij_inj (@to_wedge_bij i)).
by rewrite /from_wedge_sub /= [LHS]invK ?path_one ?in_setT.
Qed.

Local Lemma from_wedge_cts : continuous from_wedge_sub.
Proof.
apply/continuousP => A /to_wedge_open; congr (_ _).
apply/seteqP; split=> [_/= [z Az <-]|z/= Az].
  by rewrite /from_wedge_sub/= funK ?in_setT.
by eexists; [exact: Az|rewrite /from_wedge_sub/= [LHS]invK ?in_setT].
Qed.

HB.instance Definition _ := isContinuous.Build _ _
  from_wedge_sub from_wedge_cts.
...
Proof. by move=> z; rewrite unlock/= /from_wedge_sub [LHS]invK ?in_setT. Qed.

Lemma to_wedgeK {i : selfSplitType}: cancel (@to_wedge i) from_wedge.
Proof. by  move=> z; rewrite unlock/= /from_wedge_sub funK ?in_setT. Qed.
...
Let i_i := @bpwedge i i.
Local Open Scope quotient_scope.
Local Notation "f '<>' g" := (@path_concat _ i _ _ _ f g).

Lemma concat_cstr {T : topologicalType} (x y : T) (f : {path i from x to y}) :
...
Lemma concat_cstl {T : topologicalType} (x y : T) (f : {path i from x to y}) :
...
rewrite /associ/= /chain_path /mk_path/= !wedge_lift_funE//.
  by move=> [] []//; rewrite !bpwedgeE.
move=> [] []//=; rewrite ?bpwedgeE//= ?wedge_lift_funE/= ?bpwedgeE//=.
  by move=> [] []//=; rewrite !bpwedgeE.
by move=> [] []//=; rewrite !bpwedgeE.
Qed.
...
HB.mixin Record isPathDomain {d} (i : Type) of
  ...
  zero_bot : forall (y : i), (@zero i <= y)%O;
  one_top : forall (y : i), (y <= @one i)%O
}.
