...
Lemma measurable_pair1 (f : T1 * T2 -> T3) (y : T2) :
  measurable_fun [set: T1 * T2] f ->
  measurable_fun [set: T1] (fun x => f (x, y)).
Proof. by move=> mf; exact: measurableT_comp. Qed.

Lemma measurable_pair2 (f : T1 * T2 -> T3) (x : T1) :
  measurable_fun [set: T1 * T2] f ->
  measurable_fun [set: T2] (fun y => f (x, y)).
Proof. by move=> mf; exact: measurableT_comp. Qed.
...
HB.mixin Record isSigmaFiniteTransitionKernel
    d d' (X : measurableType d) (Y : measurableType d')
    (R : realType) (k : X -> {measure set Y -> \bar R}) := {
  sigma_finite_finite_transition_kernel :
    forall x, sigma_finite [set: Y] (k x) }.
...
Lemma measurable_k1comp (k : R.-ftker (T0 * T1) ~> T2) A : measurable A ->
  measurable_fun [set: T0 * T1] (k1comp k A).
Proof.
...
rewrite (_ : (fun n => _) = (fun n =>
  ...
  apply/funext => n; rewrite ge0_integral_sum//.
    by move=> m; exact: measurable_pair2 (measurable_k1comp k2 (mA m)).
...
- move=> n.
  ...
  exact: measurable_pair2 (measurable_k1comp k2 (mA m)).
...
Let kkcomp_dep_ge0 x A : 0 <= kkcomp_dep k1 k2 x A.
Proof.
...
Theorem sigma_finite_kkcomp_dep x :
  sigma_finite [set: T1 * T2] (kkcomp_dep k1 k2 x).
Proof.
...
Let kernel_snd_ge0 x A : 0 <= kernel_snd x A.
...
Let sprob_kernel :
  ereal_sup [set (mkernel_snd) z [set: T2] | z in [set: T0 * T1]] <= 1.
Proof.
...
(* thm 14.26 b *)
Lemma subprob_kkcomp x : mkkcomp k1 k2 x [set: T2] <= 1.
Proof.
