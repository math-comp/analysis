...
(*                                                                            *)
(* `R.-ker X ~> Y`                                                            *)
(*     ~ kernel from `X` to `Y` where `X` and `Y` are of type                 *)
(*       `measurableType`. The HB class is `Kernel`.                          *)
(*                                                                            *)
(* `kseries`                                                                  *)
(*     ~ countable sum of kernels; it is declared as an instance of the       *)
(*       structure `Kernel`. It will also be shown to be an instance of       *)
(*       `SFiniteKernel` if the sum is over s-finite kernels.                 *)
(*                                                                            *)
(* `measure_fam_uub k`                                                        *)
(*     ~ the kernel `k` is uniformly upper-bounded                            *)
(*                                                                            *)
(* `R.-sfker X ~> Y`                                                          *)
(*     ~ s-finite kernel. The HB class is `SFiniteKernel`.                    *)
(*                                                                            *)
(* `kzero`                                                                    *)
(*     ~ kernel defined using the `mzero` measure                             *)
(*                                                                            *)
(* `R.-sigmafker X ~> Y`                                                      *)
(*     ~ sigma-finite transition kernel. The HB class is                      *)
(*       `SigmaFiniteTransitionKernel`.                                       *)
(*                                                                            *)
(* `R.-ftker X ~> Y`                                                          *)
(*     ~ finite transition kernel. The HB class is                            *)
(*       `FiniteTransitionKernel`.                                            *)
(*                                                                            *)
(* `R.-fker X ~> Y`                                                           *)
(*     ~ finite kernel. The HB class is `FiniteKernel`.                       *)
(*                                                                            *)
(* `R.-spker X ~> Y`                                                          *)
(*     ~ subprobability kernel. The HB class is `SubProbabilityKernel`.       *)
(*                                                                            *)
(* `R.-pker X ~> Y`                                                           *)
(*     ~ probability kernel. The HB class is `ProbabilityKernel`.             *)
(*                                                                            *)
(* `kdirac mf`                                                                *)
(*     ~ kernel defined by a measurable function                              *)
(*                                                                            *)
(* `kprobability m`                                                           *)
(*     ~ kernel defined by a probability measure                              *)
(*                                                                            *)
(* `kadd k1 k2`                                                               *)
(*     ~ lifting of the addition of measures to kernels                       *)
(*                                                                            *)
(* `knormalize f P`                                                           *)
(*     ~ `fun x => mnormalize (f x) P`                                        *)
(*                                                                            *)
(* `kcomp l k`                                                                *)
(*     ~ "parameterized" composition for kernels; `l` has type                *)
(*       `X -> {measure set Y -> \bar R}`; `k` has type                       *)
(*       `X * Y -> {measure set Z -> \bar R}`; `kcomp l k` as type            *)
(*       `X -> set Z -> \bar R`                                               *)
(*                                                                            *)
(* `l \; k`                                                                   *)
(*     ~ same as `kcomp l k` but equipped with the type                       *)
(*       `X -> {measure set Z -> \bar R}`                                     *)
(*                                                                            *)
(* `kfcomp k f`                                                               *)
(*     ~ composition of a "kernel" and a function; `k` has type               *)
(*       `X -> {measure set Y -> \bar R}`.                                    *)
(*                                                                            *)
(* `kproduct k1 k2`                                                           *)
(*     ~ "parameterized" product of kernels; `k1` has type                    *)
(*       `T0 -> {measure set T1 -> \bar R}`; `k2` has type                    *)
(*       `T0 * T1 -> {measure set T2 -> \bar R}`; the result has type         *)
(*       `T0 -> set (T1 * T2) -> \bar R`.                                     *)
(*                                                                            *)
(* `mkproduct k1 k2`                                                          *)
(*     ~ same as `kproduct k1 k2` but equipped with the type                  *)
(*       `T0 -> {measure set (T1 * T2) -> \bar R}`                            *)
(*                                                                            *)
(* `kproduct_snd k1 k2`                                                       *)
(*     ~ `kproduct k1 (k2 \o snd)`                                            *)
(*     ~ `k2` has `T1 -> {measure set T2 -> \bar R}`; it "ignores" the        *)
(*       first argument of `k2`                                               *)
(*                                                                            *)
(* `kcomp_noparam k1 k2`                                                      *)
(*     ~ composition of kernels; `k1` has type                                *)
(*       `T0 -> {measure set T1 -> \bar R}`; `k2` has type                    *)
(*       `T1 -> {measure set T2 -> \bar R}`; the return type is               *)
(*       `T0 -> set (T1 * T2) -> \bar R`                                      *)
(*                                                                            *)
(******************************************************************************)
