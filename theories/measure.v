...
(*                              interfaces: isOuterMeasure, isOuterMeasureAlt *)
(* ```                                                                        *)
(*                                                                            *)
...
HB.factory Record isOuterMeasureAlt
    (R : realType) d (T : semiRingOfSetsType d) (mu : set T -> \bar R) := {
  outer_measure0Alt : mu set0 = 0 ;
  outer_measure_ge0Alt : forall x, 0 <= mu x ;
  outer_measure_sigma_subadditiveAlt :
    forall A F, subset_sigma_subadditive mu A F}.

HB.builders Context {R : realType} d T mu of isOuterMeasureAlt R d T mu.

Lemma le_outer_measure : {homo mu : A B / A `<=` B >-> A <= B}.
Proof.
move=> A B AB; pose B_ k := if k is 0%N then B else set0.
have -> : mu B = \sum_(n <oo) mu (B_ n).
  rewrite nneseries_recl; last by move=> ?; rewrite outer_measure_ge0Alt.
  rewrite eseries_cond/= eseries0 ?adde0// => -[|]//= k _ _.
  by rewrite outer_measure0Alt.
apply: outer_measure_sigma_subadditiveAlt => //.
by rewrite bigcup_recl/= bigcup0 ?setU0// => -[/negP|].
Qed.

Lemma outer_measure_sigma_subadditive : sigma_subadditive mu.
Proof. by move=> F; exact: outer_measure_sigma_subadditiveAlt. Qed.

HB.instance Definition _ := isOuterMeasure.Build R T mu outer_measure0Alt
  outer_measure_ge0Alt le_outer_measure outer_measure_sigma_subadditive.
