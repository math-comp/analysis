...
(*                       countable union                                      *)
(* monotone_class D G == G is a monotone class (or lambda-system) of subsets  *)
(*                       of D                                                 *)
...
(**md see, e.g., the definition of "class monotone" on the French wikipedia *)
...
Hypothesis monoH : monotone_class D <<m D, G >>.
...
have GGH_ X : G X -> G `<=` H_ X.
  move=> GX.
  rewrite /H_ => A GA; split.
    exact: sub_smallest GA.
  apply: (@sub_smallest _ _ _ G) => //.
  exact: setIG.
have HD X : H X -> X `<=` D by move=> ?; case: monoH => + _ _ _; apply.
have GHH_ X : G X -> H `<=` H_ X.
  move=> GX; apply: smallest_sub; last exact: GGH_.
  split => //; first by move=> A [HA _]; case: monoH => + _ _ _; exact.
  have XD : X `<=` D.
    apply: HD.
    exact: (@sub_smallest _ _ _ G).
  rewrite /H_ /= setIidr//; split; [by case: monoH|].
  exact: (@sub_smallest _ _ _ G).
...
have HHH_ X : H X -> H `<=` H_ X.
  move=> HX.
  apply: smallest_sub; last exact: HGH_.
  split=> //.
  - by move=> ? [? ?]; case: monoH => + _ _ _; exact.
  - have XD : X `<=` D := HD _ HX.
    by rewrite /H_/= setIidr//; split => //; case: monoH.
by move=> *; apply HHH_.
Qed.
...
(* TODO: move *)
Lemma monotone_class_powerset (T : Type) (D : set T) :
  monotone_class D [set X | X `<=` D].
Proof.
split => //.
...
(* TODO: move *)
Section monotone_class_smallest.
Variables (T : Type) (D : set T) (G : set (set T)).
Hypothesis GD : forall A, G A -> A `<=` D.

(* TODO: (forall E, monotone_class D E -> G `<=` E -> H `<=` E) *)

Lemma monotone_class_smallest : monotone_class D <<m D , G >>.
Proof.
split => [A MA | E [monoE] | A B BA MA MB E [[EsubD ED setDE ndE] GE] |].
- have monoH := monotone_class_powerset D.
...
rewrite -(@smallest_monotone_classE _ _ setIG D) //.
- exact: smallest_sub.
- apply: monotone_class_smallest => A GA.
...
Section sigma_ring_monotone.
Context d (T : sigmaRingType d).

Lemma sigmaRing_monotone (D : set T) : measurable D ->
  monotone_class D [set X | measurable X /\ X `<=` D].
Proof.
move=> mD; split.
- by move=> A /=[].
- by split.
...
- move=> /= F _ mFD; split.
    ...
    by apply mFD.
  by apply: bigcup_sub => i _; apply mFD.
Qed.
(* TODO: monotone ring -> sigma-ring *)

End sigma_ring_monotone.
...
have sDHE : <<s D, H >> `<=` E.
  by apply: monotone_class_subset => //; split => //; [move=> ? []|split].
by move=> X /sDHE[].
Qed.
