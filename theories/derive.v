...
Fact MVT_choice_fun f df a b : 
  ...
  {within `[a, b], continuous f} ->
  { c : R -> R | forall (x : R), x \in `]a, b[%R -> c x \in `]a, x[%R /\ f x - f a = df (c x) * (x - a) }.
Proof.
move=> a_lt_b der_f cf.
suff [c hc] : { c : `]a, b[ -> R & forall (x : `]a, b[), c x \in `]a, (\val x)[%R /\ f (\val x) - f a = df (c x) * (\val x - a) }.
  exists (fun x =>
  ...
  set X := (X in c X).
  by have := hc X.
apply: (choice (P := fun x c => c \in `]a, \val x[%R /\ f (\val x) - f a = df c * (\val x - a))).
move=> /= [x /= /set_mem/=].
...
move: (is_df).
rewrite /prop_near1/= {1}/nbhs/= {1}/at_right/= /within/= -nbhs_nearE nbhs_simpl.
rewrite -filter_from_ballE /filter_from/= => -[M M_gt0 M_def].
...
have itv_in_ball : `]a, a + M/2[ `<=` ball a M.
  ...
  - apply: lt_trans; first exact: x_lt_aM2.
    by rewrite ltrD2l. 
have /[swap] /[apply] : forall x : R, x  \in `]a, a + M/2[%R -> is_derive x 1 f (df x).
  move=> x /[dup] x_itv.
