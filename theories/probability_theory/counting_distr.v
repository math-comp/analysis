...
From mathcomp Require Import cardinality fsbigop ereal measure lebesgue_integral.
...
Definition discrete_measurable_space (T : pointedType) : Type := T.

HB.instance Definition _ (T : pointedType) :=
  Pointed.on (discrete_measurable_space T).

HB.instance Definition _ (T : pointedType) := @isMeasurable.Build
...
Lemma summableP (f : T -> R) :
  realsum.summable f <-> summable [set: T] (EFin \o f).
Proof.
...
split.
  move=> [M fM].
  rewrite /summable /esum (@le_lt_trans _ _ M%:E) ?ltey//.
