...
(*                                                                            *)
(* `itv_partition a b s`                                                      *)
(*     ~ `s` is a partition of the interval `` `[a, b] ``                     *)
(*                                                                            *)
(* `itv_partitionL s c`                                                       *)
(*     ~ the left side of splitting a partition at `c`                        *)
(*                                                                            *)
(* `itv_partitionR s c`                                                       *)
(*     ~ the right side of splitting a partition at `c`                       *)
(*                                                                            *)
(* `variation a b f s`                                                        *)
(*     ~ the sum of `f` at all points in the partition `s`                    *)
(*                                                                            *)
(* `variations a b f`                                                         *)
(*     ~ the set of all variations of `f` between `a` and `b`                 *)
(*                                                                            *)
(* `bounded_variation a b f`                                                  *)
(*     ~ all variations of `f` are bounded                                    *)
(*                                                                            *)
(* `{nnfun T >-> R}`                                                          *)
(*     ~ type of non-negative functions                                       *)
(*                                                                            *)
(* `f ^\+`                                                                    *)
(*     ~ the function formed by the non-negative outputs of `f` and `0`       *)
(*       otherwise. The codomain of `f` is the real numbers in scope          *)
(*       `ring_scope` and the extended real numbers in scope                  *)
(*       `ereal_scope`. Rendered as `f ⁺` with company-coq (U+207A).          *)
(*                                                                            *)
(* `f ^\-`                                                                    *)
(*     ~ the function formed by the non-positive outputs of `f` and `0`       *)
(*       o.w. Similar to `^\+`. Rendered as `f ⁻` with company-coq            *)
(*       (U+207B).                                                            *)
(*                                                                            *)
(* `\1_ A`                                                                    *)
