...
(*                                                                            *)
(* `'N[mu]_p[f]`                                                              *)
(*     ~ the Lp-norm of `f` with measure `mu`                                 *)
(*                                                                            *)
(* `hoelder_conjugate p`                                                      *)
(*     ~ an extended real number `q` s.t. `p^-1 + q^-1 = 1`                   *)
(*                                                                            *)
(* Lp-spaces and properties of Lp-norms:                                      *)
(*                                                                            *)
(* `finite_norm mu p f`                                                       *)
(*     ~ the L-norm of real-valued function `f` is finite. The                *)
(*       parameter `p` is an extended real                                    *)
(*                                                                            *)
(* `LfunType mu p1`                                                           *)
(*     ~ type of measurable functions `f` with a finite L-norm. `p1` is       *)
(*       a proof that the extended real number `p` is greater or equal        *)
(*       to `1`. The HB class is `Lfunction`                                  *)
(*                                                                            *)
(* `f \in Lfun`                                                               *)
(*     ~ holds for `f : LfunType mu p1`                                       *)
(*                                                                            *)
(* `ae_eq_op f g`                                                             *)
(*     ~ boolean version of `ae_eq`, `ae_eq_op` is canonically an             *)
(*       equivalence relation                                                 *)
(*                                                                            *)
(* `{mfun_mu, T1 >-> T2}`                                                     *)
(*     ~ the quotient of measurable functions `T1 -> T2`, quotiented by       *)
(*       the equivalence relation `ae_eq_op`                                  *)
(*                                                                            *)
(* `LspaceType mu p1`                                                         *)
(*     ~ type of the elements of the Lp space for the measure `mu`            *)
(*                                                                            *)
(* `mu.-Lspace p`                                                             *)
(*     ~ Lp space as a set                                                    *)
(*                                                                            *)
