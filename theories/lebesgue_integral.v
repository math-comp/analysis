...
Lemma negligible_integral_ge0 (D N : set T) (f : T -> \bar R) :
  measurable N -> measurable D -> measurable_fun D f ->
  (forall x, D x -> 0 <= f x) ->
...
rewrite (negligible_integral_ge0 mN)//; last 2 first.
  - by apply: emeasurable_funM => //; exact/EFin_measurable_fun.
  - by move=> x Dx; apply: mule_ge0 => //; [exact: f0|rewrite lee_fin].
rewrite [RHS](negligible_integral_ge0 mN)//; last 2 first.
  - by apply: emeasurable_funM => //; exact/EFin_measurable_fun.
...
- apply: (integrableS mD) => //; first exact: measurableD. 
  exact: integrable_funepos.
- exact: measurableD.
congr (_ - _); apply: negligible_integral_ge0 => //; apply: measurable_int.
  exact: (integrable_funepos mD mf).
exact: (integrable_funeneg mD mf).
Qed.

End generalize_negligible.
...
Lemma ae_ge0_le_integral : {ae mu, forall x, D x -> f1 x <= f2 x} ->
  \int[mu]_(x in D) f1 x <= \int[mu]_(x in D) f2 x.
Proof.
move=> [N [mN muN f1f2N]]; rewrite (negligible_integral_ge0 _ _ _ _ muN)//.
rewrite [leRHS](negligible_integral_ge0 _ _ _ _ muN)//.
...
exact: (@ball_open rT [normedModType rT of R^o]). 
Qed.
...
have ball_itvr r : 0 < r -> `[x-r, x+r] `\` ball x r  = [set x+r;x-r].
  move: r => _/posnumP[r].
  rewrite -setU1itv /Order.le /= ?lerBlDr -?addrA ?lerDl //.
  rewrite -setUitv1 /Order.le /= ?ltrBlDr -?addrA ?ltrDl //.
  rewrite setUC -setUA setDUl.
  rewrite -(@ball_normE rT [normedModType rT of R^o]) /ball_ .
  rewrite ?set_itvoo; under eq_fun => ? do rewrite -ltr_distlC.
  rewrite setDv set0U setDidl // -subset0 => z /= [[]] ->;
    rewrite -opprB addrAC subrr add0r ?normrN gtr0_norm // ltxx //.
