...
Lemma indic_restrict {T : pointedType} {R : numFieldType} (A : set T) :
  indic A = 1 \|_ A :> (T -> R).
Proof. by apply/funext => x; rewrite /indic /patch; case: ifP. Qed.
...
by rewrite !inE /indic/=; case: (_ \in _) => _ <-; rewrite ?eqxx ?orbT.
Qed.
...
Lemma trivIset_preimage1 {aT rT} (f : aT -> rT) :
  trivIset setT (fun x => f @^-1` [set x]).
Proof. by move=> y z _ _ [x [<- <-]]. Qed.

Lemma trivIset_preimage1_in {aT} {rT : choiceType} (A : set aT) (f : aT -> rT) :
...
Definition nnsfun_cst (r : {nonneg R}) := [the {nnsfun T >-> R} of cst r%:nngnum].

Definition nnsfun0 : {nnsfun T >-> R} := nnsfun_cst 0%R%:nng.

Lemma indic1_nnfun_subproof (D : set T) : @IsNonNegFun T R (\1_D).
...
Lemma sintegralEnnsfun (f : {nnsfun T >-> R}) :
  sintegral mu f =
  (\sum_(x \in [set r | r > 0]%R) (x%:E * mu (f @^-1` [set x])))%E.
Proof.
have ? : finite_set ((fun i => i%:E * mu (f @^-1` [set i])) @^-1` [set~ 0])%E.
...
rewrite in_finite_support; last first.
  rewrite setTI.
  apply: sub_finite_set (@fimfunP _ _ f) => y.
  rewrite /preimage/= => /eqP/set0P[t <-].
  by exists t.
...
Proof.
have := @additive_nnsfun_in setT measurableT g f x; rewrite setTI.
by under eq_fsbigr do rewrite !setTI; under [in RHS]eq_fsbigr do rewrite !setTI.
Qed.
...
have H3 r : finite_set
    ((fun x => m (f @^-1` [set r] `&` g @^-1` [set x])) @^-1` [set~ 0]).
  apply: sub_finite_set (finite_set_measure_preimage1 g m) => x/=.
  apply: contra_not; apply: subset_measure0 => //.
  by apply: measurableI.
have H4 : finite_set ((fun z => m (f @^-1` [set z.2] `&`
                      (g @^-1` [set (z.1 - z.2)%R]))) @^-1` [set~ 0]).
  have : finite_set (
    (fun x => (fun y => m ((g @^-1` [set (y - x)%R]))) @^-1` [set~ 0]) `d*`
    ((fun x => m (f @^-1` [set x])) @^-1` [set~ 0])).
...
have H7 : finite_set
    ((fun x => \sum_(i \in [set: R]) i%:E * m (f @^-1` [set x] `&` (g @^-1` [set i]))) @^-1`
     [set~ 0]).
  have := @fimfunP _ _ f.
  apply: sub_finite_set => r.
  rewrite /preimage /= => H.
  have {}H : (0 < \sum_(i \in [set: R])
      i%:E * m ([set t | f t = r] `&` ([set t | g t = i])))%E.
    rewrite lt_neqAle; apply/andP; split.
      by rewrite eq_sym; exact/eqP.
      apply: fsume_ge0 => //.
      move=> i _.
      apply: (mulem_ge0 (fun i => [set t | f t = r] `&` ([set t | g t = i]))).
      move=> i0.
   by rewrite [X in _ `&` X](preimage_nnfun0 g)// setI0.
  move/fsume_gt0 : H => [i [H]].
  have {}H : (0 < m ([set t | f t = r] `&` ([set t | g t = i])))%E.
    move: H.
    have [i0|i0|->] := ltgtP i 0%R.
    by rewrite nmule_rgt0// ltNge measure_ge0.
    by rewrite pmule_rgt0//.
    by rewrite mul0e ltxx.
  have [x [[Hx] _]] : ([set t | f t = r] `&` ([set t | g t = i])) !=set0.
    apply/set0P/negP => /eqP h.
    by rewrite h measure0 ltxx in H.
  by exists x.
have H8 : finite_set
    ((fun x => \sum_(i \in [set: R]) x%:E * m (f @^-1` [set x] `&` (g @^-1` [set i]))) @^-1`
     [set~ 0]).
  have := @fimfunP _ _ f.
  apply: sub_finite_set => r.
  rewrite /preimage /= => H.
  have {}H : (0 < \sum_(i \in [set: R])
      r%:E * m ([set t | f t = r] `&` ([set t | g t = i])))%E.
    rewrite lt_neqAle; apply/andP; split.
      by rewrite eq_sym; exact/eqP.
      apply: fsume_ge0 => //.
      move=> i _.
      apply: (mulem_ge0 (fun r => [set t | f t = r] `&` ([set t | g t = i]))).
      move=> r0.
    by rewrite [X in X `&` _](preimage_nnfun0 f)// set0I.
  move/fsume_gt0 : H => [i [H]].
  have {}H : (0 < m ([set t | f t = r] `&` ([set t | g t = i])))%E.
    move: H.
    ...
    by rewrite pmule_rgt0//.
    by rewrite mul0e ltxx.
  have [x [[Dx] _]] : ([set t | f t = r] `&` ([set t | g t = i])) !=set0.
    apply/set0P/negP => /eqP h.
    by rewrite h measure0 ltxx in H.
  by exists x.
transitivity (\sum_(z \in [set: R]) z%:E * \sum_(a \in [set: R])
    m ((f @^-1` [set a]) `&` (g @^-1` [set (z - a)%R]))).
  ...
  rewrite measure_fbigsetU; last 2 first.
    - by move=> x ?; apply: measurableI.
    - apply/(@sub_trivIset _ _ _ setT)=>//.
      by apply/trivIset_setIr0/trivIset_preimage1.
  rewrite -fsbigTE; first by [].
  move=> i; rewrite in_fset_set; last exact/fimfunP.
  rewrite notin_set/= => ifD.
  rewrite notin_setI_preimage// notin_set/=.
  by apply: contra_not ifD => -[x]; rewrite /preimage/= => <-; exists x.
under eq_fsbigr.
  move=> z.
...
rewrite exchange_fsum; last 2 first.
  - move=> x y.
    apply: (mulem_ge0 (fun x => f @^-1` [set y] `&` (g @^-1` [set (x - y)%R]))).
    move=> x0.
...
transitivity (
  ...
  rewrite -fsum_split; last 4 first.
    - move=> x; apply: fsume_ge0.
        move=> i _.
        apply: (mulem_ge0 (fun r => _ `&` (g @^-1` [set r]))).
        move=> i0.
        by rewrite (preimage_nnfun0 g)// ?setI0.
    - move=> x.
      apply: sume_ge0 => t _.
      apply: (mulem_ge0 (fun r => f @^-1` [set r] `&` _)).
      move=> i0.
  ...
  apply: eq_fsbigr => r _.
  rewrite -fsum_split; last 4 first.
    - move=> x; apply: (mulem_ge0 (fun r => _ `&` (g @^-1` [set r]))) => i0.
      by rewrite (preimage_nnfun0 g)// ?setI0.
    - move=> x; apply: (mulem_ge0 (fun r => f @^-1` [set r] `&` _)) => i0.
      by rewrite (preimage_nnfun0 f)// ?set0I.
...
- apply eq_fsbigr => x _.
  ...
  by rewrite nnsfun_cover setIT.
- rewrite exchange_fsum; last 2 first.
    - move=> x y.
      apply: (mulem_ge0 (fun y => f @^-1` [set x] `&` (g @^-1` [set y]))) => y0.
      by rewrite (preimage_nnfun0 g)// setI0.
    - apply: (@finite_set_preimage_setM2 _ (fun x1 x2 => m (f @^-1` [set x1] `&`
        (g @^-1` [set x2])))).
      have : finite_set (
        ((fun r => m (f @^-1` [set r])) @^-1` [set~ 0]) `*`
        ((fun r => m (g @^-1` [set r])) @^-1` [set~ 0])).
...
Definition scale_nnsfun (T : measurableType) (R : realType)
  ...
  mul_nnsfun (nnsfun_cst T (Nonneg.NngNum _ k0)) f.
...
Proof.
apply/funext => x /=; rewrite /patch /mindic/= /indic.
...
Let A n k := (if (k < n * 2 ^ n)%N then
  setT `&` [set x | f x \in EFin @` [set` I n k]] else set0).

Let B n := setT `&` [set x | n%:R%:E <= f x ]%E.
...
Local Lemma foo_B1 x n : f x = +oo%E -> x \in B n.
Proof.
by move=> fxoo; rewrite /B inE /=; split => //=; rewrite /= fxoo lee_pinfty.
Qed.
...
Local Lemma fpos_approx_neq0 x : (0%E < f x < +oo)%E ->
  \forall n \near \oo, approx n x != 0.
Proof.
...
Lemma cvg_approx x (f0 : forall x, (0 <= f x)%E) :
  (f x < +oo)%E -> (approx^~ x) --> fine (f x).
Proof.
move=> fxoo; have fxfin : f x \is a fin_num.
...
have /fpos_approx_neq0 [m _ Hm] : (0 < f x < +oo)%E.
...
Lemma le_approx k x (f0 : forall x, (0 <= f x)%E) :
  ((approx k x)%:E <= f x)%E.
Proof.
have [fixoo|] := ltP (f x) (+oo%E); last first.
  by rewrite lee_pinfty_eq => /eqP ->; rewrite lee_pinfty.
have nd_ag : {homo approx ^~ x : n m / (n <= m)%N >-> n <= m}.
  by move=> m n mn; exact/lefP/nd_approx.
have fi0 : forall x, (0 <= f x)%E by move=> ?; exact: f0.
have cvg_af := cvg_approx fi0 fixoo.
...
Lemma dvg_approx x : f x = +oo%E -> ~ cvg (approx^~ x : _ -> R^o).
Proof.
move=> fxoo; have approx_x n : approx n x = n%:R.
...
Lemma ecvg_approx (f0 : forall x, (0 <= f x)%E) x : EFin \o approx^~x --> f x.
Proof.
have := lee_pinfty (f x); rewrite le_eqVlt => /predU1P[|] fxoo.
have dvg_approx := dvg_approx fxoo.
...
rewrite -(@fineK _ (f x)); first exact: (cvg_comp (cvg_approx f0 fxoo)).
by rewrite fin_numElt fxoo andbT (lt_le_trans _ (f0 _)) // lte_ninfty.
Qed.
...
Lemma cvg_nnsfun_approx (f0 : forall x, (0 <= f x)%E) x :
  EFin \o nnsfun_approx^~x --> f x.
Proof.
...
Lemma approximation : (forall t, (0 <= f t)%E) ->
  exists g : {nnsfun T >-> R}^nat, nondecreasing_seq (g : (T -> R)^nat) /\
                        (forall x, EFin \o g^~x --> f x).
Proof.
...
have h10 x : 0 <= h1 x by exact: erestrict_ge0.
have mh1 : measurable_fun setT h1 by apply: measurable_restrict.
have [g [nd_g gh1]] := approximation mh1 h10.
...
have h10 x : 0 <= h1 x by exact: erestrict_ge0.
have h20 x : 0 <= h2 x by exact: erestrict_ge0.
have mh1 : measurable_fun setT h1 by apply: measurable_restrict.
have mh2 : measurable_fun setT h2 by apply: measurable_restrict.
have [g1 [nd_g1 gh1]] := approximation mh1 h10.
have [g2 [nd_g2 gh2]] := approximation mh2 h20.
...
rewrite (nd_ge0_integral_lim _ _ (fun x => lef_at x nd_g1)) //.
rewrite (nd_ge0_integral_lim _ _ (fun x => lef_at x nd_g2)) //.
rewrite ereal_limD //.
...
Lemma ge0_le_integral : (forall x, D x -> f1 x <= f2 x) ->
  \int_ D (f1 x) 'd mu[x] <= \int_ D (f2 x) 'd mu[x].
Proof.
rewrite !(integral_mkcond _ D); set h1 := f1 \|_ D; set h2 := f2 \|_ D.
move=> f12.
have h10 x : 0 <= h1 x by exact: erestrict_ge0.
have h20 x : 0 <= h2 x by exact: erestrict_ge0.
have mh1 : measurable_fun setT h1 by apply: measurable_restrict.
have mh2 : measurable_fun setT h2 by apply: measurable_restrict.
have h12 x : h1 x <= h2 x.
  by rewrite /h1 /h2 /patch; case: ifP => // /set_mem /f12 ->.
have [g1 [nd_g1 gh1]] := approximation mh1 h10.
...
Lemma approximation_sfun :
  exists g : {sfun T >-> R}^nat, (forall x, EFin \o g^~x --> f x).
Proof.
have fp0 : (forall x, 0 <= f^\+ x)%E by [].
have mfp : measurable_fun setT f^\+%E.
  by apply: emeasurable_fun_max => //; exact: measurable_fun_cst.
have fn0 : (forall x, 0 <= f^\- x)%E by [].
have mfn : measurable_fun setT f^\-%E.
  apply: emeasurable_fun_max => //; first exact: emeasurable_funN.
  exact: measurable_fun_cst.
have [fp_ [fp_nd fp_cvg]] := approximation mfp fp0.
have [fn_ [fn_nd fn_cvg]] := approximation  mfn fn0.
...
Local Definition g2' n : (T -> R)^nat := approx (g n).
Local Definition g2 n : {nnsfun T >-> R}^nat := nnsfun_approx (mg n).
...
(*   by rewrite !nnsfun_approxE; exact/lefP/(nd_approx (mgD i) ab). *)
...
- have h := dvg_approx fntoo.
  ...
  by rewrite lee_pinfty.
- have approx_g_g := cvg_approx (g0 n) fntoo.
  have <- : lim (EFin \o g2 n ^~ t) = g n t.
    have /cvg_lim <- // : EFin \o approx (g n)^~ t --> g n t.
      move/cvg_comp : approx_g_g; apply.
      by rewrite -(@fineK _ (g n t))// ge0_fin_numE// g0.
    rewrite (_ : _ \o _ = EFin \o approx (g n) ^~ t)// funeqE => m.
...
have : 0 <= \int_ D (f x) 'd mu[x] by exact: integral_ge0.
...
rewrite (_ : cst r = nnsfun_cst _ (Nonneg.NngNum _ r0)) //.
...
(* rewrite (integral_ge0M mu mD)// ?lte_mul_pinfty//. *)
...
Proof. by apply: integral_ge0 => // y _; rewrite lee_fin /indic. Qed.
...
(*- by move=> n x _; apply integral_ge0 => // y _ /=; rewrite lee_fin.*) admit.
...
(*- by move=> n y _; apply integral_ge0 => // x _ /=; rewrite lee_fin.*) admit.
...
(*  split => //; rewrite (le_lt_trans _ foo)//; apply: ge0_le_integral => //.
  ...
  - by move=> *; rewrite gee0_abs//; exact: integral_ge0.*) admit.
