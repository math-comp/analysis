...
Definition pi (f : t) := fun k : 'I_(size (rng f)) => f @^-1` [set (rng f)`_k].
End presfun.
Module Exports.
...
Arguments PreSFun.pi {T} {R} _.
Coercion PreSFun.f : presfun >-> Funclass.

Section pre_simple_function_partition.
Variables (T : measurableType) (R : realType) (f : presfun T R).
Let n := ssize f.
Let pi := PreSFun.pi f.

Lemma presfun_img_rng t : f t \in PreSFun.rng f.
Proof.
...
Lemma trivIset_presfun (A : set 'I_n) : trivIset A pi.
Proof.
apply/trivIsetP => /=; rewrite -/n => i j _ _ ij.
suff ij0 : [set (PreSFun.rng f)`_i] `&` [set (PreSFun.rng f)`_j] = set0.
  by rewrite /pi /PreSFun.pi -preimage_setI ij0 preimage_set0.
apply/eqP/negPn/negP => /set0P[r []]; rewrite /set1 /= => ->{r} /eqP.
by rewrite nth_uniq => //; [exact/negP | exact: PreSFun.uniq_rng].
Qed.

Lemma bigcup_presfun : \big[setU/set0]_(i < n) pi i = setT.
Proof.
rewrite predeqE => t; split => // _; rewrite -bigcup_set -preimage_bigcup.
have /(nthP 0)[i ni fit] := presfun_img_rng t.
by exists (Ordinal ni) => //=; rewrite mem_index_enum.
Qed.

End pre_simple_function_partition.

Section presfun_lemmas1.
Variables (T : measurableType) (R : realType) (f : presfun T R).
Let n := ssize f.

Local Lemma ltn_img_idx x : (index (f x) (PreSFun.rng f) < n)%N.
Proof. by rewrite index_mem presfun_img_rng. Qed.

Definition img_idx x := Ordinal (ltn_img_idx x).

Lemma nth_pidx x : (PreSFun.rng f)`_(img_idx x) = f x.
Proof. by rewrite nth_index //; exact: presfun_img_rng. Qed.

Lemma pi_pidx x : PreSFun.pi f (img_idx x) x.
Proof. by rewrite /PreSFun.pi /preimage mksetE nth_pidx. Qed.

Lemma pi_nth i x : PreSFun.pi f i x -> (PreSFun.rng f)`_i = f x.
Proof. by []. Qed.

Lemma presfun_ge0 : (forall t, 0 <= f t) ->
  forall k : 'I_(ssize f), 0 <= (PreSFun.rng f)`_k.
Proof.
case: f => /= f_ c uc fc f0 k.
...
Local Lemma presfun_eq_subset f g : f =1 g -> {subset PreSFun.rng f <= PreSFun.rng g}.
Proof.
move=> fg r rf.
have := PreSFun.full_rng g; rewrite predeqE => /(_ r)[+ _].
apply => /=.
have := PreSFun.full_rng f; rewrite predeqE => /(_ r)[_].
by move/(_ rf) => [t _ <-]; exists t.
Qed.

Lemma presfun_eq_perm f g : f =1 g -> perm_eq (PreSFun.rng f) (PreSFun.rng g).
Proof.
move=> fg; apply: uniq_perm; try exact: PreSFun.uniq_rng.
move=> r; apply/idP/idP; exact: presfun_eq_subset.
Qed.

Lemma presfun_size f g : f =1 g -> ssize f = ssize g.
Proof. by move/presfun_eq_perm => fg; rewrite /ssize; apply: perm_size. Qed.

End presfun_lemmas2.

Section pre_simple_function_constant.
Variables (T : measurableType) (point : T) (R : realType) (r : R).
Let rng := [:: r].
Let f : T -> R := cst r.

Local Lemma presfun_cst_uniq : uniq rng. Proof. by []. Qed.

Local Lemma presfun_cst_full_rng : f @` setT = [set x | x \in rng].
Proof.
...
Lemma ssize_presfun_cst : ssize presfun_cst = 1%N. Proof. by []. Qed.

Lemma rng_presfun_st : PreSFun.rng presfun_cst = rng. Proof. by []. Qed.

End pre_simple_function_constant.

Section pre_simple_function_scale.
Variables (T : measurableType) (point : T) (R : realType) (r : R).
Variable (f : presfun T R).
Let n := ssize f.
Let rng := if r == 0 then [:: 0] else [seq r * x | x <- PreSFun.rng f].
Let g := fun x => r * f x.

Local Lemma presfun_scale_uniq : uniq rng.
Proof.
...
End pre_simple_function_scale.

Section pre_simple_function_projection.
Variables (T : measurableType) (point : T) (R : realType) (f : presfun T R).
Variable (A : set T).

Let g (x : T) : R := f x * (x \in A)%:R.

Let s := [seq y <- PreSFun.rng f | f @^-1` [set y] `&` A != set0].

Let rng := if (0 \in s) || (A == setT) then s else
           if A == set0 then [:: 0] else 0 :: s.

Local Lemma presfun_proj_rng_subset a : a != 0 -> a \in rng -> a \in PreSFun.rng f.
Proof.
move=> a0; rewrite /rng.
...
Local Lemma presfun_proj_uniq : uniq rng.
Proof.
rewrite /rng; case: ifPn => [_|]; first exact/filter_uniq/PreSFun.uniq_rng.
rewrite negb_or => /andP[s0 _].
case: ifPn => //= /set0P[t' At']; rewrite (filter_uniq _ (PreSFun.uniq_rng f)).
by rewrite andbT.
Qed.

Local Lemma presfun_proj_full_rng : g @` setT = [set x | x \in rng].
Proof.
rewrite predeqE => x; split => [[t _]|] /=.
- rewrite /g; have [|tA] := boolP (t \in A).
  + rewrite in_setE => tA; rewrite mulr1 => <-; rewrite /rng.
    case: ifPn => [_|].
      by rewrite mem_filter presfun_img_rng andbT; apply/set0P; exists t.
    rewrite negb_or => /andP[s0 /setTP[t' At']].
    case: ifPn => [/eqP A0|_]; first by move: tA; rewrite A0.
    rewrite inE mem_filter presfun_img_rng andbT; apply/orP; right; apply/set0P.
    by exists t.
...
Definition presfun_proj :=
  PreSFun.mk presfun_proj_uniq presfun_proj_full_rng.

Lemma rng_presfun_proj : PreSFun.rng presfun_proj = rng.
Proof. by []. Qed.

End pre_simple_function_projection.

Section presimple_function_ind.
Variables (T : measurableType) (point : T) (R : realType) (r : R) (A : set T).

Definition presfun_ind := presfun_proj point (presfun_cst point r) A.

Lemma rng_presfun_ind :
  PreSFun.rng presfun_ind = PreSFun.rng (presfun_proj point (presfun_cst point r) A).
Proof. by rewrite [LHS]rng_presfun_proj. Qed.

End presimple_function_ind.

Section presimple_function_binary.
Variables (T : measurableType) (R : realType) (f g : presfun T R).
Let n := ssize f.
Let p := ssize g.
Let a := [seq z <- [seq (x, y) | x <- PreSFun.rng f, y <- PreSFun.rng g] |
  (f @^-1` [set z.1]) `&` (g @^-1` [set z.2]) != set0 ].
Let s (op : R -> R -> R) : seq R := undup [seq op z.1 z.2 | z <- a].

Local Lemma presfun_bin_full_rng (op : R -> R -> R) :
...
- rewrite /= => -[t _] <-; rewrite /s mem_undup.
  ...
  by apply/allpairsP; exists (f t, g t); split => //; apply: presfun_img_rng.
...
Definition presfun_bin_pidx (op : R -> R -> R) (k : 'I_(size (s op))) :=
  [set x : 'I_n * 'I_p | (op (PreSFun.rng f)`_x.1 (PreSFun.rng g)`_x.2 == (s op)`_k)
    && (PreSFun.pi f x.1 `&` PreSFun.pi g x.2 != set0)]%SET.

Local Lemma presfun_bin_bigcupIE (op : R -> R -> R)(k : 'I_(size (s op))) :
  \big[setU/set0]_(x : 'I_n * 'I_p | x \in presfun_bin_pidx k)
    (PreSFun.pi f x.1 `&` PreSFun.pi g x.2) =
  \big[setU/set0]_(z <- [seq (x, y) | x <- enum 'I_n, y <- enum 'I_p] |
                     z \in presfun_bin_pidx k)
    (PreSFun.pi f z.1 `&` PreSFun.pi g z.2).
Proof.
...
Local Lemma presfun_bin_preimageE (op : R -> R -> R) (k : 'I_(size (s op))) :
  (fun x => op (f x) (g x)) @^-1` [set (s op)`_k]
  = \big[setU/set0]_(x : 'I_n * 'I_p | x \in presfun_bin_pidx k)
      (PreSFun.pi f x.1 `&` PreSFun.pi g x.2).
Proof.
transitivity (\big[setU/set0]_(x : 'I_n * 'I_p |
     op (PreSFun.rng f)`_x.1 (PreSFun.rng g)`_x.2 == (s op)`_k)
    (PreSFun.pi f x.1 `&` PreSFun.pi g x.2)); last first.
  rewrite /presfun_bin_pidx big_mkcond [in RHS]big_mkcond.
...
Lemma presfun_bin_pi_cover0 (op : R -> R -> R) :
  \bigcup_(c < size (s op)) presfun_bin_pidx c =
  [set x : {: 'I_n * 'I_p}|PreSFun.pi f x.1 `&` PreSFun.pi g x.2 != set0]%SET.
Proof.
...
- move=> kl.
  suff [i kli] : exists i : 'I_(size (s op)),
      op (PreSFun.rng f)`_k (PreSFun.rng g)`_l = (s op)`_i.
    by exists i => //; rewrite inE /= kli eqxx.
  have : op (PreSFun.rng f)`_k (PreSFun.rng g)`_l \in
         [set of (fun x => op (f x) (g x))].
    rewrite inE /=; move/set0P : kl => [t [/pi_nth kt /pi_nth lt]].
    by exists t => //; rewrite -kt -lt.
  by rewrite presfun_bin_full_rng inE /= => /(nthP 0)[x xa H]; exists (Ordinal xa).
Qed.

End presimple_function_binary.

Section presimple_function_addition_lemmas.
Variables (T : measurableType) (R : realType) (f g : presfun T R).
Let n := ssize f.
Let p := ssize g.

Lemma pi_presfun_addE (c : 'I_(ssize (presfun_add f g))) : PreSFun.pi (presfun_add f g) c
  = \big[setU/set0]_(x : 'I_n * 'I_p | x \in presfun_bin_pidx c)
      (PreSFun.pi f x.1 `&` PreSFun.pi g x.2).
Proof.
transitivity ((presfun_add f g) @^-1` [set (PreSFun.rng (presfun_add f g))`_c]) => //.
by rewrite presfun_bin_preimageE.
Qed.

Lemma trivIset_presfunI (D : set T) :
  trivIset setT (fun i : 'I_n * 'I_p => PreSFun.pi f i.1 `&` PreSFun.pi g i.2 `&` D).
Proof.
apply/trivIsetP => /= -[a b] [c d] _ _ /=.
rewrite xpair_eqE negb_and => /orP[ac|bd].
  rewrite setIACA (_ : PreSFun.pi f a `&` _ `&` _ = set0) ?set0I//.
  rewrite setIACA (_ : PreSFun.pi f a `&` _ = set0) ?set0I//.
  by move/trivIsetP: (@trivIset_presfun _ _ f setT) => /(_ _ _ Logic.I Logic.I ac).
rewrite setIACA (_ : PreSFun.pi f a `&` _ `&` _ = set0) ?set0I//.
rewrite setIACA (_ : PreSFun.pi g b `&` _ = set0) ?setI0//.
by move/trivIsetP : (@trivIset_presfun _ _ g setT) => /(_ _ _ Logic.I Logic.I bd).
Qed.

End presimple_function_addition_lemmas.
...
Section nnpresfun_lemmas.
Variables (T : measurableType) (R : realType).

Lemma NNPreSFun_ge0 (f : nnpresfun T R) (k : 'I_(ssize f)) : 0 <= (PreSFun.rng f)`_k.
Proof. exact: presfun_ge0. Qed.

Lemma SFuncdom_ge0 (f : nnpresfun T R) (r : R) :
  (r \in PreSFun.rng f) -> (0 <= r%:E)%E.
Proof.
by move=> /(nthP 0)[i fi <-]; rewrite lee_fin (NNPreSFun_ge0 (Ordinal fi)).
Qed.

End nnpresfun_lemmas.

(*Hint Extern 0 (0%E <= ((SFun.cdom _)`__)%:E)%E => solve [apply: NNSFun_ge0] : core.*)

Section non_negative_presimple_function_ind.
Variables (T : measurableType) (point : T) (R : realType) (r : {nonneg R}).
Variables (A : set T).

Local Lemma nnpresfun_point_ge0 t : 0 <= presfun_ind point r%:nngnum A t.
Proof. by rewrite mulr_ge0. Qed.

Definition nnpresfun_ind := NNPreSFun.mk nnpresfun_point_ge0.

End non_negative_presimple_function_ind.

Section non_negative_presimple_function_constant.
Variables (T : measurableType) (point : T) (R : realType) (r : {nonneg R}).

Local Lemma nnpresfun_cst_ge0 x : 0 <= presfun_cst point r%:nngnum x.
Proof. by []. Qed.

Definition nnpresfun_cst := NNPreSFun.mk nnpresfun_cst_ge0.

End non_negative_presimple_function_constant.

Section nnpresfun_scale.
Variables (T : measurableType) (point : T) (R : realType) (r : R).
Variables (f : nnpresfun T R) (k : R) (k0 : 0 <= k).

Local Lemma nnpresfun_scale_ge0 x : 0 <= presfun_scale point k f x.
Proof. by rewrite mulr_ge0. Qed.
...
Section non_negative_presimple_function_add.
Variables (T : measurableType) (R : realType) (f g : nnpresfun T R).

Local Lemma nnpresfun_add_ge0 x : 0 <= presfun_add f g x.
Proof. by rewrite addr_ge0. Qed.

Definition nnpresfun_add := NNPreSFun.mk nnpresfun_add_ge0.

End non_negative_presimple_function_add.

Section nnpresfun_max.
Variables (T : measurableType) (R : realType) (f g : nnpresfun T R).

Local Lemma nnpresfun_max_ge0 x : 0 <= presfun_max f g x.
...
Lemma nnpresfun_bigmaxE n t : nnpresfun_bigmax n t = \big[maxr/0]_(i < n) (f i t).
...
Record t := mk {
  f :> presfun T R ;
  mpi : forall k, measurable (PreSFun.pi f k) }.
End sfun.
Module Exports.
...
have [[k fkr]|/forallNP fr] :=
    pselect (exists k : 'I_(ssize f), (PreSFun.rng f)`_k = r).
  by have := SFun.mpi k; rewrite /PreSFun.pi fkr.
rewrite (_ : _ @^-1` _ = set0); first exact: measurable0.
rewrite predeqE => t; split => //= ftr.
have /(nthP 0)[i fi fift] := presfun_img_rng f t.
...
rewrite /pi (_ : _ @^-1` _ = PreSFun.pi f k'); first exact: SFun.mpi.
rewrite predeqE => t; split.
  rewrite /rng /preimage mksetE {1}(negbTE r0) (nth_map 0) //.
  by apply: mulrI; rewrite unitfE.
rewrite /PreSFun.pi /preimage mksetE /= /g => ->.
transitivity ([seq r * x | x <- PreSFun.rng f]`_k).
...
- rewrite (_ : _ @^-1` _ = (f @^-1` [set a] `&` A) `|` ~` A); last first.
  ...
  apply: measurableI => //.
  have [fa|fa] := boolP (a \in PreSFun.rng f).
    move: fa => /(nthP 0)[i fi fia].
    rewrite -fia (_ : _ @^-1` _ = PreSFun.pi f (Ordinal fi)) //.
    exact: SFun.mpi.
  rewrite (_ : _ @^-1` _ = set0); first exact: measurable0.
  rewrite predeqE => t; split => // fta; move/negP : fa; apply.
  by rewrite -fta; exact: presfun_img_rng.
rewrite (_ : _ @^-1` _ = (f @^-1` [set a] `&` A)); last first.
  ...
  by move: At; rewrite -in_setE => ->; rewrite mulr1.
apply: measurableI => //; have /(nthP 0)[i fi fia] : a \in PreSFun.rng f.
  by apply: (@presfun_proj_rng_subset _ _ f A a a0) => //; apply/(nthP 0); exists k.
by rewrite -fia (_ : _ @^-1` _ = PreSFun.pi f (Ordinal fi)) //; exact: SFun.mpi.
Qed.

Definition sfun_proj :=
  SFun.mk (presfun_proj point f A) sfun_proj_mpi.
...
Proof.
rewrite presfun_bin_preimageE presfun_bin_bigcupIE.
by apply: bigsetU_measurable => -[i j] aij; apply: measurableI; exact: SFun.mpi.
Qed.

Definition sfun_add := SFun.mk (presfun_add f g) (@sfun_bin_mpi (fun x y => x + y)).

Definition sfun_max := SFun.mk (presfun_max f g) (@sfun_bin_mpi maxr).
...
Lemma measure_sfun_bin_pi (mu : {measure set T -> \bar R}) (D : set T)
  ...
  mu (PreSFun.pi (sfun_add f g) c `&` D) =
  (\sum_(kl in presfun_bin_pidx c) mu (PreSFun.pi f kl.1 `&` PreSFun.pi g kl.2 `&` D))%E.
Proof.
rewrite pi_presfun_addE big_distrl /=.
rewrite (additive_set mu _ _ (@trivIset_presfunI _ _ _ _ D)) //=.
...
move=> _ [/= _ [r ->] <-]; rewrite [X in measurable X](_ : _ =
  \bigcup_(i in [set j | (PreSFun.rng f)`_j > r]) (f @^-1` [set (PreSFun.rng f)`_i] `&` D)); last first.
...
Lemma sintegralE : sintegral =
  \sum_(x <- PreSFun.rng f) x%:E * mu (f @^-1` [set x] `&` D).
Proof.
rewrite big_tnth; apply: eq_bigr => i _; congr (_%:E * mu _).
  by apply: set_nth_default; rewrite /= ltn_ord.
rewrite /A /PreSFun.pi; congr (_ @^-1` _ `&` _); rewrite predeqE => r; split;
...
Lemma NNSFun_ge0 (T : measurableType) (R : realType) (f : nnsfun T R) (t : 'I_(ssize f)) :
  (0 <= (PreSFun.rng f)`_t).
Proof.
case: f t => // f h /= t.
by apply: presfun_ge0.
Qed.

Lemma NNSFuncdom_ge0 (T : measurableType) (R : realType) (f : nnsfun T R) (r : R) :
  (r \in PreSFun.rng f) -> (0 <= r%:E)%E.
Proof.
...
transitivity (\sum_(i < n) \sum_(l < p)
  ((PreSFun.rng f)`_i + (PreSFun.rng g)`_l)%:E * m (PreSFun.pi f i `&` PreSFun.pi g l `&` D)).
  rewrite /sintegral; under eq_bigr do rewrite (measure_sfun_bin_pi _ mD).
  transitivity (\sum_(i : 'I_(ssize (sfun_add f g)))
    (\sum_(x in presfun_bin_pidx i)
      ((PreSFun.rng f)`_x.1 + (PreSFun.rng g)`_x.2)%:E *
      m (PreSFun.pi f x.1 `&` PreSFun.pi g x.2 `&` D))).
  ...
  transitivity (\sum_(x in [set x|PreSFun.pi f x.1 `&` PreSFun.pi g x.2 != set0]%SET)
      ((PreSFun.rng f)`_x.1 + (PreSFun.rng g)`_x.2)%:E *
      m (PreSFun.pi f x.1 `&` PreSFun.pi g x.2 `&` D)); last first.
  ...
  rewrite -(presfun_bin_pi_cover0 _ _ (fun x y => x + y)%R).
...
have ggf k : m (PreSFun.pi g k `&` D) =
    \sum_(i < n) m (PreSFun.pi g k `&` PreSFun.pi f i `&` D).
  rewrite -[in LHS](setIT (PreSFun.pi g k `&` D)) -(bigcup_presfun f) big_distrr /=.
  under eq_bigr do rewrite setIAC.
  rewrite additive_ord //; last first.
    under [in X in trivIset setT X]eq_fun do rewrite setIAC.
    exact/trivIset_setI/trivIset_presfun.
...
transitivity (\sum_(i < p) (\sum_(j < n)
     ((PreSFun.rng g)`_i)%:E * m (PreSFun.pi g i `&` PreSFun.pi f j `&` D)) +
   \sum_(k < n) ((PreSFun.rng f)`_k)%:E * m (PreSFun.pi f k `&` D)); last first.
...
have ffg k : m (PreSFun.pi f k `&` D) =
             \sum_(l < p) m (PreSFun.pi f k `&` PreSFun.pi g l `&` D).
  rewrite -[in LHS](setIT (PreSFun.pi f k `&` D)) -(bigcup_presfun g) big_distrr /=.
  under eq_bigr do rewrite setIAC.
  rewrite additive_ord //; last first.
    under [in X in trivIset setT X]eq_fun do rewrite setIAC.
    exact/trivIset_setI/trivIset_presfun.
...
transitivity (\sum_(i < p) \sum_(j < n)
    ((PreSFun.rng g)`_i)%:E * m (PreSFun.pi g i `&` PreSFun.pi f j `&` D) +
    \sum_(i < n) \sum_(l < p)
    ((PreSFun.rng f)`_i)%:E * m (PreSFun.pi f i `&` PreSFun.pi g l `&` D)); last first.
  congr adde; apply: eq_bigr => i _.
  rewrite ge0_sume_distrr // => j _; rewrite measure_ge0 //.
  by apply: measurableI => //; apply: measurableI; exact: SFun.mpi.
...
rewrite -[in RHS](setIC (PreSFun.pi f i)).
...
Definition preimgI_rng (S : set T) :=
  let s := [seq x <- PreSFun.rng g | (g @^-1` [set x]) `&` S != set0] in
  if (0 \in s) || (S == setT) then s else rcons s 0.

Program Definition preimgI_presfun (S : set T) : presfun T R :=
  @PreSFun.mk _ _ (fun x => g x * (x \in S)%:R) (preimgI_rng S) _ _.
Next Obligation.
move=> S; rewrite /preimgI_rng; set s := seq.filter _ _ => /=.
have [_|] := ifPn; first by rewrite filter_uniq // PreSFun.uniq_rng.
rewrite negb_or => /andP[s0 _].
by rewrite rcons_uniq s0 /= filter_uniq // PreSFun.uniq_rng.
Qed.
Next Obligation.
move=> S; rewrite /preimgI_rng; set s := seq.filter _ _ => /=.
rewrite predeqE => x; split => [[t _ <-{x}]|] /=.
  have [tS|tS] /= := boolP (t \in S).
    rewrite mulr1; have [_|_] := ifPn.
      rewrite mem_filter presfun_img_rng andbT; apply/set0P.
...
Program Definition preimgI_sfun (S : set T) (mS : measurable S) : sfun T R :=
  @SFun.mk _ _ (preimgI_presfun S) _.
Next Obligation.
move=> S mS.
rewrite /preimgI_presfun.
rewrite /=.
rewrite /preimgI_rng.
...
have [/orP[s0 k|/eqP ST k]|] := ifPn.
  - have [k' kk'] : exists k' : 'I_(ssize g), s`_k = (PreSFun.rng g)`_k'.
      have : s`_k \in PreSFun.rng g.
    ...
    apply: measurableU.
      apply: measurableI => //.
      by have := @SFun.mpi _ _ g k'; rewrite /PreSFun.pi /= -kk'.
...
rewrite /preimage /= gSxE; have [ks|ks] := eqVneq (nat_of_ord k) (size s).
  rewrite nth_rcons ks ltnn eqxx; apply: measurableU.
    ...
    by rewrite -g0t presfun_img_rng.
  apply: measurableI; first exact: measurableC.
  by rewrite (_ : (fun _ => _) = setT) ?predeqE //; exact: measurableT.
have [k' kk'] : exists k' : 'I_(ssize g), (rcons s 0)`_k = (PreSFun.rng g)`_k'.
  have @k' : 'I_(size s).
    apply: (@Ordinal _ k).
...
rewrite [X in _ X](_ : _ = \big[setU/set0]_(y <- PreSFun.rng f)
    \big[setU/set0]_(x <- PreSFun.rng (g n) | c * y <= x)
      ((f @^-1` [set y]) `&` (g n @^-1` [set x]))); last first.
  rewrite predeqE => t; split.
    rewrite /= => fgcn; rewrite -bigcup_set.
    exists (f t); first exact: presfun_img_rng.
    rewrite -bigcup_set_cond; exists (g n t) => //.
    by apply/andP; split => //; exact: presfun_img_rng.
...
rewrite [X in X --> _](_ : _ =
    (fun n => \sum_(k < ssize f) ((PreSFun.rng f)`_k)%:E *
    mu (f @^-1` [set (PreSFun.rng f)`_k] `&` fleg c n `&` D))); last first.
  rewrite funeqE => n; rewrite sintegralE.
  transitivity (\sum_(x <- PreSFun.rng f) x%:E * mu (g1 n @^-1` [set x] `&` D)).
    rewrite (_ : PreSFun.rng (g1 n) = preimgI_rng f (fleg c n))// /preimgI_rng/=.
  ...
  have [fi0|fi0] := eqVneq ((PreSFun.rng f)`_i) 0%R.
...
rewrite [X in X --> _](_ : _ = fun n => \sum_(x <- PreSFun.rng f) x%:E *
...
(* NB: improve? *)
have [->|/set0P[t [fit Dt]]] := eqVneq (PreSFun.pi f i `&` D) set0.
...
Proof.
rewrite sintegralE rng_presfun_ind rng_presfun_proj rng_presfun_st /=.
...
Lemma sintegralE2 : sintegral mu D f =
  \sum_(k < n) (a`_k)%:E * sintegral mu D (presfun_ind point 1%R (PreSFun.pi f k)).
Proof.
...
Lemma measurable_xsection A x : measurable A ->
  measurable (xsection A x).
Proof.
move=> mA.
pose f : _ -> \bar R := EFin \o sfun_ind (point1, point2) 1 mA.
...
have _ : (fun y => (y \in xsection A x)%:R%:E) = f \o (fun y => (x, y)).
  by rewrite funeqE => y /=; rewrite /xsection /f /= /cst /= mul1r mem_set_pair.
have -> : xsection A x = (fun y => f (x, y)) @^-1` [set 1%E].
  rewrite predeqE => y; split; rewrite /xsection /preimage /f /= mul1r.
    by move=> ->.
  by case: (_ \in _) => //= -[] /eqP; rewrite eq_sym oner_eq0.
rewrite -(setIT (_ @^-1` _)).
by apply: partial_measurable_fun => //; exact: emeasurable_set1.
Qed.

End measurable_section.

Section ndseq_closed_B.
Variables (T1 T2 : measurableType) (point2 : T2) (R : realType).
Variables (m2 : {measure set T2 -> \bar R}).

Let phi (A : set (T1 * T2)) := fun x => m2 (xsection A x).
Let B := [set A : set (T1 * T2) | measurable A /\ measurable_fun setT (phi A)].

Lemma section_ndseq_closed_B : ndseq_closed B.
Proof.
...
apply: (@emeasurable_fun_cvg _ _ _ (phi \o F)) => //; first exact: measurableT.
- by move=> i; have [] := BF i.
...
Hypothesis GT : G `<=` @measurable T.

Definition measure_restr' : set (g_measurableType G) -> \bar R := mu.

Local Lemma measure_restr'0 : measure_restr' set0 = 0%E.
Proof. by rewrite /measure_restr' measure0. Qed.

Local Lemma measure_restr'_ge0 (A : set (g_measurableType G)) :
  measurable A -> (0 <= measure_restr' A)%E.
Proof.
move=> mA; rewrite /measure_restr'; apply: measure_ge0.
apply: (@g_salgebra_smallest _ setT _ _ GT) => //.
exact: are_measurable_sets_measurable.
Qed.

Local Lemma measure_restr'_sigma_additive : semi_sigma_additive measure_restr'.
Proof.
move=> F mF tF mU; apply: (@measure_sigma_additive _ _ mu F) => // i.
have := mF i; apply: (@g_salgebra_smallest _ setT _ _ GT) => //.
exact: are_measurable_sets_measurable.
Qed.

Definition measure_restr : {measure set (g_measurableType G) -> \bar R} :=
  Measure.Pack _ (Measure.Axioms
    measure_restr'0 measure_restr'_ge0 measure_restr'_sigma_additive).
...
Lemma measurable_prod_subset (m2_bounded : exists M, forall X, measurable X -> (m2 X < M%:E)%E) :
...
have CB : C `<=` B.
  ...
  have -> : phi (X1 `*` X2) = (fun x => m2 X2 * (sfun_ind point1 1 mX1 x)%:E)%E.
    rewrite funeqE => x; rewrite /phi /= /cst /= mul1r.
    have [xX1|xX1] := boolP (x \in X1); first by rewrite mule1 in_xsectionM.
    by rewrite /= mule0 notin_xsectionM// /measure_restr' set0I measure0.
...
have monoB : is_monotone_class B.
  ...
  - move=> X Y XY [mX mphiX] [mY mphiY]; split; first exact: measurableD.
    have -> : phi (X `\` Y) = (fun x => phi X x - phi Y x)%E.
      rewrite funeqE => x; rewrite /phi xsectionD measureD; last 3 first.
...
Lemma measurable_fun_section_sigma_finite (m2_sigma_finite : sigma_finite setT m2')
  (A : set (T1 * T2)) :
...
suff : @measurable (prod_measurableType T1 T2) `<=` B.
  move=> + A mA.
  by move/(_ A); rewrite -in_setE => /(_ mA)[].
move/sigma_finiteP : m2_sigma_finite => [S2_ S2_T [S2_nd S2_oo]].
move=> X mX.
have -> : X = \bigcup_n (X `&` (setT `*` S2_ n)).
  by rewrite -setI_bigcupr -setM_bigcupr -S2_T setMTT setIT.
apply: (section_ndseq_closed_B point2).
  move=> m n mn; apply/subsetPset; apply: setIS; apply: setMS.
  by apply/subsetPset; apply: S2_nd.
move=> n.
rewrite -/B.
pose I2_n : set (set T2) := strace (@measurable T2) (S2_ n).
have I2_nT : I2_n `<=` @measurable T2.
  (*NB:lemma?*)
  by move=> _ [A' mA' <-]; apply: measurableI => //; have [] := S2_oo n.
pose m2 : {measure set _ -> \bar R} := measure_restr m2' (S2_oo n).1.
have m2_bounded : (exists M, forall X, measurable X -> (m2 X < M%:E)%E).
  exists (fine (m2 (S2_ n)) + 1) => Y mY.
  rewrite [in X in (_ < X)%E]addEFin (le_lt_trans _ (lte_addl _ _))//; last by rewrite lte_fin.
  rewrite fineK; last first.
    rewrite ge0_fin_numE.
      by rewrite /m2 /measure_restr /measure_restr' /= setIid//; have [] := S2_oo n.
    by rewrite measure_ge0//; have [] := S2_oo n.
  rewrite /m2 /measure_restr /= /measure_restr' /= setIid; apply: le_measure => //.
  - by rewrite in_setE /=; apply: measurableI => //; have [] := S2_oo n.
  - by rewrite in_setE /=; have [] := S2_oo n.
  - by apply: subIset; right.
pose phi' (A : set (T1 * T2(*mI2_n*))) := m2 \o xsection A.
pose B' := [set A : set (T1 * T2) | measurable A /\ measurable_fun setT (phi' A)].
have subset_B' : @measurable (@prod_measurableType T1 T2(*mI2_n*)) `<=` B'.
  exact: measurable_prod_subset.
split=> [|Y mY].
...
rewrite /B' /= => -[_] /(_ Y mY).
rewrite /phi'.
have ->// : (m2 \o xsection X) = (fun x => m2' [set y | (x, y) \in X `&` setT `*` S2_ n]).
rewrite funeqE => x /=; congr (m2' _).
rewrite predeqE => y; split => [[]|].
  by rewrite /xsection /= in_setE => Xxy S2ny; rewrite in_setE.
by rewrite /xsection /= !in_setE => -[] Xxy /= [_].
Qed.

(*pose Itilda := [set A : set (T1 * T2) | A \in measurable /\
                @measurable _(*I1oI2_n*) (A `&` setT `*` S2_ n)].
have H1 : are_measurable_sets (setT `*` S2_ n) Itilda.
  split.
  - by rewrite /Itilda /= in_setE set0I; split; exact: measurable0.
  - move=> A []; rewrite in_setE => mA H; split.
      rewrite in_setE; apply: measurableD => //.
      apply: measurableM; first exact: measurableT.
      by have [] := S2_oo n.
    rewrite setIidl; last by apply: subIset; left.
    have rew_lem (A0 : set (T1 * T2)) :
      setT `*` S2_ n `\` A0 = (~` (A0 `&` (setT `*` S2_ n))) `&` (setT `*` S2_ n).
      rewrite predeqE => -[x y]; split.
        by move=> -[[_ /= S2ny] Axy]; split => // -[] /Axy.
      by move=> [/= AxyS2n [_ S2ny]]; split => //; apply: contra_not AxyS2n.
    rewrite rew_lem.
    apply: measurableI; first exact: measurableC.
    apply: measurableM; first exact: measurableT.
(*     apply/g_salgebra_self/stracexx.*)
     by have [] := S2_oo n.
  - move=> F mF; rewrite /Itilda /= in_setE; split.
      apply: measurable_bigcup => i.
      by have [] := mF i; rewrite in_setE.
    rewrite setI_bigcupl; apply: measurable_bigcup => i.
    by have [] := mF i.
have {H1}H2 : s<| (setT `*` S2_ n), [set A `*` B | A in @measurable T1 &
                                               B in @measurable T2]|> `<=` Itilda.
  apply: g_salgebra_smallest => // _ /= [X1 mX1] [X2 mX2] <-.
  rewrite /Itilda /=; split.
    by rewrite in_setE; apply: measurableM.
  rewrite -setIM setIT measurable_prod_measurableType.
  apply: g_salgebra_self => /=.
  exists X1 => //; exists (X2 `&` S2_ n) => //.
  apply: measurableI => //.
  by have [] := S2_oo n.*)
...
Let m A := integral m1 setT (phi A).

Local Lemma m0 : m set0 = 0%E.
Proof.
rewrite /m (_ : phi _ = cst 0)%E ?integral0// funeqE => n.
by rewrite /phi /= xsection0 measure0.
Qed.

Local Lemma m_ge0 A : measurable A -> (0 <= m A)%E.
Proof.
move=> mA; apply: ge0_integral => // x _.
by apply: measure_ge0 => //; exact: measurable_xsection.
Qed.
...
have -> : m (\bigcup_n F n) = \sum_(n <oo) (m (F n)).
  transitivity (integral m1 setT (fun x => \sum_(n <oo) m2 (xsection (F n) x))).
    rewrite /m; congr (integral _ _ _); rewrite funeqE => x; rewrite /phi.
    suff : (fun n => (\sum_(i < n) m2 (xsection (F i) x))%E) --> m2 (xsection (\bigcup_n F n) x).
      by move=> ?; rewrite -lim_mkord; exact/esym/cvg_lim.
    rewrite xsection_bigcup; apply: (@measure_sigma_additive _ _ _ (fun n => xsection (F n) x)).
    by move=> i; apply: measurable_xsection.
    exact: trivIset_xsection.
  rewrite integral_sum//; first exact: measurableT.
  - by move=> n; apply: measurable_fun_section_sigma_finite => //; rewrite in_setE.
  - by move=> n x _; apply: measure_ge0; apply: measurable_xsection.
have /cvg_ex[l cl] : cvg (fun n => (\sum_(i < n) m (F i))%E).
  rewrite (_ : (fun n => _) = (fun n => (\sum_(0 <= i < n) m (F i))%E)).
    apply: is_cvg_ereal_nneg_natsum (*TODO: fix*) => //.
      exact: predT.
    move=> n _; apply: ge0_integral => // x _; apply: measure_ge0.
    exact: measurable_xsection.
  by rewrite funeqE => n; rewrite big_mkord.
by move: (cl) => /cvg_lim; rewrite -lim_mkord => ->.
Qed.

Definition product_measure : {measure set (T1 * T2) -> \bar R} :=
...
Lemma product_measureE (A1 : set T1) (A2 : set T2) :
  ...
  product_measure (A1 `*` A2) = (m1 A1 * m2 A2)%E.
Proof.
move=> mA1 mA2; rewrite /product_measure /= /m.
rewrite (_ : phi _ = (fun x => m2 A2 * (nnsfun_ind point1 1%:nng mA1 x)%:E)%E); last first.
  rewrite funeqE => x; rewrite /phi /=.
  have [xA1|xA1] := boolP (x \in A1).
    by rewrite in_xsectionM//= mulr1 mule1.
  by rewrite mulr0 mule0 notin_xsectionM.
rewrite ge0_integralM //; last 4 first.
  - exact: measurableT.
  - by apply: measurable_fun_comp_EFin; apply: measurable_sfun; exact: measurableT.
  - by move=> x _; rewrite lee_fin; exact: nnsfun_point_ge0.
  - exact: measure_ge0.
...
apply: eq_nnintegral => y _ /=; rewrite /cst !mul1r; congr EFin.
...
Proof.
rewrite FE//.
by apply: measurable_fun_section_sigma_finite => //; rewrite in_setE.
Qed.
...
Let m : {measure set (T1 * T2) -> \bar R} := product_measure point1 point2 m1 sm2.

(* par definition de la mesure produit *)
Let mE (A : set (T1 * T2)) (mA : measurable A) : m A = integral m1 setT (F A).
Proof.
rewrite /m /product_measure /= /F /fubini_tonelli_F.
congr (integral m1 setT _).
rewrite funeqE => x.
by rewrite -FE /F /fubini_tonelli_F.
Qed.

Let m' (B : set (T1 * T2)) := integral m2 setT (G B).
(* m' est aussi une mesure positive *)

Section measure_m'.

Local Lemma m'0 : m' set0 = 0%E.
Proof.
rewrite /m'.
rewrite /G /fubini_tonelli_G /f /=.
rewrite (_ : (fun y => _) = cst 0%E) ?integral0// funeqE => y.
rewrite (_ : (fun y => _) = cst 0%E) ?integral0// funeqE => x.
rewrite /cst /= mul1r.
congr (_%:E). (* ok *)
Admitted.

Local Lemma m'_ge0 (A : set _) : measurable A -> (0 <= m' A)%E.
Proof. by move=> mA; apply: ge0_integral. Qed.

Local Lemma m'_sigma_additive : semi_sigma_additive m'.
Proof.
move=> H mH tH mUH.
rewrite /m'.
Admitted.

Definition measure_m' : {measure set _ -> \bar R} :=
  Measure.Pack _ (Measure.Axioms
    m'0 m'_ge0 m'_sigma_additive).
...
rewrite ge0_integralM//; last 4 first.
  - exact: measurableT.
  - apply: measurable_fun_comp_EFin.
    apply: measurable_sfun.
    exact: measurableT.
  - move=> y _.
    by rewrite lee_fin.
  - exact: measure_ge0.
congr (_ * _)%E.
rewrite integral_ind; last exact: measurableT.
by rewrite setTI mul1e.
Qed.

Lemma m'm (A1 : set T1) (A2 : set T2) (mA1 : measurable A1) (mA2 : measurable A2) :
  measure_m' (A1 `*` A2) = m (A1 `*` A2).
Proof.
apply: (@measure_unique R (prod_measurableType T1 T2) _ erefl).
Abort.
...
Fail Lemma fubini_tonelli_sfun1_G :
  sintegral (product_measure point1 point2 m1 sm2) setT f =
  integral m2 setT G.

End fubini_tonelli_sfun1.

Section fubini_tonelli_sfun.
Variable f : nnsfun (prod_measurableType T1 T2) R.
Hypotheses (mf : measurable_fun setT f) (f0 : (forall x, 0 <= (f x)%:E)%E).
...
Lemma fubini_tonelli_sfun :
  integral (product_measure point1 point2 m1 sm2) setT (EFin \o f) =
  integral m1 setT F.
Proof.
rewrite integral_nnsfun//; last exact: measurableT.
rewrite (sintegralE2 (point1, point2)).
under eq_bigr.
  move=> i _.
  rewrite fubini_tonelli_sfun1_F; last first.
    exact: SFun.mpi.
  over.

Abort.

End fubini_tonelli_sfun.
