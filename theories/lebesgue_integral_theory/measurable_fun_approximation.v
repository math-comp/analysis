...
(*                                                                            *)
(* `dyadic_itv n k`                                                           *)
(*     ~ the interval `` `[(k%:R * 2 ^- n), (k.+1%:R * 2 ^- n)[ ``            *)
(*                                                                            *)
(* `approx D f`                                                               *)
(*     ~ nondecreasing sequence of functions that approximates `f` over       *)
(*       `D` using dyadic intervals It uses the sets `dyadic_approx` and      *)
(*       `integer_approx`.                                                    *)
(*                                                                            *)
(* `nnsfun_approx D f`                                                        *)
(*     ~ same as `approx D f` but as a `nnsfun` approximates `f` over `D`     *)
(*       using dyadic intervals                                               *)
(*                                                                            *)
(******************************************************************************)
