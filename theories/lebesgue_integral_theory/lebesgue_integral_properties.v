...
- move=> /cvgrPdist_lt/(_ _ ltr01) -[n _].
  ...
  rewrite -leNgt approx_x distrC addSnnS natrD ger0_norm//; last first.
    by rewrite subr_ge0 ltW// (lt_le_trans (truncnS_gt _))// lerDl.
  by rewrite lerBrDl lerD ?ler1n// ltW// truncnS_gt.
- move=> /cvgrPdist_lt/(_ _ ltr01)[n _].
  move=> /(_ ((trunc l).+2 + n))/(_ (leq_addl _ _)); apply/negP.
  rewrite approx_x -leNgt distrC natrD ger0_norm; last first.
    by rewrite subr_ge0 -natr1 -addrA ler_wpDr// ltW// truncnS_gt.
  by rewrite lerBrDl -natr1 -addrA nat1r lerD ?ler1n// ltW// truncnS_gt.
Qed.
