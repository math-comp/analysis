...
From Coq Require Import ssreflect ssrfun ssrbool ssrsearch.
...
(*                     S, i.e., it is surjective and S = e @` setT            *)
(*                                                                            *)
(******************************************************************************)

(* TODO: PR *)
Lemma setM0 (T U : Type) (A : set T) : A `*` @set0 U = set0.
Proof. by rewrite predeqE => -[t u]; split => // -[]. Qed.

Lemma set0M (T U : Type) (B : set U) : @set0 T `*` B = set0.
Proof. by rewrite predeqE => -[t u]; split => // -[]. Qed.

Lemma subset_set1 T (A : set T) t : A `<=` [set t] -> A = set0 \/ A = [set t].
Proof.
move=> At; have [[t0 At0]|/set0P/negP] := pselect (A !=set0); last first.
  by rewrite negbK => /eqP ->; left.
by right; apply: eqEsubset => // ? ->; rewrite -(At _ At0).
Qed.

Lemma image_set0_set0 T U (A : set T) (f : T -> U) : f @` A = set0 -> A = set0.
Proof.
move=> fA0; rewrite predeqE => t; split => // At.
have [u] : exists u, (f @` A) u by exists (f t); exists t.
by rewrite fA0.
Qed.
...
Definition enumeration T (S : set T) (e : nat -> T) :=
  surjective setT S e /\ S = e @` setT.

Lemma enumeration_id : enumeration setT id.
Proof. by split; [exact: surjective_id|rewrite image_id]. Qed.
...
have {}As : A `<=` (fun i => i \in s).
  move=> t At; case: Ae => es AeT.
  have [i [_ ->]] := es _ At.
  by have /negP/negPn := Ps i.
exact/infiniteA/(proj1 (set_finite_subset As _))/set_finite_seq.
Qed.
...
- move=> u [/= i _ <-{u}].
  case: i => [|i]; first by rewrite /enum_wo_rep /= (proj2 Ae); exists 0%N.
  by rewrite enum_wo_repE /= [X in X (e _)](proj2 Ae); exists (smallest_of_e i).
...
Proof.
split; first exact: surjective_enum_wo_rep.
apply/eqEsubset; last exact: (sub_of_bij (set_bijective_enum_wo_rep nfinA Ae)).
...
- move: n2 => [/eqP|n2]; rewrite ?(expn0,mul1n).
  ...
  by move/(congr1 (fun x => 2 %| x)); rewrite ?(Euclid_dvdX,Euclid_dvdM).
- move: m2 => [/eqP|m2]; rewrite ?(expn0,muln1).
    by rewrite eqn_exp2l // => /eqP ->.
  by move/(congr1 (fun x => 3 %| x)); rewrite ?(Euclid_dvdX,Euclid_dvdM).
