...
(*                                                                            *)
(* `cluster F`                                                                *)
(*     ~ set of cluster points of `F`                                         *)
(*                                                                            *)
(* `compact`                                                                  *)
(*     ~ set of compact sets w.r.t. the filter-based definition of            *)
(*       compactness                                                          *)
(*                                                                            *)
(* `near_covering`                                                            *)
(*     ~ a reformulation of covering compact better suited for use            *)
(*       with `near`                                                          *)
(*                                                                            *)
(* `near_covering_within`                                                     *)
(*     ~ equivalent definition of `near_covering`                             *)
(*                                                                            *)
(* `compact_near F`                                                           *)
(*     ~ the filter `F` contains a closed compact set                         *)
(*                                                                            *)
(* `precompact A`                                                             *)
(*     ~ `A` is contained in a closed compact set                             *)
(*                                                                            *)
(* `locally_compact A`                                                        *)
(*     ~ every point in `A` has a compact (and closed) neighborhood           *)
(*                                                                            *)
(* `finite_subset_cover D F A`                                                *)
(*     ~ the family of sets `F` is a cover of `A` for a finite number         *)
(*       of indices in `D`                                                    *)
(*                                                                            *)
(* `cover_compact`                                                            *)
(*     ~ set of compact sets w.r.t. the open cover-based definition           *)
(*       of compactness                                                       *)
(*                                                                            *)
(* `open_fam_of A D f`                                                        *)
(*     ~ the family of `f` indexed by `D` restricted to `A` is a              *)
(*       family of open sets                                                  *)
(*                                                                            *)
(* `closed_fam_of A D f`                                                      *)
(*     ~ the family of `f` indexed by `D` restricted to `A` is a              *)
