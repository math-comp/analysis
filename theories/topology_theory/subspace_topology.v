...
(*                                                                            *)
(* `subspace A`                                                               *)
(*     ~ for `(A : set T)`, this is a copy of `T` with a topology that        *)
(*       ignores points outside `A`                                           *)
(*                                                                            *)
(* `incl_subspace x`                                                          *)
(*     ~ with `x` of type `subspace A` with `(A : set T)`, inclusion of       *)
(*       `subspace A` into `T`                                                *)
(*                                                                            *)
(* `nbhs_subspace x`                                                          *)
(*     ~ filter associated with `x : subspace A`                              *)
(*                                                                            *)
(* `from_subspace A f`                                                        *)
(*     ~ function of type `subspace A -> U` given a function `f` of type      *)
(*       `A -> U`. The purpose of this definition is to preserve the          *)
(*       pretty-printing of the notation `{within _, continuous _}`           *)
(*       below. Its use is however likely to be later superseded by a         *)
(*       better (compositional) mechanism.                                    *)
(*                                                                            *)
(* `{within A, continuous f}`                                                 *)
(*     ~ `continuous (from_subspace A f))`                                    *)
(*                                                                            *)
(* `subspace_ent A`                                                           *)
(*     ~ subspace entourages                                                  *)
(*                                                                            *)
(* `subspace_ball A`                                                          *)
(*     ~ balls of the pseudometric subspace structure                         *)
(*                                                                            *)
(* `continuousSubspaceType A B`                                               *)
(*     ~ type of continuous functions from set `A` to set `B` with            *)
(*       domain `subspace A`. The HB structure is `ContinuousFun`.            *)
(******************************************************************************)
