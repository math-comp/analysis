...
(* `increasing_seq u`                                                         *)
(*     ~ the sequence `u` is (strictly) increasing                            *)
(*                                                                            *)
(* `decreasing_seq u`                                                         *)
(*     ~ the sequence `u` is (strictly) decreasing                            *)
(*                                                                            *)
(* ## About sequences of real numbers                                         *)
(* `[sequence u_n]_n`                                                         *)
(*     ~ the sequence of general element `u_n`                                *)
(*                                                                            *)
(* `series u_`                                                                *)
(*     ~ the sequence of partial sums of `u_`                                 *)
(*                                                                            *)
(* `telescope u_`                                                             *)
...
(*                                                                            *)
(* `geometric`                                                                *)
(*     ~ geometric sequence, also `arithmetic_mean`, `harmonic_mean`,         *)
(*       `root_mean_square`                                                   *)
(*                                                                            *)
(* `[series u_n]_n`                                                           *)
(*     ~ the series of general element `u_n`                                  *)
(*                                                                            *)
(* `[normed S]`                                                               *)
(*     ~ transforms a series `S = [series u_n]_n` in its normed series        *)
(*       ``[series `|u_n|]_n]`` (useful to represent absolute and normed      *)
(*       convergence: `cvg [norm S_n]`)                                       *)
(*                                                                            *)
(* `exp_coeff n`                                                              *)
(*     ~ the sequence of coefficients of the real exponential                 *)
(*                                                                            *)
(* `expR x`                                                                   *)
(*     ~ the exponential function defined on a `realType`                     *)
(*                                                                            *)
(* `is_cvg_series_exp_coeff`                                                  *)
(*     ~ convergence of `\sum_n^+oo x^n / n!`                                 *)
(*                                                                            *)
(* `\sum_<range> F i`                                                         *)
(*     ~ `lim (fun n => (\sum_<range>) F i))` where `<range>` can be          *)
(*       `(i <oo)`, `(i <oo | P i)`, `(m <= i <oo)`, or                       *)
(*       `(m <= i <oo | P i)`                                                 *)
(*                                                                            *)
(* Sections sequences_R_* contain properties of sequences of real numbers.    *)
(* For example:                                                               *)
(* `nonincreasing_cvgn_ge u_`                                                 *)
(*     ~ if `u_` is nonincreasing and convergent then                         *)
(*       `forall n, lim u_ <= u_ n`                                           *)
(*                                                                            *)
(* `nondecreasing_cvgn_le u_`                                                 *)
(*     ~ if `u_` is nondecreasing and convergent then                         *)
(*       `forall n, lim u_ >= u_ n`                                           *)
(*                                                                            *)
(* `nondecreasing_cvgn u_`                                                    *)
(*     ~ if `u_` is nondecreasing and bounded then `u_` is convergent         *)
(*       and its limit is `sup u_n`                                           *)
(*                                                                            *)
(* `nonincreasing_cvgn u_`                                                    *)
(*     ~ if `u_` is nonincreasing `u_` and bound by below then `u_` is        *)
(*       convergent                                                           *)
(*                                                                            *)
(* `adjacent_seq`                                                             *)
(*     ~ adjacent sequences lemma                                             *)
(*                                                                            *)
(* `cesaro`                                                                   *)
(*     ~ Cesaro's lemma                                                       *)
(*                                                                            *)
...
(* `nseries u`                                                                *)
(*     ~ `fun n => \sum_(0 <= k < n) u k` where `u` has type `nat^nat`        *)
(*                                                                            *)
(* ## About sequences of extended real numbers                                *)
(* `eseries u`                                                                *)
(*     ~ `[sequence \sum_(0 <= k < n) u k]_n` where `u` has type              *)
(*       `(\bar R)^nat`                                                       *)
(*                                                                            *)
(* `etelescope u`                                                             *)
(*     ~ `[sequence u n.+1 - u n]_n`                                          *)
(*                                                                            *)
...
(*                                                                            *)
(* `sups u`                                                                   *)
(*     ~ `[sequence sup (sdrop u n)]_n`                                       *)
(*                                                                            *)
(* `infs u`                                                                   *)
(*     ~ `[sequence inf (sdrop u n)]_n`                                       *)
(*                                                                            *)
(* `limn_sup`, `limn_inf`                                                     *)
(*     ~ limit sup/inferior for a sequence of reals                           *)
(*                                                                            *)
(* `esups u`                                                                  *)
(*     ~ `[sequence ereal_sup (sdrop u n)]_n`                                 *)
(*                                                                            *)
(* `einfs u`                                                                  *)
(*     ~ `[sequence ereal_inf (sdrop u n)]_n`                                 *)
(*                                                                            *)
(* `limn_esup u`, `limn_einf`                                                 *)
(*     ~ limit sup/inferior for a sequence of extended reals                  *)
(*                                                                            *)
(* `adjacent_set L R`                                                         *)
(*     ~ `L` and `R` are two adjacent sets of real numbers                    *)
(*                                                                            *)
(* `cut L R`                                                                  *)
(*     ~ `L` and `R` are two sets of real numbers that form a cut             *)
(*                                                                            *)
(* ## Bounded functions                                                       *)
...
(* `bounded_fun_norm f`                                                       *)
(*     ~ a function between normed spaces transforms a bounded set into       *)
(*       a bounded set                                                        *)
(*                                                                            *)
(* `pointwise_bounded F`                                                      *)
(*     ~ `F` is a set of pointwise bounded functions between normed           *)
(*       spaces                                                               *)
(*                                                                            *)
(* `uniform_bounded F`                                                        *)
(*     ~ `F` is a set of uniform bounded functions between normed spaces      *)
(******************************************************************************)
