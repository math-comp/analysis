...
Lemma binomial_over1 n p U :
  ...
  bernoulli_trunc (1 - p.~ ^+ n) U :> \bar R)%E.
Proof.
move=> /andP[p0 p1].
...
rewrite addeC -ge0_sume_distrl.
  ...
  have -> : (\sum_(i < n) (p ^+ (1 + i) * p.~ ^+ (n - (1 + i)) *+ 'C(n, 1 + i))%:E)%E =
  (\sum_(i < n.+1) (p ^+ i * p.~ ^+ (n - i) *+ 'C(n, i))%:E - (p.~ ^+ n)%:E)%E.
  ...
  rewrite -(@exprDn_comm _ p.~ p n); last first.
...
move=> ->//.
rewrite /Baa'bb'Bab addn0 /beta_nat_norm/=.
repeat rewrite !factE/=.
