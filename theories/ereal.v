...
(* elements.                                                                  *)
(* `(\sum_(i \in A) f i)%E`                                                   *)
(*     ~ finitely supported sum, see `fsbigop.v`                              *)
(*                                                                            *)
(* `ereal_sup E`                                                              *)
(*     ~ supremum of `E`                                                      *)
(*                                                                            *)
...
(*     ~ topology for extended real numbers over `R`, a `realFieldType`       *)
(*                                                                            *)
(* `ereal_pseudoMetricType R`                                                 *)
(*     ~ pseudometric space for extended reals over `R` where is a            *)
(*       `realFieldType`; the distance between `x` and `y` is defined by      *)
(*       `` `|contract x - contract y| ``                                     *)
(*                                                                            *)
(* ## Filters                                                                 *)
(* `ereal_dnbhs x`                                                            *)
(*     ~ filter on extended real numbers that corresponds to the deleted      *)
(*       neighborhood `x^'` if `x` is a real number and to predicates         *)
(*       that are eventually true if `x` is `+oo`/`-oo`                       *)
(*                                                                            *)
(* `ereal_nbhs x`                                                             *)
(*     ~ same as `ereal_dnbhs` where `dnbhs` is replaced with `nbhs`          *)
(*                                                                            *)
(* `ereal_loc_seq x`                                                          *)
(*     ~ sequence that converges to `x` in the set of extended real           *)
(*       numbers                                                              *)
(*                                                                            *)
(******************************************************************************)
