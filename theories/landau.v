...
(* ## Parsable notations                                                      *)
(* `[bigO of f]`                                                              *)
(*     ~ recovers the canonical structure of big-o of `f` expands to          *)
(*       itself                                                               *)
(*                                                                            *)
(* `f =O_F h`                                                                 *)
(*     ~ `f` is a bigO of `h` near `F`, this is the preferred way for         *)
(*       statements. expands to the equation `(f = 'O_F h)` rewrite           *)
(*       from LEFT to RIGHT only                                              *)
(*                                                                            *)
(* `f = g +O_F h`                                                             *)
(*     ~ `f` is equal to `g` plus a bigO near `F`, this is the                *)
(*       preferred way for statements. expands to the equation                *)
(*       `(f = g + 'O_F h)` rewrite from LEFT to RIGHT only `/!\` When        *)
(*       you have to prove `(f =O_F h)` or `(f = g +O_F h)`. you must         *)
(*       `(apply: eqOE)` as soon as possible in a proof in order to           *)
(*       turn it into `'a_O_F f` with a shelved content `/!\` under           *)
(*       rare circumstances, a hint may do that for you                       *)
(*                                                                            *)
(* `[O_F h of f]`                                                             *)
(*     ~ returns a function with a bigO canonical structure provably          *)
(*       equal to `f` if `f` is indeed a bigO of `h` provably equal to        *)
(*       `0` otherwise expands to `('O_F h)`                                  *)
(*                                                                            *)
(* `'O_F`                                                                     *)
(*     ~ pattern to match a bigO with a specific `F`                          *)
(*                                                                            *)
(* `'O`                                                                       *)
(*     ~ pattern to match a bigO with a generic `F`                           *)
(*                                                                            *)
(* `f x =O_(x \near F) e x`                                                   *)
(*     ~ alternative way of stating `f =O_F e` (provably equal using          *)
(*       the lemma `eqOEx`                                                    *)
(*                                                                            *)
...
(* `{O_F f}`                                                                  *)
(*     ~ the type of functions that are a bigO of `f` near `F`                *)
(*                                                                            *)
(* `'a_O_F f`                                                                 *)
(*     ~ an existential bigO, must come from `(apply: eqOE)`                  *)
(*                                                                            *)
(* `'O_F f`                                                                   *)
(*     ~ a generic bigO, with a function you should not rely on, but          *)
(*       there is no way you can use `eqOE` on it.                            *)
(* The former works exactly same by with littleo instead of bigO.             *)
(*                                                                            *)
(* ## Asymptotic equivalence                                                  *)
(* `f ~_ F g`                                                                 *)
(*     ~ function `f` is asymptotically equivalent to function `g` for        *)
(*       filter `F`, i.e., `f = g +o_ F g`                                    *)
(*                                                                            *)
(* `f ~~_ F g`                                                                *)
(*     ~ `f == g +o_ F g` (i.e., as a boolean relation)                       *)
...
(* `{Omega_F f}`                                                              *)
(*     ~ the type of functions that are a big Omega of `f` near `F`           *)
(*                                                                            *)
(* `[bigOmega of f]`                                                          *)
(*     ~ recovers the canonical structure of big-Omega of `f`                 *)
(*                                                                            *)
(* `[Omega_F e of f]`                                                         *)
(*     ~ returns a function with a bigOmega canonical structure               *)
(*       provably equal to `f` if `f` is indeed a bigOmega of `e` or          *)
(*       `e` otherwise                                                        *)
(*                                                                            *)
(* `f \is 'Omega_F(e)`                                                        *)
(*     ~ `f : T -> V` is a bigOmega of `e : T -> W` near `F`                  *)
(*                                                                            *)
(* `f =Omega_F h`                                                             *)
(*     ~ `f : T -> V` is a bigOmega of `h : T -> V` near `F`                  *)
