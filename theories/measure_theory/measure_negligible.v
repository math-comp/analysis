...
(*                                                                            *)
(* `{ae mu, P}`                                                               *)
(*     ~ `P` holds almost everywhere for the measure `mu`, declared as        *)
(*       an instance of the type of filters `P` must be of the form           *)
(*       `forall x, Q x`. Prefer this notation when `P` is an existing        *)
(*       statement (i.e., a definition) that needs to be relativised.         *)
(*       The notation used the definition `almost_everywhere`.                *)
(*                                                                            *)
(* `\forall x \ae mu, P x`                                                    *)
(*     ~ equivalent to `{ae mu, forall x, P x}` Prefer this notation          *)
(*       when the statement `forall x, P x` does not stand alone.             *)
(*                                                                            *)
(* `f = g %[ae mu in D ]`                                                     *)
(*     ~ `f` is equal to `g` almost everywhere in `D`                         *)
(*                                                                            *)
(* `f = g %[ae mu]`                                                           *)
(*     ~ `f` is equal to `g` almost everywhere                                *)
(*                                                                            *)
(* `mu-.null_set`                                                             *)
(*     ~ (measure-theoretic) null sets                                        *)
(*                                                                            *)
(* ``m1 `<< m2``                                                              *)
(*     ~ `m1` is absolutely continuous w.r.t. `m2` or `m2` dominates          *)
(*       `m1`                                                                 *)
(*                                                                            *)
(* `content_dominates mu nu`                                                  *)
...
Lemma measure0_null_setP m A : measurable A -> m.-null_set A <-> m A = 0.
Proof.
...
Lemma null_setU m B : measurable B ->
  m.-null_set B <-> (forall A, measurable A -> m (A `|` B) = m A).
Proof.
...
Lemma null_content_dominatesP (nu : set T -> \bar R) mu :
...
Proof.
move=> mE /null_content_dominatesP m21 [A [*]]; exists A; split => //.
exact: m21.
Qed.
