...
(*     ~ notation for additive charges where `T` is a semiring of sets        *)
(*       and `R` is a `numFieldType`. The HB class is `AdditiveCharge`        *)
(*                                                                            *)
(* `{charge set T -> \bar R}`                                                 *)
(*     ~ type of charges over `T` a semiring of sets where `R` is a           *)
(*       `numFieldType`. The HB class is `Charge`                             *)
(*                                                                            *)
(* `isCharge`                                                                 *)
(*     ~ factory corresponding to the "textbook definition" of charges        *)
(*                                                                            *)
(* ## Instances of mathematical structures                                    *)
(* `measure_of_charge nu nu0`                                                 *)
(*     ~ measure corresponding to the charge `nu`, `nu0` is a proof           *)
(*       that `nu` is non-negative                                            *)
(*                                                                            *)
(* `crestr nu mD`                                                             *)
(*     ~ restriction of the charge `nu` to the domain `D` where `mD` is       *)
(*       a proof that `D` is measurable                                       *)
(*                                                                            *)
(* `crestr0 nu mD`                                                            *)
(*     ~ `csrestr nu mD` that returns `0` for non-measurable sets             *)
(*                                                                            *)
(* `czero`                                                                    *)
(*     ~ zero charge                                                          *)
(*                                                                            *)
(* `cscale r nu`                                                              *)
(*     ~ charge `nu` scaled by a factor `r : R`                               *)
(*                                                                            *)
(* `copp nu`                                                                  *)
(*     ~ the charge corresponding to the opposite of the charges `nu`         *)
(*                                                                            *)
(* `cadd n1 n2`                                                               *)
(*     ~ the charge corresponding to the sum of charges `n1` and `n2`         *)
(*                                                                            *)
(* `charge_of_finite_measure mu`                                              *)
(*     ~ charge corresponding to a finite measure `mu`                        *)
(*                                                                            *)
(* ## Theory                                                                  *)
(* `nu.-positive_set P`                                                       *)
(*     ~ `P` is a positive set with `nu` a charge                             *)
(*                                                                            *)
(* `nu.-negative_set N`                                                       *)
(*     ~ `N` is a negative set with `nu` a charge                             *)
(*                                                                            *)
(* `hahn_decomposition nu P N`                                                *)
(*     ~ the full set can be decomposed in `P` and `N`, a positive set        *)
(*       and a negative set for the charge `nu`                               *)
(*                                                                            *)
(* `jordan_pos nuPN`                                                          *)
(*     ~ the charge obtained by restricting the charge `nu` to the            *)
(*       positive set `P` of the Hahn decomposition                           *)
(*       `nuPN`: `hahn_decomposition nu P N`                                  *)
(*                                                                            *)
(* `jordan_neg nuPN`                                                          *)
(*     ~ the charge obtained by restricting the charge `nu` to the            *)
(*       positive set `N` of the Hahn decomposition                           *)
(*       `nuPN`: `hahn_decomposition nu P N`                                  *)
(*                                                                            *)
(* `charge_variation nuPN`                                                    *)
(*     ~ variation of the charge `nu`                                         *)
(*     ~ `:= jordan_pos nuPN \+ jordan_neg nuPN`                              *)
(*                                                                            *)
(* `charge_dominates mu nuPN`                                                 *)
