...
(*                                                                            *)
(* `subset_sigma_subadditive mu`                                              *)
(*     ~ alternative predicate defining sigma-subadditivity                   *)
(*                                                                            *)
(* `{content set T -> \bar R}`                                                *)
(*     ~ type of contents; `T` is expected to be a semiring of sets and       *)
(*       `R` a `numFieldType`. The HB class is `Content`.                     *)
(*                                                                            *)
(* `Content_isMeasure`                                                        *)
(*     ~ interface that extends a content to a measure with the proof         *)
(*       that it is `semi_sigma_additive`                                     *)
(*                                                                            *)
(* `{measure set T -> \bar R}`                                                *)
(*     ~ type of (non-negative) measures; `T` is expected to be a             *)
(*       semiring of sets and `R` is expected to be a `numFieldType`.         *)
(*       The HB class is `Measure`.                                           *)
(*                                                                            *)
(* `isMeasure`                                                                *)
(*     ~ interface corresponding to the "textbook definition" of              *)
(*       measures                                                             *)
(* ## Instances of measures                                                   *)
(*                                                                            *)
(* `msum mu n`                                                                *)
(*     ~ the measure corresponding to the sum of the measures `mu_0`,         *)
(*       ..., `mu_{n-1}`                                                      *)
(*                                                                            *)
(* `@mzero T R`                                                               *)
(*     ~ the zero measure                                                     *)
(*                                                                            *)
(* `measure_add m1 m2`                                                        *)
(*     ~ the measure corresponding to the sum of the measures `m1` and        *)
(*       `m2`                                                                 *)
(*                                                                            *)
(* `mscale r m`                                                               *)
(*     ~ the measure of corresponding to `fun A => r * m A` where `r`         *)
(*       has type `{nonneg R}`                                                *)
(*                                                                            *)
(* `mseries mu n`                                                             *)
(*     ~ the measure corresponding to the sum of the measures `mu_n`,         *)
(*       `mu_{n+1}`, ...                                                      *)
(*                                                                            *)
(* `pushforward m f`                                                          *)
(*     ~ pushforward of a set function `m : set T1 -> \bar R` by              *)
(*       `f : T1 -> T2`; pushforward/image measure if `m` is a measure        *)
(*       and `f` measurable                                                   *)
(*                                                                            *)
(* `G.-ring.-measurable A`                                                    *)
(*     ~ `A` belongs to the ring of sets `<<r G >>`                           *)
(*                                                                            *)
(* `fin_num_fun`                                                              *)
(*     ~ predicate for finite function over measurable sets                   *)
(*                                                                            *)
(* `sfinite_measure`                                                          *)
(*     ~ predicate for s-finite measure functions                             *)
(*                                                                            *)
(* `sigma_finite A f`                                                         *)
(*     ~ the measure function `f` is sigma-finite on the `A : set T`          *)
(*       with `T` a semiring of sets                                          *)
(*                                                                            *)
(* `mrestr mu mD`                                                             *)
(*     ~ restriction of the measure `mu` to a set `D`; `mD` is a proof        *)
(*       that `D` is measurable                                               *)
(*                                                                            *)
(* `isSFinite`                                                                *)
(*     ~ interface for functions that satisfy the `sfinite_measure`           *)
(*       predicate                                                            *)
(*                                                                            *)
(* `{sfinite_measure set T -> \bar R}`                                        *)
(*     ~ type of s-finite measures. The HB class is `SFiniteMeasure`.         *)
(*                                                                            *)
(* `isSigmaFinite`                                                            *)
(*     ~ interface for functions that satisfy sigma finiteness                *)
(*                                                                            *)
(* `{sigma_finite_content set T -> \bar R}`                                   *)
(*     ~ contents that are also sigma finite. The HB class is                 *)
(*       `SigmaFiniteContent`.                                                *)
(*                                                                            *)
(* `{sigma_finite_measure set T -> \bar R}`                                   *)
(*     ~ measures that are also sigma finite. The HB class is                 *)
(*       `SigmaFiniteMeasure`.                                                *)
(*                                                                            *)
(* `Measure_isSFinite`                                                        *)
(*     ~ interface that extends a measure to an s-finite measure using        *)
(*       a sequence of finite measures                                        *)
(*                                                                            *)
(* `isFinNumFun`                                                              *)
(*     ~ interface for functions that satisfy the `fin_num_fun`               *)
(*       predicate                                                            *)
(*                                                                            *)
(* `FinNumFun.type`                                                           *)
(*     ~ type of functions over semiring of sets returning a `fin_num`.       *)
(*       The HB class is `FinNumFun`.                                         *)
(*                                                                            *)
(* `{finite_measure set T -> \bar R}`                                         *)
(*     ~ finite measures. The HB class is `FiniteMeasure`.                    *)
(*                                                                            *)
(* `Measure_isFinite`                                                         *)
(*     ~ interface that extends a measure to a finite measure using a         *)
(*       proof of `fin_num_fun`                                               *)
(*                                                                            *)
(* `sfinite_measure_seq mu`                                                   *)
(*     ~ the sequence of finite measures of the s-finite measure `mu`         *)
(*                                                                            *)
(* `mfrestr mD muDoo`                                                         *)
(*     ~ finite measure corresponding to the restriction of the measure       *)
(*       `mu` over `D` with `mu D < +oo`, `mD : measurable D`,                *)
(*       `muDoo : mu D < +oo`                                                 *)
(*                                                                            *)
(* `lim_sup_set F`                                                            *)
(*     ~ limit superior (or upper limit) of a sequence of sets `F`            *)
(*                                                                            *)
(******************************************************************************)
