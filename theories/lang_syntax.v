...
(*Definition ubeta_nat_pdf (t : R) :=
  if (0 <= t <= 1)%R then (t ^+ a.-1 * t.~ ^+ b.-1)%R else 0%R.
...
Lemma continuous_onemXn {R : realType} (n : nat) x :
  {for x, continuous (fun y : R => y.~ ^+ n)%R}.
Proof.
...
Definition XMonemX a b := fun x : R => x ^+ a * x.~ ^+ b.
...
Lemma XMonemX0 n x : XMonemX 0 n x = x.~ ^+ n :> R.
...
Lemma derivable_oo_continuous_bnd_onemXnMr {R : realType} (n : nat) (r : R) :
  derivable_oo_LRcontinuous (fun x : R => x.~ ^+ n.+1 * r : R^o)%R 0 1.
Proof.
split.
...
Lemma derive_onemXn {R : realType} (n : nat) x :
  ((fun y : R => y.~ ^+ n.+1 : R^o)^`() x = - n.+1%:R * x.~ ^+ n)%R.
Proof.
...
(* TODO: move *)
Lemma normr_onem {R : realType} (x : R) : (0 <= x <= 1 -> `| x.~ | <= 1)%R.
Proof.
...
rewrite (@Rintegration_by_parts _ _
    (fun x => x.~ ^+ b.+1 / - b.+1%:R)%R (fun x => a.+1%:R * x ^+ a)%R); last 7 first.
  exact: ltr01.
  apply/continuous_subspaceT.
  move=> x.
...
Lemma beta_prob_bernoulliE a b c d U : (a > 0)%N -> (b > 0)%N ->
  beta_prob_bernoulli a b c d U = bernoulli_prob (div_betafun a b c d) U.
Proof.
move=> a0 b0.
...
under eq_integral => x.
  ...
  over.
rewrite /=.
rewrite [in RHS]bernoulli_probE/= ?div_betafun_ge0 ?div_betafun_le1//=.
