...
Lemma epsilon_trick (A : (er R) ^nat) (e : R) (P : pred nat) : (0 <= e)%R ->
  (forall n, 0%:E <= A n)%E ->
  (lim (fun n => \sum_(i < n | P i) ((A i) + (e / (2 ^ i)%:R)%:E)%E) <=
  lim (fun n => \sum_(i < n | P i) (A i)) + (2 * e)%R%:E)%E.
Proof.
move=> e0 A0.
rewrite (@le_trans _ _ (lim (fun n => (\sum_(i < n | P i) (A i))%E +
                                   (\sum_(i < n) ((e / (2 ^ i)%:R)%:E)))%E)) //.
  apply: lee_lim.
    apply: (@is_cvg_ereal_nneg_series _ (fun i => A i + (e / (2 ^ i)%:R)%:E)%E P).
    move=> n Pn; apply: adde_ge0 => //.
    by apply divr_ge0 => //; rewrite ler0n.
    apply/cvg_ex; eexists.
    apply/nondecreasing_seq_ereal_cvg => i j ij.
    apply: lee_add => //.
      exact: lee_sum_nneg_ord.
      apply: (@lee_sum_nneg_ord _ (fun i => (e / (2 ^ i)%:R)%:E) xpredT) => //.
      by move=> n _; apply divr_ge0 => //; rewrite ler0n //.
  near=> n.
  rewrite big_split /=.
  apply: lee_add => //.
  move: (@lee_sum_nneg R _ (enum 'I_n) xpredT P (fun i => (e / (2 ^ i)%:R)%:E)).
  rewrite big_enum_cond big_enum; apply => i _ Pi.
  by rewrite lee_fin; apply divr_ge0 => //; rewrite ler0n.
...
- by apply: (@is_cvg_ereal_nneg_series _ A P) => n.
...
apply: (le_trans _ (@epsilon_trick (mu_ext \o A) e%:num xpredT _ _)) => //; last first.
...
Lemma Rfloor_gt0 (R : realType) (r : R) : r < 0 -> Rfloor r < 0.
Proof. by move=> r0; rewrite (le_lt_trans _ r0) // ge_Rfloor. Qed.

Lemma floor_gt0 (R : realType) (r : R) : r < 0 -> floor r < 0.
Proof.
by move=> r0; rewrite /floor -(@ltrz0 R) RtointK ?isint_Rfloor // Rfloor_gt0.
Qed.

Lemma ceil_gt0 (R : realType) (r : R) : 0 < r -> 0 < ceil r.
Proof. by move=> r0; rewrite /ceil oppr_gt0 floor_gt0 // ltr_oppl oppr0. Qed.

Lemma ler_floor (R : realType) (r : R) : (floor r)%:~R <= r.
Proof. by rewrite -RfloorE; exact: ge_Rfloor. Qed.

Lemma ler_ceil (R : realType) (r : R) : r <= (ceil r)%:~R.
Proof. by rewrite -RceilE; exact: le_Rceil. Qed.

Lemma intN (R : numDomainType) (i : int) : (- i)%:~R = - i%:~R :> R.
Proof. by case: i => [[] //|[|?]]; rewrite ?(oppr0,NegzE,opprK). Qed.
...
Lemma ge_is_realoo (R : numDomainType) (r : R) (x : er R) :
  (r%:E <= x)%E -> is_real x \/ x = +oo%E.
Proof. by case: x => [r'| |] //; [left|right]. Qed.

Lemma is_real_adde_undef (R : numDomainType) (x y : {ereal R}) :
  is_real y -> ~~ adde_undef x y.
Proof. by move: x y => [x| |] [y | |]. Qed.

Lemma adde_undefC (R : numDomainType) (x y : er R) :
  adde_undef x y = adde_undef y x.
Proof. by rewrite /adde_undef orbC andbC (andbC (x == +oo)%E). Qed.

Lemma lee_is_real (R : realDomainType) (a b : R) (x : {ereal R}) :
  (a%:E <= x <= b%:E)%E -> is_real x.
Proof. by move: x => [x | |] //; rewrite lee_pinfty_eq lee_pinfty. Qed.
...
(* NB: not used *)
Lemma trivIset_subsequence (T : Type) (s : nat -> set T) (e : nat -> nat) : injective e ->
...
Lemma csum_nat_lim (R : realType) (a : nat -> {ereal R}) :
  (forall n, 0%:E <= a n)%E ->
  csum setT a = lim (fun n => (\sum_(i < n) a i)%E).
Proof.
move=> a0; rewrite -(@csum_countable _ _ _ id xpredT) //; congr csum.
by rewrite predeqE => /= n; split => [Pn|[m Pm <-//]]; exists n.
Qed.

Lemma sum_lim_sumC (R : realType) N (f : nat -> nat -> {ereal R}) :
  (forall a b, 0%:E <= f a b)%E ->
  (\sum_(i < N) (lim (fun n => (\sum_(k < n) f i k)%E)) <=
  lim (fun n : nat => \sum_(i < n) (\sum_(k < N) f k i)%E)%E)%E.
Proof.
move=> f0; elim: N => [|N ih].
  rewrite big_ord0 (_ : (fun n : nat => _) = (fun n : nat => 0%:E)) ?lim_cst//.
  by rewrite funeqE => i; rewrite exchange_big big_ord0.
rewrite big_ord_recr /=.
set x := (fun n => \sum_(i < n) (\sum_(k < N) f k i)%E)%E.
set y := (fun n => \sum_(k < n) f N k)%E.
set z := (fun n => \sum_(i < n) (\sum_(k < N.+1) f k i)%E)%E.
have xyz : (x \+ y = z)%E.
  rewrite funeqE => n; rewrite /x /y /z -big_split; apply eq_bigr => i _.
  by rewrite big_ord_recr.
rewrite (@le_trans _ _ (lim x + lim y)%E) //; first exact: lee_add2r.
have xcvg : x --> ereal_sup (x @` setT).
  apply nondecreasing_seq_ereal_cvg.
  apply: (@lee_sum_nneg_ord _ (fun n => \sum_(k < N) f k n)%E xpredT) => n _.
  exact: sume_ge0.
have ? : cvg x.
  apply: (@is_cvg_ereal_nneg_series _ (fun i => (\sum_(k < N) f k i)%E) xpredT) => // i _.
  exact: sume_ge0.
have ? : cvg y by exact: (@is_cvg_ereal_nneg_series _ (f N) xpredT).
have /ge_is_realoo[rx|xoo] : (0%:E <= ereal_sup (x @` setT))%E.
  by apply: ereal_sup_ub; rewrite /x /=; exists O => //; rewrite big_ord0.
- rewrite /x /y -ereal_limD // ?xyz // -/x.
  by rewrite (cvg_lim _ xcvg) // adde_undefC is_real_adde_undef.
- suff : (lim x <= lim z)%E.
    by rewrite (cvg_lim _ xcvg) // xoo lee_pinfty_eq => /eqP ->; rewrite lee_pinfty.
  apply: lee_lim => //.
  apply: (@is_cvg_ereal_nneg_series _ (fun i => (\sum_(k < N.+1) f k i)%E) xpredT) => // i _.
  exact: sume_ge0.
...
Lemma lt_lim (R : realFieldType) (u_ : R^o^nat) (M : R) :
  ...
  \forall n \near \oo, (M <= u_ n)%R.
Proof.
move=> ndu cu Ml; have [[n Mun]|] := pselect (exists n, M <= u_ n).
  near=> m; suff : u_ n <= u_ m by exact: le_trans.
  by near: m; by exists n.+1 => // p q; apply/ndu/ltnW.
move/forallNP => Mu.
have {}Mu : forall x, M > u_ x by move=> x; rewrite ltNge; apply/negP.
have : lim u_ <= M by apply lim_le => //; near=> m; exact/ltW/Mu.
by move/(lt_le_trans Ml); rewrite ltxx.
Grab Existential Variables. all: end_near. Qed.

(* NB: too similar to lt_lim *)
Lemma ereal_lt_lim (R : realFieldType) (u_ : {ereal R}^nat) (M : R) :
  nondecreasing_seq u_ -> cvg u_ -> (M%:E < lim u_)%E ->
  \forall n \near \oo, (M%:E <= u_ n)%E.
Proof.
move=> ndu_ cu_ Ml; have [[n Mu_n]|] := pselect (exists n, (M%:E <= u_ n)%E).
  near=> m; suff : (u_ n <= u_ m)%E by exact: le_trans.
  by near: m; exists n.+1 => // p q; apply/ndu_/ltnW.
move/forallNP => Mu.
have {}Mu : forall x, (M%:E > u_ x)%E by move=> x; rewrite ltNge; apply/negP.
have : (lim u_ <= M%:E)%E by apply ereal_lim_le => //; near=> m; apply/ltW/Mu.
by move/(lt_le_trans Ml); rewrite ltxx.
Grab Existential Variables. all: end_near. Qed.

Lemma lee_sum_lim (R : realType)
  (u_ : (er [topologicalType of R^o])^nat) (F : {fset nat}) (P : pred nat) :
  (forall i, P i -> 0%:E <= u_ i)%E ->
...
Lemma mem_Decompose (j : interval R) (s : seq (interval R)) (r : R) :
  j \in s -> set_of_itv j r ->
  exists j', j' \in Decompose s /\ set_of_itv j' r.
Proof.
move=> js jr.
have : (\big[setU/set0]_(i <- s) set_of_itv i) r by rewrite -bigcup_mkset; exists j.
...
Lemma itvs_ringOfSets_set_itv (i : interval R) :
  itvs_ringOfSets_carrier (set_of_itv i).
Proof. by exists [:: i]; rewrite /ufint big_seq1. Qed.

Lemma itvs_ringOfSets_bigsetU (s : seq (interval R)) :
  itvs_ringOfSets_carrier (\big[setU/set0]_(j <- s) set_of_itv j).
Proof.
apply: (@bigsetU_measurable itvs_ringOfSetsType) => i _.
exact: itvs_ringOfSets_set_itv.
Qed.

End ring_of_sets_instance.
End ITVS.
Notation itvs_ringOfSetsType := ITVS.itvs_ringOfSetsType.
...
Lemma hlength_setT : hlength setT = +oo%E :> er R.
Proof. by rewrite -set_of_itv_minfty_pinfty hlength_itv. Qed.

Lemma hlength_ninfty b r : hlength (set_of_itv (Interval -oo%O (BSide b r))) = +oo%E :> er R.
Proof. by rewrite hlength_itv /= lte_ninfty. Qed.

Lemma hlength_pinfty b r : hlength (set_of_itv (Interval (BSide b r) +oo%O)) = +oo%E :> er R.
...
Lemma hlength_set_of_itv_infinite (R : realType) (i : interval R) :
  hlength (set_of_itv i) = +oo%E ->
  i = `]-oo, +oo[ \/ exists b r, i = Interval -oo%O (BSide b r) \/
                           i = Interval (BSide b r) +oo%O.
Proof.
move: i => [[ba a|[]] [bb b[]|[]]]; try by rewrite hlength_itv.
- by rewrite hlength_itv /=; case: ifPn.
- by move=> _; right; exists ba, a; right.
- by move=> _; right; exists bb, b; left.
- by left.
Qed.
...
Fail Lemma partition_disjoint_bigcup (f : T -> R) (F : I -> set T)
  ...
  \big[
op/idx]_(i <- \big[setU/set0]_(x in K) (F x)) f i =
...
Definition length (x : set R) : {ereal R} :=
 if pselect (ITVS.itvs_ringOfSets_carrier x) is
   left e then (\sum_(i <- Decompose (projT1 (cid e))) hlength (set_of_itv i))%E
 else -oo%E.

Lemma length_ge0 (X : set (itvs_ringOfSetsType R)) :
  ITVS.itvs_ringOfSets_carrier X -> (0%:E <= length X)%E.
Proof.
...
Lemma length_sigma_finite : sigma_finite setT (length : set (itvs_ringOfSetsType R) -> {ereal R}).
Proof.
...
Lemma length_itv2n (n : nat) : length (set_of_itv `[(- n%:R), n%:R]) = n.*2%:R%:E.
Proof.
rewrite length_itv hlength_itv /= lte_fin -{1}(add0r (-n%:R)) ltr_subl_addl.
rewrite -natrD ltr0n addnn double_gt0; case: ifPn => [n0|].
  by rewrite -addnn natrD 2!addERFin opprK.
by rewrite -leqNgt leqn0 => /eqP ->.
Qed.

Lemma length_itv2n_sym b r N :
  length (set_of_itv (Interval (BSide b r) +oo%O) `&` set_of_itv `[(-N%:R), N%:R]) =
  length (set_of_itv (Interval -oo%O (BSide b (- r)%R)) `&` set_of_itv `[(-N%:R), N%:R]).
Proof.
rewrite -2!itv_meetE 2!length_itv 2!hlength_itv /= 2!lte_fin; case: ifPn.
...
- case: ifPn => //.
  ...
  rewrite -leNgt lexU leNgt rN /= -subr_le0 opprK -natrD lern0 addnn.
  by rewrite double_eq0 => /eqP N0; rewrite N0 addr0 meet_r // ler_oppr oppr0 ltW //; move: rN; rewrite N0.
Qed.

Lemma length_additive : additive (length : set (itvs_ringOfSetsType R) -> {ereal R}).
Proof.
...
suff : (length B = length A + length (B `\` A))%E.
  by move=> ->; rewrite lee_addl // length_ge0 //; exact: (@measurableD (itvs_ringOfSetsType R)).
...
Definition length_additive_measure : {additive_measure set (itvs_ringOfSetsType R) -> {ereal R}}.
apply: (@AdditiveMeasure.Pack _ (itvs_ringOfSetsType R) _ length (AdditiveMeasure.Axioms length_set0 length_ge0 _)).
...
Lemma is_cvg_sum_length (S : (set (itvs_ringOfSetsType R))^nat) (P : pred nat) :
  (forall k, P k -> measurable (S k)) -> cvg (fun n => (\sum_(k < n | P k) length (S k))%E).
Proof.
move=> mS; apply: (@is_cvg_ereal_nneg_series _ (length \o S) P) => n Pn.
exact/length_ge0/mS.
Qed.

Lemma bigcup_cond_neq0 (j : nat -> interval R) :
  \bigcup_k (set_of_itv (j k)) = \bigcup_(k in (fun x : nat => set_of_itv (j x) != set0)) (set_of_itv (j k)).
Proof.
by rewrite eqEsubset; split => [r [k ? kr]|r [k ? kr]]; exists k => //; apply/set0P; exists r.
Qed.

Lemma sum_length_cond_neq0 (j : (interval R)^nat) :
  (fun n => (\sum_(k < n) length (set_of_itv (j k)))%E) =
  (fun n => \sum_(k < n | set_of_itv (j k) != set0) length (set_of_itv (j k)))%E.
Proof.
rewrite funeqE => n; rewrite (bigID (fun k : 'I_n => set_of_itv (j k) == set0)) /=.
by rewrite big1 ?add0e // => k /eqP ->; rewrite length_set0.
Qed.

Lemma length_sub_sigma_additive_on_intervals_finite_case (i : interval R)
  (j : nat -> interval R) (P : pred nat) :
  (hlength (set_of_itv i) < +oo)%E ->
  (forall k, P k -> set_of_itv (j k) != set0) ->
  set_of_itv i `<=` \bigcup_(k in P) set_of_itv (j k) ->
  (length (set_of_itv i) <= lim (fun n => \sum_(k < n | P k) length (set_of_itv (j k))))%E.
Proof.
move=> iNoo jne ij.
set l := lim _.
have := lee_pinfty l.
...
have [j_finite|] := pselect (forall k, P k -> (hlength (set_of_itv (j k)) < +oo)%E); last first.
  ...
  rewrite /l.
  rewrite (@ereal_nneg_series_pinfty _ (fun k0 => length (set_of_itv (j k0))) P k) //.
  by rewrite lee_pinfty.
  move=> n Pn; apply length_ge0.
  exact: ITVS.itvs_ringOfSets_set_itv.
  by rewrite length_itv.
have [/eqP->|i0] := boolP (set_of_itv i == set0).
  rewrite length_set0.
  apply: (@ereal_nneg_series_lim_ge0 _ (fun k => length (set_of_itv (j k))) P) => k _.
  exact/length_ge0/ITVS.itvs_ringOfSets_set_itv.
have [ri1 ri2] := hlength_finite_is_real i0 iNoo.
...
have [ab|ba] := ltP a b; last first.
  ...
  apply: (@ereal_nneg_series_lim_ge0 _ (fun k => length (set_of_itv (j k))) P) => k _.
  exact/length_ge0/ITVS.itvs_ringOfSets_set_itv.
suff baj : forall e : {posnum R}, (b%:E - a%:E <= lim (fun n => \sum_(k < n | P k) length (set_of_itv (j k))) + e%:num%:E)%E.
...
have : (b'%:E - a'%:E <= lim (fun n => \sum_(k < n | P k) length (set_of_itv (j k))) + (e%:num / 2)%:E)%E.
  ...
  rewrite (@le_trans _ _ (length (\big[setU/set0]_(k <- F') set_of_itv `] (a'_ k), (b'_ k) [))) //.
    apply/le_length => /=.
    exact/ITVS.itvs_ringOfSets_set_itv.
    by (* TODO: lemma?*) exists [seq `](a'_ k), (b'_ k)[ | k <- F']; rewrite /ufint /= big_map.
  ...
  apply: (le_trans _ (@epsilon_trick _ (fun k => length (set_of_itv (j k)))%E (e%:num / (4%:R)) P _ _)) => //; last first.
    by move=> n; apply/length_ge0/ITVS.itvs_ringOfSets_set_itv.
  ...
  have ? : cvg (fun n : nat => (\sum_(k < n | P k) (e%:num / 4%:R / (2 ^ k)%:R)%:E)%E).
    apply: (@is_cvg_ereal_nneg_series _ (fun i => (e%:num / 4%:R / (2 ^ i)%:R)%:E) P) => n _.
    by rewrite lee_fin divr_ge0 // ler0n.
  apply (@le_trans _ _ (lim (fun n => \sum_(k < n | P k) (length (set_of_itv (j k))))%E
                        +
                        lim (fun n => \sum_(k < n | P k) (e%:num / 4%:R / (2 ^ k)%:R)%:E)))%E; last first.
    rewrite -ereal_limD //; last 2 first.
      apply: (@is_cvg_sum_length (fun k => set_of_itv (j k))) => k Pk.
      exact/ITVS.itvs_ringOfSets_set_itv.
    have K1 : (0%:E <= lim (fun n => (\sum_(k < n | P k) (e%:num / 4%:R / (2 ^ k)%:R)%:E)) <= (e%:num / 2)%:E)%E.
      apply/andP; split.
        ...
        move=> k _ _.
        by apply divr_ge0 => //; rewrite ler0n.
      apply: is_real_adde_undef => //.
      by apply: lee_is_real K1.
    rewrite (_ : (fun x : nat => _) = (fun x => \sum_(k < x | P k) (length (set_of_itv (j k)) + ((e)%:num / 4%:R / (2 ^ k)%:R)%:E)%E)%E) //.
    by rewrite funeqE => n; rewrite big_split.
  have Htmp : forall x, x \in F -> P x.
    by move=> x; have := HF_ x.
  have Htmp2 : forall f : nat -> {ereal R}, (\sum_(i1 <- F') (f i1) = \sum_(i1 <- F' | P i1) (f i1))%E.
  ...
  apply: lee_add; last first.
    ...
    rewrite (@le_trans _ _ (lim (fun n => \sum_(k < n | P k) (e%:num / (2 ^ k.+2)%:R)%:E))%E) //.
      rewrite Htmp2.
      by apply: lee_sum_lim => n _; apply divr_ge0 => //; rewrite ler0n.
    apply lee_lim => //.
      apply: (@is_cvg_ereal_nneg_series _ (fun i => (e%:num / (2 ^ i.+2)%:R)%:E) P) => n _.
      by rewrite lee_fin divr_ge0 // ler0n.
    near=> n.
    apply: lee_sum.
    move=> /= k _.
  ...
  rewrite Htmp2.
  by apply: lee_sum_lim => k _; exact/length_ge0/ITVS.itvs_ringOfSets_set_itv.
...
apply/eqP; rewrite eq_le; apply/andP; split.
  have H3 : set_of_itv i = \bigcup_(k in (fun x => set_of_itv (j x) != set0)) set_of_itv (j k).
    rewrite ij; apply/predeqP => r; split => [[x _ jxr]|[x]].
      by exists x => //; apply/set0P; exists r.
    by exists x.
  pose P := fun x => set_of_itv (j x) != set0.
  have H2 : set_of_itv i `<=` \bigcup_(k in (fun x => set_of_itv (j x) != set0)) set_of_itv (j k).
    by rewrite -H3.
  move: (@length_sub_sigma_additive_on_intervals_finite_case _ j P iNoo (fun x => id) H2).
  move/le_trans; apply.
  apply: lee_lim.
  + apply: (@is_cvg_sum_length (fun k => set_of_itv (j k))) => n jn0.
    exact/ITVS.itvs_ringOfSets_set_itv.
  + apply: (@is_cvg_sum_length (fun k => set_of_itv (j k)) xpredT) => n _.
    exact/ITVS.itvs_ringOfSets_set_itv.
  + near=> n.
    move: (@lee_sum_nneg R _ (enum 'I_n) xpredT (fun x => set_of_itv (j x) != set0) (fun x => length (set_of_itv (j x)))).
    rewrite /= big_enum_cond big_enum; apply.
    by move=> x _ _; exact/length_ge0/ITVS.itvs_ringOfSets_set_itv.
apply: ereal_lim_le.
  apply: (@is_cvg_sum_length (fun k => set_of_itv (j k)) xpredT) => n _.
  exact/ITVS.itvs_ringOfSets_set_itv.
near=> n.
...
Lemma length_sub_sigma_additive_on_intervals_infinite_case
    (i : interval R) (j : nat -> interval R) (P : pred nat ) :
  (hlength (set_of_itv i) = +oo)%E ->
 (forall k : nat, P k -> set_of_itv (j k) != set0) ->
  set_of_itv i `<=` \bigcup_(k in P) set_of_itv (j k) ->
  (length (set_of_itv i) <= lim (fun n => \sum_(k < n | P k) length (set_of_itv (j k))))%E.
Proof.
move=> ioo jne ij.
set IN := fun N => set_of_itv i `&` set_of_itv `[ (-(N%:R))%R, N%:R].
set IkN := fun k N => set_of_itv (j k) `&` set_of_itv `[ (-(N%:R))%R, N%:R].
have K6 : forall M, (M > 0)%R -> \forall n \near \oo, (M%:E <= (\sum_(k < n | P k) length (set_of_itv (j k)))%E)%E.
  have K1 : forall M, (M > 0)%R -> exists N : nat, (N >= 1)%nat /\ (M%:E < length (IN N))%E.
    move=> M M0.
    have ? : (0 < `|ceil M|)%N by rewrite absz_gt0 gt_eqF // ceil_gt0.
    move/hlength_set_of_itv_infinite : ioo => [ioo|[b [r iroo]]].
      exists (absz (ceil M)); split => //.
      rewrite /IN setIC (proj2 (setIidPl _ _)); last first.
        by rewrite ioo set_of_itv_minfty_pinfty.
      rewrite length_itv2n -addnn natrD addERFin lte_fin -{1}(addr0 M) ler_lt_add // ?ltr0n //.
      by rewrite (le_trans (ler_ceil _)) // gez0_norm // ceil_ge0 // ltW.
    rewrite /IN.
    wlog : i {ij IN} b r {iroo} / i = Interval -oo%O (BSide b r).
      move=> h; move: iroo => [->|iroo]; first exact: h.
      have [N [N0 MN]] := h _ b (- r) erefl.
      by exists N; split => //; rewrite iroo length_itv2n_sym.
    move=> ->{i}.
    have Nnn : forall n : nat, (0 < n)%N -> - n%:R < n%:R :> R.
      move=> n n0.
      by rewrite -subr_lt0 -opprD ltr_oppl oppr0 -natrD addnn ltr0n lt0n double_eq0 -lt0n.
    have ? : (0 < `|ceil (`|r| + M)|)%N by rewrite absz_gt0 gt_eqF // ceil_gt0 // -(addr0 0) ler_lt_add.
    have [r0|r0] := ler0P r.
      exists (`|ceil (`| r | + M) |%N.+1); split => //.
      rewrite -itv_meetE length_itv hlength_itv /= lte_fin ltxI Nnn // andbT ltr_oppl opprK.
      case: ifPn => [rrM|].
        rewrite meet_l ?(le_trans r0) // lte_fin mulrSr -gez0_norm // ?ceil_ge0// ?addr_ge0//; last exact/ltW.
        rewrite -ltr_subl_addl.
        rewrite (@le_lt_trans _ _ (`|r| + M)) // ?ler_addr//.
          by rewrite ler0_norm // addrC.
        by rewrite (@lt_le_trans _ _ (`|r| + M + 1)) // ?ltr_addl // ler_add2r -RceilE le_Rceil.
      move=> /negP; apply: absurd.
      rewrite -(ler0_norm r0) mulrSr ltr_spaddr // (@le_trans _ _ (`|r| + M)) // ?ler_addl //; first exact/ltW.
      rewrite -gez0_norm ?ceil_ge0 // ?addr_ge0 //; last exact/ltW.
      by rewrite ler_ceil.
    exists (`|ceil (`| r | + M)|%N); split => //.
    rewrite -itv_meetE length_itv hlength_itv /= lte_fin ltxI Nnn // ltr_oppl opprK andbT.
    case: ifPn => [_|].
      rewrite meet_l; last first.
        ...
        by rewrite ler_ceil.
      rewrite lte_fin -{1}(add0r M) ltr_le_add // -gez0_norm ?ceil_ge0// ?addr_ge0//; last exact/ltW.
      rewrite ger0_norm; last exact/ltW.
      rewrite (@le_trans _ _ (r + M)) // ?ler_addr //; first exact/ltW.
      by rewrite ler_ceil.
    move/negP; apply: absurd.
    by rewrite (@lt_le_trans _ _ 0) // ltr_oppl oppr0.
  move=> M M0.
  have {K1} [N [N1 MINN]] := K1 _ M0.
  have K2 : forall N, IN N `<=` \bigcup_(k in P) (IkN k N).
    move=> n.
    rewrite /IN.
    move/(@setSI _ _ _ (set_of_itv `[(1 *- n), n%:R] )) : ij.
    by rewrite bigcup_distrl.
  have K6 : (length (IN N) <= lim (fun n => \sum_(k < n | P k) length (IkN k N)))%E.
    rewrite /IN.
    rewrite -itv_meetE.
    suff : (length (set_of_itv (itv_meet i `[(1 *- N), N%:R])) <= lim (fun n : nat => \sum_(k < n | P k) length (set_of_itv (itv_meet (j k) `[(1 *- N), N%:R]))))%E.
      rewrite /IkN.
      move/le_trans; apply.
      rewrite le_eqVlt; apply/orP; left; apply/eqP.
      rewrite (_ : (fun n : nat => (\sum_(k < n | P k) length (set_of_itv (itv_meet (j k) `[(1 *- N), N%:R])))%E) = (fun n : nat => (\sum_(k < n | P k) length (set_of_itv (j k) `&` set_of_itv `[(1 *- N), N%:R]))%E)) //.
     by rewrite funeqE => /= n; under eq_bigr do rewrite /IkN itv_meetE.
    rewrite (_ : (fun n : nat => \sum_(k < n | P k) length (set_of_itv (itv_meet (j k) `[(1 *- N), N%:R])))%E =
                 (fun n : nat => \sum_(k < n | P k && (set_of_itv ((itv_meet (j k) `[(1 *- N), N%:R])) != set0)) length (set_of_itv (itv_meet (j k) `[(1 *- N), N%:R])))%E); last first.
      rewrite funeqE => /= n.
      rewrite (bigID (fun k : 'I_n => set_of_itv (itv_meet (j k) `[(1 *- N), N%:R]) == set0)) /=.
      rewrite big1 ?add0e // => k /andP[?] /eqP ->.
      by rewrite length_set0.
    apply: (@length_sub_sigma_additive_on_intervals_finite_case (itv_meet i `[(1 *- N), N%:R]) (fun k => itv_meet (j k) (`[(- N%:R), N%:R])) (fun k => P k && (set_of_itv ((itv_meet (j k) `[(1 *- N), N%:R])) != set0))) => //.
    - rewrite (@le_lt_trans _ _ (hlength (set_of_itv `[(-N%:R), (N%:R)]))) //.
        by apply le_hlength; rewrite itv_meetE; apply subIset; right.
      by rewrite -length_itv length_itv2n lte_pinfty.
    - by move=> k /andP[].
    - apply: (@subset_trans _ _ (\bigcup_(k in P) (set_of_itv (j k) `&` set_of_itv `[(1 *- N), N%:R]))).
        move=> x.
        rewrite itv_meetE.
        rewrite /IN in K2.
        move/K2.
       ...
       exists k => //.
       apply/andP; split => //.
         apply/set0P; exists r.
         by rewrite itv_meetE.
       by rewrite itv_meetE.
  have K7 : (M%:E < lim (fun n => \sum_(k < n | P k) length (IkN k N)))%E.
    by rewrite (lt_le_trans MINN) // K6.
  have K8 : \forall n \near \oo, (M%:E <= \sum_(k < n | P k) length (IkN k N))%E.
    apply ereal_lt_lim => //.
    move=> a b ab.
    rewrite -(@big_mkord _ _ _ b P (fun n => length (IkN n N))).
    rewrite -(subnKC ab) /index_iota subn0 iotaD big_cat /=.
    rewrite -[in X in (_ <= X + _)%E](subn0 a) big_mkord lee_addl //.
    apply: sume_ge0 => n _; apply length_ge0.
    by rewrite /IkN -itv_meetE; exact/ITVS.itvs_ringOfSets_set_itv.
    apply: (@is_cvg_ereal_nneg_series _ (fun k => length (IkN k N)) P) => n _.
    by apply: length_ge0; rewrite /IkN -itv_meetE; exact/ITVS.itvs_ringOfSets_set_itv.
  have K5 : \forall n \near \oo, (M%:E <= (\sum_(k < n | P k) length (set_of_itv (j k)))%E)%E.
    case: K8 => m [mN].
    exists m => // p /= mp.
    have := mN _ mp.
    ...
    rewrite /IkN.
    apply le_length => //.
    by rewrite -itv_meetE; exact/ITVS.itvs_ringOfSets_set_itv.
    exact/ITVS.itvs_ringOfSets_set_itv.
...
apply/cvg_lim => //.
apply/ereal_cvgPpinfty => M M0; exact: K6.
Grab Existential Variables. all: end_near. Qed.

Lemma length_sub_sigma_additive_on_intervals (i : interval R) (j : nat -> interval R) (P : pred nat) :
  (forall k, P k -> set_of_itv (j k) != set0) ->
  set_of_itv i `<=` \bigcup_(k in P) set_of_itv (j k) ->
  (length (set_of_itv i) <= lim (fun n => \sum_(k < n | P k) length (set_of_itv (j k))))%E.
Proof.
...
rewrite le_eqVlt => /orP[/eqP|ioo]; last first.
  by apply: length_sub_sigma_additive_on_intervals_finite_case.
move=> ioo.
exact: (@length_sub_sigma_additive_on_intervals_infinite_case i j).
Qed.
...
rewrite le_eqVlt => /orP[/eqP ioo|iNoo].
  ...
  apply: (@length_sub_sigma_additive_on_intervals i j (fun x => set_of_itv (j x) != set0)) => //.
  by rewrite -bigcup_cond_neq0 ij.
exact: length_sigma_additive_on_intervals_finite_case.
Qed.

Lemma length_semi_sigma_additive_helper (S : nat -> set (itvs_ringOfSetsType R))
  (mS : forall i, measurable (S i))
  (US : measurable (\bigcup_n S n))
  (tS : trivIset setT S) (n : nat) :
  (\sum_(k < n) length (S k) <= length (\bigcup_k S k))%E.
Proof.
...
Proof.
move=> S mS tS US.
suff -> : length (\bigcup_k S k) = lim (fun n => \sum_(k < n) length (S k))%E.
  exact/(@is_cvg_sum_length _ xpredT).
apply/eqP; rewrite eq_le; apply/andP; split; last first.
  apply: ereal_lim_le; first exact: (@is_cvg_sum_length _ xpredT).
  by near=> n; exact: length_semi_sigma_additive_helper.
...
have H1 : forall i, set_of_itv i `<=` \bigcup_k (S k) ->
  (length (set_of_itv i) <= lim (fun n => \sum_(k < n) length (set_of_itv i `&` S k))%E)%E.
  move=> i iS.
  ...
  have Hj : (lim (fun n => (\sum_(k < n) (length (set_of_itv (nth_idx2 i_inter_S' k))))%E)
      ...
      apply: lee_lim.
      + apply: (@is_cvg_sum_length (fun k => set_of_itv (nth_idx2 i_inter_S' k)) xpredT) => k _.
        exact: ITVS.itvs_ringOfSets_set_itv.
      ...
      apply: sume_ge0 => I _.
      exact/length_ge0/ITVS.itvs_ringOfSets_set_itv.
  rewrite (_ : (fun n => \sum_(k < n) (\sum_(x <- i_inter_S k) length (set_of_itv x))%E)%E =
               (fun n => \sum_(k < n) (\sum_(x <- i_inter_S' k) length (set_of_itv x))%E)%E)%E; last first.
...
rewrite SI /ufint (big_nth 0%O) big_mkord.
rewrite (@length_additive (fun n => set_of_itv (nth 0%O I n))) //; last first.
  by move=> i; exact: ITVS.itvs_ringOfSets_set_itv.
rewrite (@le_trans _ _ (\sum_(i < size I) (lim (fun n => \sum_(k < n) length (set_of_itv (nth 0%O I i) `&` S k))%E))%E) //.
apply: lee_sum => i _.
  apply H1 => //.
  rewrite SI /ufint (big_nth 0%O) big_mkord => r ir.
...
rewrite (@le_trans _ _ (lim (fun n => \sum_(i < n) (\sum_(k < size I) length (set_of_itv (nth 0%O I k) `&` S i))%E)%E)) //.
  apply: (@sum_lim_sumC _ (size I) (fun i k => length (set_of_itv (nth 0%O I i) `&` S k))).
  move=> a b; apply length_ge0.
  apply: (@measurableI (itvs_ringOfSetsType R)) => //.
  exact: ITVS.itvs_ringOfSets_set_itv.
apply lee_lim.
+ apply: (@is_cvg_ereal_nneg_series _ (fun i => \sum_(k < size I) length (set_of_itv (nth 0%O I k) `&` S i))%E xpredT) => n _.
  apply: sume_ge0 => /= i _; apply: length_ge0.
  by apply: (@measurableI (itvs_ringOfSetsType R)) => //; exact: ITVS.itvs_ringOfSets_set_itv.
+ by apply/(@is_cvg_sum_length _ xpredT) => ? _; exact/mS.
+ near=> n.
  apply: lee_sum => /= k _.
  have HSk : S k = \bigcup_k0 (set_of_itv (nth 0%O I k0) `&` S k).
    rewrite -bigcup_distrl setIC; apply/esym/setIidPl.
    rewrite (_ : bigsetU _ _ = ufint I); last first.
      rewrite /ufint (big_nth 0%O) big_mkord.
      rewrite (bigcup_recl (size I)) bigcup1 ?setU0// => i _.
      by rewrite nth_default ?set_of_itvE// leq_addr.
    by rewrite -SI; apply: bigcup_sup.
  rewrite {2}HSk.
  apply: length_semi_sigma_additive_helper.
  - by move=> i; apply: measurableI => //; exact: ITVS.itvs_ringOfSets_set_itv.
  - by rewrite -HSk.
...
Definition length_measure0
    : {measure set (itvs_ringOfSetsType R) -> {ereal R}} :=
  Measure (Measure.Axioms length_set0 length_ge0 length_semi_sigma_additive).

Definition outer_measure0
    : {outer_measure set (itvs_ringOfSetsType R) -> {ereal R}} :=
  outer_measure_of_measure length_measure0.

Definition length_measure
   : {measure set (caratheodory_measurableType outer_measure0) -> {ereal R}} :=
  measure_of_outer_measure outer_measure0.
