...
Fact salgebra_is_dynkin' (T : measurableType) : dynkinP (@measurable T).
Proof.
...
case: (ltgtP i1 k1) => // [ik1 _| <-{k1} ik2].
  case: (ltgtP k1 j1) => // [kj1 _|kj1]; by [rewrite (lt_trans ik1) | rewrite -kj1 ik1].
...
Proof.
move=> [i1 i2] [j1 j2]; rewrite /le_itv /=; case/boolP : (_ == _) => ij //=.
case: (ltgtP i1 j1) => //=; rewrite ?orbT // => ij1.
rewrite eq_sym (negbTE ij) /= -neq_lt; apply/negP => /eqP ij2.
by apply: absurdb ij; rewrite ij1 ij2.
Qed.
...
Proof.
move: i j => [a b] [c d]; rewrite /lt_itv; case: (ltgtP a c) => // [ac _|].
...
- move=> <-{c} ij2; rewrite /disjoint_itv -set_of_itv_meet => /eqP /set_of_itv0P => abd x y.
  rewrite 2!itv_boundlr => /andP[ax xb] /andP[ay yd].
...
case : (ltgtP a1 b1) => // [a1b1 _|-> // a2b2].
  ...
  rewrite itv_leEmeet/= (join_l (ltW _))//.
  case/orP: xb1b2 => [xb1|b2x].
    by have := lt_trans (lt_le_trans b1c1 c1x) xb1; rewrite ltxx.
  by rewrite meet_l//= (le_trans _ xa2) // ltW.
rewrite 3!itv_boundlr => /andP[b1x xa2] /andP[c1x xc2] _; left.
by rewrite itv_leEmeet/= joinxx (meet_l (ltW _)).
Qed.
...
case: ltgtP => [ab _ _ ji ijb|//|<-{c}].
  have [db|db] /= := leP d b; last by rewrite ab.
  apply: (absurdNb ji).
  by rewrite itv_leEmeet /= (join_l (ltW _))// meet_l.
move=> ij0 bd _ abd; rewrite leNgt bd /=.
...
Proof.
move: k i j => [k1 k2] [a1 a2] [b1 b2] /=.
case: (ltgtP a1 b1) => [a1b1 ba + _| // | <-{b1}].
  have a2b2 : (a2 < b2)%O.
    rewrite ltNge; apply/negP => b2a2.
    move/negP : ba; apply.
    by rewrite itv_leEmeet /= (join_l (ltW _))// meet_l.
  move: ba a1b1; case: (ltgtP k1 a1) => [k1a1 _ a1b1 ya _| // | <-{a1} ba a1b1 ya k2a2 _].
    rewrite (lt_trans k1a1) // /itv_diff => _.
    case: ifPn => [_ |ab0]; first by rewrite k1a1.
    by rewrite leNgt a1b1 /= leNgt a2b2 /= k1a1.
  rewrite /itv_diff; case: ifPn => [_|/negPn ab0]; first by rewrite ltxx eqxx.
  rewrite leNgt a1b1 /= leNgt a2b2 /= ltxx eqxx ltNge; apply/negP => b1k2.
  move/disjoint_itvP : ya => /=.
  move/asboolP; apply/negP; rewrite -neitvNpred0.
  by rewrite /neitv /= joinxx (meet_l (ltW k2a2)) (lt_le_trans a1b1).
case: (ltgtP k1 a1) => [k1a1 ab ya a2b2 _ _| // |<-{a1} ab ya a2b2 k2a2 k2b2].
  ...
  rewrite -leNgt ge_pinfty => /eqP k1oo.
  by move: k1a1; rewrite k1oo.
rewrite /itv_diff; case: ifPn => [_|/negPn ab0]; first by rewrite ltxx eqxx.
rewrite lexx leNgt a2b2 /=; case: ifPn => //.
rewrite -leNgt ge_pinfty => /eqP k1oo.
by apply: (absurd ab0); rewrite /neitv /= k1oo join_r // set_of_itvNlt_set0.
Qed.
...
have : all (fun x => le_itv (itv_diff a b) x && (disjoint_itv (itv_diff a b) x)) (b :: t).
  ...
  rewrite (le_trans _ zx1) //=; last first.
   ...
   rewrite itv_leEmeet/= meet_l; last first.
     by rewrite (le_trans (ltW b2x)).
   rewrite join_l//.
...
Proof. by congr trivIset; rewrite funeqE => ?; rewrite setIC. Qed.
...
Proof.
exact/subset_gen_salgebra_caratheodory/gen_salgebra_self/measurable_itv.
Qed.
...
Let gssets := gen_salgebra_measurableType ssets.

Definition lebesgue_measure : {measure set gssets -> \bar R} :=
  Hahn_extension (@slength_measure R).
...
Proof.
move=> mu muslength X mX; apply: Hahn_extension_unique => //.
