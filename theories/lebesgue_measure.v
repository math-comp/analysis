...
(*                                                                            *)
(*   lim_inf == limit inferior                                                *)
(*   lim_inf == limit superior                                                *)
(*   lim_ereal_inf == limit inferior                                          *)
(*   lim_ereal_inf == limit superior                                          *)
(*                                                                            *)
(******************************************************************************)
...
Lemma cons_head_beheadE {T : eqType} (s : seq T) def : s != [::] -> head def s :: behead s = s.
Proof. by case: s. Qed.
...
rewrite {1}/le_itv => /orP[/eqP ya|ya].
  ...
  have [a0 _|a0 _] := eqVneq [set` i] set0.
...
Lemma nonempty_sorted_disj_itv h (t : seq (interval R)) :
  all neitv t ->
  sorted le_itv (h :: t) ->
...
Lemma nonempty_sorted_disj_itv_trivIset h (t : seq (interval R)) :
  all neitv t ->
  sorted le_itv (h :: t) ->
  path disjoint_itv h t ->
...
- have [jbt|btj] := ltP j (size (b :: t)).
    apply/eqP/(@nonempty_sorted_disj_itv _ (b :: t)) => //=.
    by rewrite b0.
    by rewrite lt_ab.
    by rewrite dis_ab.
    by rewrite mem_nth.
...
Lemma path_disj_itv_pinfty_eq0 (t : seq (interval R)) j :
  all neitv t ->
...
case: j => [b r|[]]; last 2 first.
  move=> _; apply: path_disj_setT_eq0.
...
Local Lemma decompose_nonempty_sorted_helper i j :
...
apply/allP => k; rewrite inE => /orP[/eqP ->|].
  exact: decompose_nonempty_sorted_helper.
by move: k; apply/allP; rewrite ih //= ne andbT.
Qed.

Local Lemma path_disj_itv_sorted_decompose_helper i j t
...
exact: path_disj_itv_sorted_decompose_helper.
Qed.

Lemma path_disj_itv_sorted_decompose_head_behead s : sorted le_itv s ->
  path disjoint_itv (head 0%O (sorted_decompose s)) (behead (sorted_decompose s)).
Proof.
move sn : (size s) => n.
elim: n s sn => [|n ih [//|i [|j t]] []].
...
exact: path_disj_itv_sorted_decompose_helper (* TODO: rename *).
Qed.
...
Lemma trivIset_sorted_decompose s :
...
rewrite size_eq0 => s0.
rewrite -(cons_head_beheadE 0%O s0); apply: nonempty_sorted_disj_itv_trivIset.
...
rewrite -(cons_head_beheadE 0%O s0).
apply: nonempty_sorted_disj_itv_trivIset.
...
Lemma le_itv_disj_bnd2_bnd1 (i j : interval R) :
  neitv i -> neitv j ->
...
(* TODO: rename *)
Lemma le_itv_disj_bnd2Noo (i j : interval R) :
  neitv i -> neitv j ->
  le_itv i j -> disjoint_itv i j ->
  i.2 != +oo%O.
Proof.
move=> i0 j0 ss ts.
have ii1 := le_itv_disj_bnd2_bnd1 i0 j0 ss ts.
apply/negP => /eqP ioo.
...
Lemma le_itv_disj_bnd2r (i j : interval R) :
  neitv i -> neitv j ->
  le_itv i j -> disjoint_itv i j ->
...
Lemma le_itv_disj_bnd1Noo (i j : interval R) :
  neitv i -> neitv j ->
  le_itv i j -> disjoint_itv i j ->
  j.1 != -oo%O.
Proof.
...
Lemma le_itv_disj_bnd1r (i j : interval R) :
  neitv i -> neitv j ->
  le_itv i j -> disjoint_itv i j ->
...
Lemma le_itv_disj_contiguous (i j : interval R) :
  neitv i -> neitv j ->
  le_itv i j -> disjoint_itv i j ->
...
have : contiguous_itv (Rhull A) (Rhull B).
  apply le_itv_disj_contiguous => //; try by rewrite -set_itv_neq0 RhullK ?inE//; exact/set0P.
  by rewrite ltW_itv.
  by rewrite disj_itv_Rhull.
...
- apply: (absurd A2B1); apply/eqP.
  apply: (@le_itv_disj_bnd2Noo _ _ (Rhull B)) => //; try by rewrite -set_itv_neq0 RhullK ?inE//; exact/set0P.
  exact/ltW_itv.
  by rewrite disj_itv_Rhull.
Qed.
...
rewrite ABE slength_additive_seq // (_ : A = [sset of a']); last first.
  rewrite /a' /decompose (cover_sorted_decompose (sort_sorted total_le_itv _)) //.
  by rewrite sset_sort_le_itv sset_filter_neitv.
...
move=> ij tj.
have := lee_pinfty (hlength ([set` i])).
rewrite le_eqVlt => /orP[/eqP ioo|iNoo]; last first.
...
rewrite (_ : (fun _ => _) = (fun n => \sum_(0 <= k < n)
  ...
  have -> : forall n, [set` i] `&` F n = \big[setU/set0]_(x <- ids n) [set` x].
    move=> n; rewrite big_map.
    have [_ _] := is_decomposition_decompose (s n).
...
rewrite (le_trans _ (@sum_nth_interval_sum_sum _ (nil_cons0 ids) slength _ _))//.
...
rewrite (@le_trans _ _ (\sum_(0 <= n < size j)
                        (\sum_(k <oo) slength ([set` nth 0%O j n] `&` F k))))%E //.
...
rewrite (@le_trans _ _ (\sum_(n <oo)
  (\sum_(0 <= k < size j) slength ([set` nth 0%O j k] `&` F n))))%E //.
...
Lemma ereal_lebesgue_measure'_ge0 X : measurable X ->
  (0 <= ereal_lebesgue_measure' X)%E.
...
(* NB : move along open_lt? *)
Lemma open_lt_lt (R : realType) (a b : R) :
  open (fun x : [topologicalType of R^o] => a < x < b).
Proof.
have [ab|ba] := ltP a b.
...
Lemma punct_eitv_setTL (R : realType) :
  [set of @EFin R] `|` [set -oo%E] = (fun b => (-oo <= b)%E) `\` [set +oo%E].
Proof.
...
- move: x => [s| _ n _|//].
  + rewrite lee_fin => rs n _.
    rewrite -subEFin lte_fin ltr_subl_addl (le_lt_trans rs)// ltr_addr.
    by rewrite invr_gt0.
...
Definition G := [set A : set \bar R | exists x, A = (fun z => x < z)].
Let T := g_measurableType G.
...
- rewrite [X in measurable X](_ : _ = (fun x => r%:E <= x) `\` [set +oo]).
...
(* TODO: move *)
Section lim_inf_sup_realType.
Variable R : realType.
Implicit Types (r : R) (u : R^o^nat).

Lemma sup_preimage (T : Type) r (f : (T -> R)^nat) n :
  (forall t, has_ubound (sdrop (f^~t) n)) ->
  (fun x => sup (sdrop (f^~x) n)) @^-1` [set x | r < x](*TODO*) =
  \bigcup_(k in [set k | n <= k]%N) (f k @^-1` [set x | r < x]).
Proof.
move=> f_ub; rewrite predeqE => t; split => /=.
  have [|/set0P h] := eqVneq (sdrop (f^~t) n) set0; last first.
    by move/(sup_gt h) => [y [[m /= nm <-{y}]]] rfmt; exists m.
  by move/absurd; apply; apply/eqP/set0P; exists (f n t), n => //=.
move=> -[k /= nk] /lt_le_trans; apply.
by apply: sup_ub => //; exists k.
Qed.

Lemma inf_preimage (T : Type) r (f : (T -> R)^nat) n :
  (forall t, has_lbound (sdrop (f^~t) n)) ->
  ((fun x => inf (sdrop (f^~x) n)) @^-1` `]-oo, r[%classic) =
...
Lemma cvg_has_ubound_sdrop u : cvg u -> forall m, has_ubound (sdrop u m).
Proof.
...
Lemma cvg_has_lbound_sdrop u : cvg u -> forall m, has_lbound (sdrop u m).
Proof.
move/cvg_seq_bounded => [M [Mr Mf_]] m.
...
Lemma nonincreasing_sup u : (forall m, has_ubound (sdrop u m)) ->
  nonincreasing_seq (fun n => sup (sdrop u n)).
Proof.
move=> f_ub m n mn; apply: le_sup.
- move=> k /= [p np] <-{k}; apply/downP; exists (u p) => //=; exists p => //.
  exact: leq_trans np.
- by exists (u n) => /=; exists n => /=.
- by split => //; exists (u m); exists m => //=.
Qed.

Lemma nondecreasing_inf u : (forall m, has_lbound (sdrop u m)) ->
...
Lemma inf_sup_sdrop u n : cvg u -> inf (sdrop u n) <= sup (sdrop u n).
Proof.
move=> cu; set A := sdrop _ _.
have [a Aa] : A !=set0 by exists (u n); rewrite /A /=; exists n => //=.
rewrite (@le_trans _ _ a) //; [apply/inf_lb|apply/sup_ub] => //.
- exact/cvg_has_lbound_sdrop.
- exact/cvg_has_ubound_sdrop.
Qed.

Lemma is_cvg_sup_sdrop u : cvg u -> cvg (fun n => sup (sdrop u n) : R^o).
Proof.
...
Lemma is_cvg_inf_sdrop u : cvg u -> cvg (fun n => inf (sdrop u n) : R^o).
Proof.
...
Definition lim_inf u := lim (fun n => inf (sdrop u n) : R^o).

Definition lim_sup u := lim (fun n => sup (sdrop u n) : R^o).

Lemma lim_supE (T : Type) (f : (T -> R)^nat) :
  (forall n t, has_ubound (sdrop (f^~t) n)) ->
  (forall n t, has_lbound (sdrop (f^~t) n)) ->
  (fun x => lim_sup (f^~ x)) =
    (fun x => inf [set sup (sdrop (f^~ x) n) | n in setT]).
Proof.
...
rewrite (@limN _ _ _ _ _ (fun n => sup (sdrop u n) : R^o)) //.
exact: is_cvg_sup_sdrop.
Qed.

Lemma lim_inf_sup u : cvg u -> lim_inf u <= lim_sup u.
Proof.
move=> cf_; apply: ler_lim; [exact: is_cvg_inf_sdrop|exact: is_cvg_sup_sdrop|].
by apply: nearW => n; apply: inf_sup_sdrop.
Qed.

Lemma cvg_inf_sup u l : u --> l -> lim_inf u = l /\ lim_sup u = l.
Proof.
move=> ul.
...
- apply/ler_addgt0Pr => e e0.
  ...
  apply: lim_le.
    apply: (@nonincreasing_is_cvg _ _ (- (M + 1))).
      exact: nonincreasing_sup.
    move=> n.
    rewrite (@le_trans _ _ (u n))//.
      by apply/lerNnormlW/Mu => //; rewrite ltr_addl.
...
(* TODO: move *)
Section lim_inf_sup_ereal.
Variable R : realType.
Implicit Types (u : (\bar R)^nat).
Local Open Scope ereal_scope.

Lemma nonincreasing_ereal_sup u :
...
Lemma nondecreasing_ereal_inf u :
...
Lemma ereal_inf_sup_sdrop u n :
  ereal_inf (sdrop u n) <= ereal_sup (sdrop u n).
Proof.
set A := sdrop _ _; have [a Aa] : A !=set0.
  by exists (u n); rewrite /A /=; exists n => //=.
by rewrite (@le_trans _ _ a) //; [exact/ereal_inf_lb|exact/ereal_sup_ub].
Grab Existential Variables. all: end_near. Qed.

Lemma is_cvg_ereal_sup_sdrop u : cvg (fun n => ereal_sup (sdrop u n)).
Proof.
apply: ereal_nonincreasing_is_cvg => n m nm.
apply le_ereal_sup => _ [p /= np <-].
by exists p => //=; rewrite (leq_trans _ np).
Qed.

Lemma ereal_sup_preimage (T : Type) (a : \bar R) (f : (T -> \bar R)^nat) n :
...
Lemma ereal_inf_preimage (T : Type) (a : \bar R) (f : (T -> \bar R)^nat) n :
...
Definition lim_ereal_inf u := lim (fun n => ereal_inf (sdrop u n)).

Definition lim_ereal_sup u := lim (fun n => ereal_sup (sdrop u n)).

Lemma lim_ereal_infN u : lim_ereal_inf (-%E \o u) = - lim_ereal_sup u.
Proof.
rewrite /lim_ereal_inf (_ : (fun n => _) = (fun n => - ereal_sup (sdrop u n))).
...
by exists (- u m); [exists m|rewrite oppeK].
Qed.
...
apply: (@measurability _ _ _ (@RbarGenOpenRays.G R));
  [ exact/esym/RbarGenOpenRays.measurableEoray | ].
move=> /= A [_ [x ->]] <-; move: x => [x| |].
- rewrite [X in measurable X](_ : _ = `]x, +oo[%classic).
...
rewrite preimage_o_inftyE [X in measurable X](_ : _ = \bigcup_(r : rat)
  ...
  - rewrite -preimage_o_inftyE -[X in measurable X]setIT.
...
rewrite (_ :  (fun _ => _) =
    (fun x => inf [set sup (sdrop (f^~ x) n0) | n0 in [set n0 | (n0 >= O)%N]])).
  ...
  by exists m => //=.
by rewrite funeqE => n; congr (inf [set _ | _ in _]); rewrite predeqE.
Qed.
...
apply: (@measurability _ _ _ (@RbarGenOpenRays.G R));
  [ exact/esym/RbarGenOpenRays.measurableEoray | ].
move=> /= _ [_ [x ->] <-]; move: x => [x| |].
- rewrite [X in _ @^-1` X](punct_eitv_o_infty x) preimage_setU.
...
rewrite [X in measurable X](_ : _ =
    (fun x : T => (g x)%:E) @^-1` [set x%:E | x in A] `&` D).
...
Lemma measurable_fun_cvg T (R : realType) (f_ : (T -> R)^nat) (f : T -> R^o) :
...
- by apply: cvg_has_ubound_sdrop; apply/cvg_ex; eexists; exact: (f_f t).
- by apply: cvg_has_lbound_sdrop; apply/cvg_ex; eexists; exact: (f_f t).
Qed.
