...
Local Notation gT := (g_measurable G).

Lemma g_salgebra_on_setC_setT (A : set T) : s<< G >> A -> s<< G >> (~` A).
Proof. by move=> sGA; rewrite -setTD; exact: g_salgebra_setC. Qed.

Canonical g_measurable_eqType := EqType gT (Equality.class T).
Canonical g_measurable_choiceType := ChoiceType gT (Choice.class T).
Canonical g_measurable_pointedType := PointedType gT (Pointed.class T).
HB.instance Definition g_salgebra_mixin :=
  @isMeasurable.Build gT (Pointed.class T) (g_salgebra setT G)
...
(* TODO: rename? *)
Lemma transfer (aT : Type) (rT : pointedType)
...
Definition ereal_isMeasurable : isMeasurable (\bar R) :=
  isMeasurable.Build _ (Pointed.class [pointedType of \bar R])
...
Canonical measurableTypeR_eqType := [eqType of measurableTypeR].
Canonical measurableTypeR_choiceType := [choiceType of measurableTypeR].
Canonical measurableTypeR_pointedType := [pointedType of measurableTypeR].
...
HB.instance Definition prod_salgebra_mixin :=
  @isMeasurable.Build (T1 * T2)%type (Pointed.class [pointedType of T1 * T2])
