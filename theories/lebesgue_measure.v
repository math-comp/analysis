...
(*                            gen_class G dynkin                              *)
(*     is_monotone_class C == C is a monotone class                           *)
...
Lemma has_ub_lbN (R : numDomainType) (E : set R) :
  has_ubound E <-> has_lbound [set - x | x in E]%R.
Proof.
split=> [h|/has_lb_ubN].
- apply/has_lb_ubN.
...
Lemma bounded_funD (R : realType) (a b : R^o^nat) :
  bounded_fun a -> bounded_fun b -> bounded_fun (a \+ b)%R.
Proof.
move=> [M [Mreal Ma]] [N [Nreal Nb]]; exists (`|M| * 2 + `|N| * 2)%R.
...
Lemma bounded_fun_has_ubound (R : realFieldType) (a : R^o^nat) : bounded_fun a ->
  has_ubound [set of a].
Proof.
move=> [M [Mreal]]/(_ (`|M| + 1)%R).
rewrite (le_lt_trans (ler_norm _)) ?ltr_addl// => /(_ erefl) aM.
by exists (`|M| + 1)%R => _ [n _ <-]; rewrite (le_trans (ler_norm _))// aM.
Qed.

Lemma bounded_funN (R : realFieldType) (a : R^o^nat) : bounded_fun a ->
  bounded_fun (- a)%R.
Proof.
move=> [M [Mreal aM]]; rewrite /bounded_fun /bounded_near; near=> n.
under eq_fun do rewrite normrN; apply aM.
near: n; exists (`|M| + 1)%R; rewrite realD ?(normr_real, real1)//; split => // r.
by apply: le_lt_trans; rewrite (le_trans (ler_norm _)) ?ler_addl.
Grab Existential Variables. all: end_near. Qed.

Lemma bounded_fun_has_lbound (R : realFieldType) (a : R^o^nat) : bounded_fun a ->
  has_lbound [set of a].
Proof.
...
Lemma nondecreasing_cvg_new (u_ : R^o ^nat) :
    nondecreasing_seq u_ -> has_ubound [set of u_] ->
  u_ --> sup (u_ @` setT) .
Proof.
move=> u_nd u_M; set S := _ @` _; set M0 := sup S.
have supS : has_sup S.
  by split; first by exists (u_ 0%N), 0%N.
apply: cvg_distW => _/posnumP[e]; rewrite near_map.
have [p /andP[M0u_p u_pM0]] : (exists p, M0 - e%:num <= u_ p <= M0)%R.
...
Lemma nondecreasing_is_cvg_new (u_ : R ^nat) :
  nondecreasing_seq u_ -> has_ubound [set of u_] -> cvg u_.
Proof. by move=> u_incr u_bnd; apply: cvgP; apply: nondecreasing_cvg_new. Qed.

Lemma nonincreasing_cvg_new (u_ : R^o ^nat) :
    nonincreasing_seq u_ -> has_lbound [set of u_] ->
  u_ --> inf (u_ @` setT).
Proof.
rewrite -nondecreasing_opp => /(@nondecreasing_cvg_new _) u_sup mu_.
...
Lemma nonincreasing_is_cvg_new (u_ : R ^nat) :
...
Definition setD_closed T (D : set (set T)) :=
  forall A B, B `<=` A -> D A -> D B -> D (A `\` B).
Definition ndseq_closed (T : Type) (D : set (set T)) :=
  forall F, nondecreasing_seq F -> (forall i, D (F i)) -> D (\bigcup_i (F i)).
...
Lemma are_measurable_setsP T (C : set (set T)) : are_measurable_sets setT C <->
  [/\ C setT, setC_closed C, ndseq_closed C & setI_closed C].
Proof.
split => [[D0 DD DU]|[DT DC DU DI]]; split.
- by rewrite -setC0 -setTD; exact: DD.
- by move=> A /DD; rewrite setTD.
- by move=> F ndF DF; exact: DU.
- move=> A B DA DB; rewrite -(setCK (A `&` B)) -setTD; apply: (DD).
  rewrite setCI -bigcup2E.
  by apply: DU => -[|[|[]]] //=; rewrite /bigcup2 /= -setTD; exact: DD.
- by rewrite -setCT; exact: DC.
- by move=> A DA; rewrite setTD; exact: DC.
- move=> F DF.
  rewrite [X in C X](_ : _ = \bigcup_i \big[setU/set0]_(j < i.+1) F j).
    ...
    have CU : setU_closed C.
      move=> A B DA DB.
      by rewrite -(setCK (_ `|` _)) setCU; apply: (DC); apply: DI; exact: DC.
...
Proof.
split=> //; [exact: measurable0| |exact: measurable_bigcup].
by move=> A; apply: measurableD => //; exact: measurableT.
Qed.
...
Definition dynkin G := [/\ G setT, setC_closed G &
...
Lemma dynkin_setI_bigI G (F : (set T)^nat) : dynkin G -> setI_closed G ->
...
Definition is_monotone_class T (D : set (set T)) :=
  [/\ D setT, setD_closed D & ndseq_closed D].

Section smallest_monotone_class_salgebra.
Variables (T : Type) (C : set (set T)).
Hypotheses (CI : setI_closed C) (CT : C setT).
Variable D : set (set T).
Hypotheses (monoD : is_monotone_class D) (CD : C `<=` D).

Lemma smallest_monotone_class_salgebra :
  (forall E, is_monotone_class E -> C `<=` E -> D `<=` E) -> D = s<< C >>.
Proof.
move=> smallestD; rewrite eqEsubset; split.
  case: monoD => _ DD ndD; apply: smallestD => //; last exact: g_salgebra_self.
  split; first exact: g_salgebra_self.
  - by move=> A B BA sCA sCB; apply: (@measurableD (g_measurableType _)).
  - by move=> F ndF mF; exact: (@measurable_bigcup (g_measurableType _)).
suff: forall A B, D A -> D B -> D (A `&` B).
  move=> DI; apply: g_salgebra_smallest => //.
  apply/are_measurable_setsP; split=> //; first exact: CD.
  - move=> A DA; rewrite -setTD.
    by case: monoD => _ + _; apply => //; exact: CD.
  - by move=> F tF DF; case: monoD => _ _; exact.
pose D_ A := [set X : set T | D X /\ D (X `&` A)].
have D_setD_closed A : setD_closed (D_ A).
  move=> X Y XY [DX DXA] [DY DYA]; case: monoD => _ DD _; split; first exact: DD.
  rewrite (_ : _ `&` _ = (X `&` A) `\` (Y `&` A)).
    by apply DD => //; apply: setSI.
  rewrite predeqE => x; split=> [[[Xx Yx] Ax]|]; first by split => // -[].
  by move=> [[Xx Ax] YAx]; split => //; split => //; apply: contra_not YAx => Yx.
have D_ndseq_closed A : ndseq_closed (D_ A).
  move=> F ndF D_F; split.
    by case: monoD => _ _; apply => // => n; have [] := D_F n.
  rewrite setI_bigcupl; case: monoD => _ _; apply => //.
    by move=> m n mn; apply/subsetPset; apply: setSI; apply/subsetPset/ndF.
  by move=> n; have [] := D_F n.
have CCD_ X : C X -> C `<=` D_ X.
  by move=> CX A CA; split; [exact: CD|apply: CD; exact: CI].
have CDD_ X : C X -> D `<=` D_ X.
  move=> CX; apply: smallestD; last exact: CCD_.
  by split; [exact: CCD_|exact: D_setD_closed|exact: D_ndseq_closed].
have DCD_ X : D X -> C `<=` D_ X.
  by move=> DX A CA; split; [exact: CD|rewrite setIC; apply CDD_].
have DDD_ X : D X -> D `<=` D_ X.
  by move=> DX; apply: smallestD; [split => //; exact: DCD_|exact: DCD_].
by move=> A B DA DB; apply DDD_.
Qed.

End smallest_monotone_class_salgebra.
...
Lemma monotone_class_subset : s<< C >> `<=` D.
Proof.
set M0 := gen_class C (@is_monotone_class T).
rewrite -(@smallest_monotone_class_salgebra _ C CI CT M0).
- exact: gen_class_smallest.
- split => [E /= [[]]//| |].
  + move=> A B BA M0A M0B E [[ET ED ndE] CE].
    by apply ED => //; [exact: M0A|exact: M0B].
  + by move=> F ndF M0F E [[ET ED ndE] CE]; apply ndE => // i; apply: M0F.
- exact: subset_gen_class.
- by move=> E monoE CE; apply: gen_class_smallest.
Qed.
...
have mDE n : measurable = D n.
  ...
  by rewrite TG -setI_closed_gdynkin_salgebra//; exact: gen_class_smallest.
...
apply: g_salgebra_smallest; last first.
  split; [exact: measurable0 |
    by move=> X mX; rewrite setTD; exact: measurableC |
...
Definition neitv {d : unit} {T : porderType d} (i : interval T) :=
  [set` i] != set0.

Lemma set_itvNlt_set0 {d : unit} {T : porderType d} (i : interval T) :
  neitv i -> (i.1 < i.2)%O.
Proof.
...
Lemma neitvE (R : numFieldType) (i : interval R) : neitv i = (i.1 < i.2)%O.
Proof.
apply/idP/idP; first exact: set_itvNlt_set0.
...
Proof. by apply/eqP/negPn/negP => /set_itvNlt_set0. Qed.

Lemma set_itv_bnd_ninfty a : [set` Interval a -oo%O] = set0.
Proof. by apply/eqP/negPn/negP => /set_itvNlt_set0 /=; case: a => [[]a|[]]. Qed.
...
Lemma neitvPpred0 i : reflect (i =i pred0) (~~ neitv i).
Proof.
apply: (iffP idP) => [i0 r|i0].
  by apply/idP/idP; apply: contra i0 => ri; apply/set0P; exists r.
rewrite negbK; apply/eqP; rewrite predeqE => r; split => //=.
by rewrite i0.
Qed.
...
Lemma eitv_set0 i : ~~ neitv i -> [set` i] = set0.
Proof. by move=> /neitvP i0; apply/eqP/negPn/neitvP. Qed.

Lemma pred0_hasNsup i : i =i pred0 -> ~ has_sup [set` i].
Proof.
case: i => a b /= ab0; rewrite /has_sup not_andP; left.
by apply/set0P => /neitvPpred0.
Qed.
...
Proof.
have [i0 iT|/neitvPpred0 i0 /(congr1 (@Rhull _))] := asboolP (i =i pred0).
  exfalso; suff: 0 \in i by rewrite i0.
  by move: iT; rewrite predeqE => /(_ 0) /= [_]; apply.
...
rewrite boo => /sne.
by rewrite /neitv /= set_itv_infty_set0 => /negP.
Qed.
...
rewrite sset_cons ih.
rewrite negbK in h0.
by rewrite (eqP h0) set0U.
Qed.
...
- move=> /disjoint_neitv/neitvPpred0 ij0 x y.
...
- move=> /= ?; rewrite /disjoint_itv -set_itv_meet => /negPn/neitvPpred0 abd.
...
by rewrite neitvE /= k1oo join_r // set_itvNlt_set0.
Qed.
...
have [/size0nil -> |] := eqVneq (size (sorted_decompose s)) 0%N.
  apply/trivIsetP => /= i j _ _ ij.
  by rewrite nth_nil eitv_set0 ?set0I// neitvE.
rewrite size_eq0 => s0.
...
- suff: path disjoint_itv 0%O
  ...
  apply/eqP.
  by rewrite eitv_set0 ?set0I// neitvE.
Qed.
...
Proof.
have [/Rhull_pred0 ->|/neitvPpred0 RhullN0] := asboolP (Rhull A =i pred0).
  by rewrite eitv_set0// Rhull0 neitvE/= lte_bnd ltxx.
by rewrite set_itvK.
Qed.
...
rewrite le_eqVlt => /orP[|/lt_ereal_bnd i12]; last first.
  by rewrite eitv_set0 ?hlength0// neitvE -leNgt ltW.
case: i => -[ba a|[|]] [bb b|[|]] //=.
- rewrite /= => /eqP[->{b}]; move: ba bb => -[] []; try
    by rewrite eitv_set0 ?hlength0// neitvE lte_bnd -leNgt.
...
have : m \in [sset of itv_cplt [:: i; j]].
  ...
  rewrite /m -/(miditv (Interval (BSide b x) (BSide c y))).
  by rewrite mem_miditv // set_itv_neq0.
...
Proof. by move=> i0; rewrite eitv_set0 ?hlength0//; apply/neitvPpred0. Qed.
...
- suff: exists b r, (Rhull A).2 = BSide b r by move=> [b [r Abr]]; rewrite -A2B1 Abr.
...
by move=> i /x0/neitvPpred0/eitv_set0 => ->; rewrite hlength0.
Qed.
...
by rewrite /= addeC big1 ?add0e // => k /eitv_set0 ->; rewrite slength0.
Qed.
...
have ia_b_ : [set` i] `<=` \bigcup_(k in P) `] (a'_ k) , (b'_ k) [%classic.
  ...
  exists k => //=; rewrite in_itv /=; apply/andP; split.
    ...
    rewrite -{2}(fineK jk1) // -addEFin lte_fin.
...
have : b'%:E - a'%:E <= \sum_(k <oo | P k) slength [set` j k] + (e%:num / 2)%:E.
  ...
  have cvggeo : (fun n => \sum_(i < n) (e%:num / (2 ^ i.+2)%:R)%:E) --> (e%:num / 2)%:E.
    rewrite (_ : (fun n => _) = EFin \o series (fun k => e%:num / (2 ^ (k + 2))%:R)); last first.
...
- move=> n; have := mF n.
  ...
  case: mX'; [exact:measurable0|exact: emeasurable_set1
...
- move=> F; rewrite /preimage_class /= => mF.
  ...
  by rewrite preimage_bigcup setI_bigcupl; apply eq_bigcupr => i _; exact: (mF' i).2.
Qed.

(* sigma-algebra generated by f *)
Definition sigma_fun (aT rT : Type) (D : set aT) (f : aT -> rT)
    (G : set (set aT)) : set (set rT) :=
  [set B : set rT | G (f @^-1` B `&` D)].

Lemma are_measurable_sets_sigma_fun (aT rT : Type) (D : set aT) (f : aT -> rT)
    (G : set (set aT)) :
  are_measurable_sets D G -> are_measurable_sets setT (sigma_fun D f G).
Proof.
move=> [G0 GC GU]; split; rewrite /sigma_fun.
...
have G'sfun : G' `<=` sigma_fun D f I.
  by move=> A' /G'pre[B G'B h]; apply: g_salgebra_self; exists B.
have sG'sfun : s<< G' >> `<=` sigma_fun D f I.
  apply: g_salgebra_smallest => //; apply: are_measurable_sets_sigma_fun.
  exact: are_measurable_sets_g_salgebra.
...
apply: g_salgebra_smallest => //; split; first exact: measurable0.
...
Proof.
move=> u_ub u_lb.
apply: nonincreasing_cvg_new; first exact: nonincreasing_sups.
...
move=> ba bb; have ab k : infs u k + infs v k <= infs (u \+ v) k.
  ...
  by move=> M [n /= kn <-]; apply: ler_add; apply: inf_lb; [
    ...
    exact/has_lbound_sdrop/bounded_fun_has_lbound; exact | exists n ].
have cu : cvg (infs u).
  apply: nondecreasing_is_cvg_new; last exact: bounded_fun_has_ubound_infs.
...
rewrite -(@limD _ [normedModType R of R^o] _ _ _ _ _ cu cv); apply: ler_lim.
- exact: (@is_cvgD _ [normedModType R of R^o] _ _ _ _ _ cu cv).
- apply: nondecreasing_is_cvg_new; last first.
...
Proof. by apply: nondecreasing_ereal_cvg => //; exact: nondecreasing_einfs. Qed.
...
have -> : esups u n = (EFin \o sups (fine \o u)) n.
  rewrite /= -ereal_sup_EFin; last 2 first.
    - apply/has_ubound_sdrop/bounded_fun_has_ubound/cvg_seq_bounded/cvg_ex.
      by eexists; exact ul.
...
- rewrite [X in measurable X](_ : _ = set0) ?predeqE; first exact: measurable0.
...
- rewrite [X in _ @^-1` X](punct_eitv_bnd_pinfty _ x) preimage_setU setIUl.
  ...
  exists (normr @^-1` `]x, +oo[%classic).
    ...
    by apply: measurable_fun_normr; [exact: measurableT|exact: measurable_itv].
...
Lemma EFin_measurable_fun (T : measurableType) D (g : T -> R) :
...
- move=> A mA.
  ...
  have [/eqP ->|apoo] := boolP (a == +oo%E).
    rewrite (_ : `]+oo%E, +oo[%classic = set0).
      by rewrite preimage_set0 set0I; exact: measurable0.
...
have -> : A `*` B = (A `*` setT) `&` (setT `*` B) :> set (T1 * T2).
  by rewrite -{1}(setIT A) -{1}(setTI B) setIM.
...
rewrite eqEsubset; split.
  ...
  - have /subset_trans : preimage_class setT fst M1 `<=` M1xM2.
      by move=> _ [X MX <-]; exists X=> //; exists setT=> //; exact: measurableT.
    by apply; exact: g_salgebra_self.
  - have /subset_trans : preimage_class setT snd M2 `<=` M1xM2.
      move=> _ [Y MY <-]; exists setT; first exact: measurableT.
