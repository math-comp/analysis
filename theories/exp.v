...
(*                                                                            *)
(* `expeR x`                                                                  *)
(*     ~ extended real number-valued exponential function                     *)
(*                                                                            *)
(* `ln x`                                                                     *)
(*     ~ the natural logarithm, in `ring_scope`                               *)
(*                                                                            *)
(* `lne x`                                                                    *)
(*     ~ the natural logarithm, in `ereal_scope`                              *)
(*                                                                            *)
(* ``s `^ r``                                                                 *)
(*     ~ power function, in `ring_scope` (assumes `s >= 0`)                   *)
(*                                                                            *)
(* ``e `^ r``                                                                 *)
(*     ~ power function, in `ereal_scope` (assumes `e >= 0`)                  *)
(*                                                                            *)
(* `riemannR a`                                                               *)
(*     ~ sequence ``n |-> 1 / (n.+1) `^ a`` where `a` has a type of type      *)
(*       `realType`                                                           *)
(*                                                                            *)
(* ``e `^?(r +? s)``                                                          *)
(*     ~ validity condition for the distributivity of the power of the        *)
(*       addition, in `ereal_scope`                                           *)
(*                                                                            *)
(******************************************************************************)
...
rewrite (_ : (fun _ => _) = (fun i => `|f i| * `|z| ^+ i)).
...
rewrite sub0r normrN ger0_norm; first exact: expR_ge0.
...
Proof.
apply/cvgrNyPle => y; near=> x; rewrite -ler_expR lnK; first by rewrite posrE.
...
rewrite derive1E.
rewrite (@near_eq_derive _ _ _ _ (fun x => expR (a * ln x)))//.
