...
rewrite inE big_cons /Order.max/= => /orP[/eqP <-|/IHl {}IHl]; case: ifP => [/ltW|]//.
move=> /negP/negP.
...
HB.instance Definition _ := Num.Zmodule_isNormed.Build R oo_space ler_oo_normD oo_norm0_eq0 oo_normMn oo_normN.

HB.instance Definition _ := PseudoMetric.copy oo_space (pseudometric oo_space).

HB.instance Definition _ := NormedZmod_PseudoMetric_eq.Build R oo_space erefl.

HB.instance Definition _ := PseudoMetricNormedZmod_Lmodule_isNormedModule.Build R oo_space oo_normZ.
...
pose f (X : {ptws 'I_(\dim V') -> R^o}) : Voo := \sum_(i < \dim V') (X i) *: (vbasis V')`_i.
have -> : [set x : Voo | `|x| <= 1] = f @` [set X | forall i, `[-1, 1]%classic (X i)].
...
Lemma equivalence_norms (N : V -> R) :
  N 0 = 0 -> (forall x, 0 <= N x) -> (forall x, N x = 0 -> x = 0) -> (forall x y, N (x + y) <= N x + N y) ->
...
move=> /(@continuous_compact _ _ (@GRing.inv R)) -/(_ _)/wrap[].
  move=> /= x.
  rewrite /continuous_at.
  apply: (@continuous_in_subspaceT _ _ [set N x | x in [set x : Voo | `|x| = 1]] (@GRing.inv R)).
  move=> r /set_mem/=[] y y1 <-.
...
rewrite (@eq_cvg _ _ _ _ (fun y => \sum_(i < \dim V') coord B i y *: f B`_i)); last first.
...
have := @equivalence_norms R V (@normr R V) (@normr0 _ _) (@normr_ge0 _ _) (@normr0_eq0 _ _) (@ler_normD _ _) (@normrZ _ _).
move=> [] M [] M0 MP.
exists (M * r) => x.
