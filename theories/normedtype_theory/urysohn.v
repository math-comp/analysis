...
(*                                                                            *)
(* `edist`                                                                    *)
(*     ~ the extended distance function for a pseudometric `X`, from          *)
(*       `X * X -> \bar R`                                                    *)
(*                                                                            *)
(* `edist_inf A`                                                              *)
(*     ~ the infimum of distances to the set `A`                              *)
(*                                                                            *)
(* `uniform_separator A B`                                                    *)
(*     ~ there is a suitable uniform space and entourage separating `A`       *)
(*       and `B`                                                              *)
(*                                                                            *)
(* `Urysohn A B`                                                              *)
(*     ~ a continuous function `T -> [0,1]` which separates `A` and `B`       *)
(*       when `uniform_separator A B`                                         *)
(*                                                                            *)
(* `completely_regular_space`                                                 *)
(*     ~ a space where points and closed sets can be separated by a           *)
(*       function into `R`                                                    *)
(*                                                                            *)
(* `completely_regular_uniformity`                                            *)
(*     ~ equips a `completely_regular_space` with the uniformity induced      *)
(*       by continuous functions into the reals note this uniformity is       *)
(*       always the only choice, so its placed in a module                    *)
(*                                                                            *)
(******************************************************************************)
