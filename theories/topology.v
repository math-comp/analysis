...
Canonical pfilter_filter_on T (F : pfilter_on T) := Eval hnf in 
...
Ltac near_skip :=
  match goal with |- locked_with near_key (forall _, @FilterElt.t _ _ ?P _ -> _) =>
    tryif is_evar P then fail "nothing to skip" else apply: near_fork end.
...
Canonical pfilter_on_eqType T := Eval hnf in EqType (pfilter_on T) gen_eqMixin.
Canonical pfilter_on_choiceType T := Eval hnf in 
...
Canonical pointfilter_filter T (x : T) := Eval hnf in 
  Filter (pointfilter x) (pointfilter_is_filter x).

Lemma pointfilterN0 T (x : T) :  ~ [set A : set T | A x] set0.
Proof. by []. Qed.
...
Definition nbhs_from {I T : Type} (D : set I) (B : I -> set T) : set (set T) :=
...
Definition cvg_to {T : Type} {fT gT : nbhsType T} (F : fT) (G : gT) := nbhs G `<=` nbhs F.

Notation "F --> G" := (cvg_to F G)
  (at level 70, format "F  -->  G") : classical_set_scope.

Lemma cvg_refl {T} {fT : nbhsType T} (F : fT) : F --> F.
Proof. exact. Qed.

Lemma cvg_trans  {T : Type} 
  {F G H : {nbhs T}} :
(* {fT gT hT : nbhsType T} (F : fT) (G : gT) (H : hT) : *)
  (F --> G) -> (G --> H) -> (F --> H).
Proof.  by move=> FG GH P /GH /FG. Qed.

Definition type_of_nbhs {T} {fT : nbhsType T} (F : fT) := T.
Definition lim_in {U : Type} (fU1 fU2 : nbhsType U) (F : fU1) (phU2 : phant fU2) : fU2 :=
  [get l : fU2 | F --> l].
Notation "[ 'lim' F 'in' T ]" := (@lim_in _ _ _ F (Phant T))
  (format "[ 'lim'  F  'in'  T ]") : classical_set_scope.
Notation lim F := [lim F in (type_of_nbhs F)].
Notation "[ 'cvg' F 'in' T ]" := (F --> [lim F in T])
  (format "[ 'cvg'  F  'in'  T ]") : classical_set_scope.
Notation cvg F := [cvg F in (type_of_nbhs F)].

Section is_filteredTheory.

Lemma cvg_prod {T} {fT1 fT2 fT3 fT4 : nbhsType T} (x : fT1) (y : fT2) (l : fT3) (k : fT4) :
  x --> l -> y --> k -> (x, y) --> (l, k).
Proof.
...
Lemma cvg_ex {U : Type} (fU1 fU2 : filterType U) (F : fU1) :
  [cvg F in fU2] <-> (exists l : fU2, F --> l).
Proof. by split=> [cvg|/getPex//]; exists [lim F in _]. Qed.

Lemma cvgP {U : Type} (fU1 fU2 : filterType U) (F : fU1) (l : fU2) :
   F --> l -> [cvg F in fU2].
Proof. by move=> Fl; apply/cvg_ex; exists l. Qed.

Lemma dvgP {U : Type} (fU1 fU2 : filterType U) (F : fU1) :
  ~ [cvg F in fU2] -> [lim F in fU2] = point.
...
Lemma near2_curry {U V} {fU : nbhsType U} {fV : nbhsType V}
  (F : fU) (G : fV) (P : U -> set V) :
...
Definition Build_is_pfilter {T : Type} (F : set (set T))
  ...
  (filter_filter : is_filter (nbhs F)) :=
  Build_is_pfilter' (filter_filter) (filter_not_empty_ex near_ex).
...
Lemma near {T} {fT : nbhsType T} {F : fT} P (FP : nbhs F P) (x : T)
  (Px : prop_of (FilterElt FP) x) : P x.
Proof. by move: Px; rewrite prop_ofE. Qed.
Arguments near {T fT F P} FP x Px.
...
Lemma near_all {x : fT} {P : set T} : (forall x, P x) -> \near x, P x.
...
Lemma nearS2 {F} :
  ...
  nbhs F P -> nbhs F Q -> nbhs F R.
Proof. by move=> ? ? ? ?; apply/near_app2/near_all. Qed.

Lemma nearS3 {F} :
  forall P Q R S : set T, (forall x, P x -> Q x -> R x -> S x) ->
  nbhs F P -> nbhs F Q -> nbhs F R -> nbhs F S.
Proof. by move=> ? ? ? ? ?; apply/near_app3/near_all. Qed.

End NearTheorems.
Arguments near_all {T fT x P}.
...
Lemma in_nbhs_from (D : set I) (B : I -> set T) (i : I) :
   D i -> nbhs_from D B (B i).
Proof. by exists i. Qed.

Lemma nbhs_fromP (D : set I) (B : I -> set T) (F : fT) :
  F --> nbhs_from D B <-> forall i, D i -> nbhs F (B i).
Proof.
split; first by move=> FB i ?; apply/FB/in_nbhs_from.
by move=> FB P [i Di BjP]; apply: (nearS BjP); apply: FB.
Qed.

Lemma nbhs_fromTP (B : I -> set T) (F : fT) :
  F --> nbhs_from setT B <-> forall i, nbhs F (B i).
Proof. by rewrite nbhs_fromP; split=> [P i|P i _]; apply: P. Qed.

Lemma nbhs_from_proper (D : set I) (B : I -> set T) :
  is_filter (nbhs (nbhs_from D B)) ->
  (forall i, D i -> B i !=set0) ->
  is_pfilter (nbhs_from D B).
Proof.
...
Structure call_of (condition : unit) (result : bool) := Call {callee : U -> V}.
Definition maybeId (f : U -> V) := tt.
Definition call f := Call (maybeId f) false f.

Structure test_of (result : bool) := Test {condition :> unit}.
Definition test_negative := Test false tt.

Structure type (phUV : phant (U -> V)) :=
  Check {result : bool; test : test_of result; frame : call_of test result}.
Definition check result test frame := @Check (Phant (U -> V)) result test frame.
End Ctx.
Definition test_id U := Test true (maybeId (@id U)).
...
Canonical check.
Notation nonIdFun T := (@type _ _ (Phant T)).
Coercion callee : call_of >-> Funclass.
Coercion frame : type >-> call_of.
Notation notId f := (@check false test_negative (@call _ _ f)).
End Exports.

End NonIdFun.
Export NonIdFun.Exports.

Definition filtermap {T U : Type} (f : T -> U) (F : set (set T)) :=
  [set P | F (f @^-1` P)].
Canonical filtermap_nbhs T (fT : nbhsType T) U (f : T -> U) (F : fT) := Eval hnf in 
  Nbhs (filtermap f (nbhs F)).

Definition mkfun {T T'} (f : forall x : T, T') := fun x => f x.
Arguments mkfun _ _ _ /.

Notation "f @ F" := (filtermap f (in_nbhs (nbhs F)))
  (at level 60, format "f  @  F") : classical_set_scope.
Notation "E @[ x --> F ]" := (filtermap (mkfun (fun x => E)) (in_nbhs (nbhs F)))
  (at level 60, x ident, format "E  @[ x  -->  F ]") : classical_set_scope.

Lemma near_map {U V : Type} {fU : nbhsType U} (f : nonIdFun (U -> V))
  (F : fU) (P : set V) : nbhs (f @ F) P = \for x \near F, P (f x).
Proof. by []. Qed.

Lemma filtermap_is_filter T (fT : filterType T) U (f : T -> U) (F : fT) : is_filter (nbhs (f @ F)).
Proof.
constructor => [|P Q|P Q PQ]; rewrite ?near_map ?filter_ofE //=.
- exact: nearT.
- exact: nearI.
- by apply: nearS=> ?/PQ.
Qed.

Canonical filtermap_filter T (fT : filterType T) U (f : T -> U) (F : fT) := Eval hnf in 
  Filter (filtermap f (nbhs F)) (filtermap_is_filter f F).

Lemma filtermap_is_not_empty {T : Type} (fT : pfilterType T) {U : Type}
   (f : T -> U) (F : fT) : ~ \for _ \near f @ F, False.
Proof. by rewrite near_map; apply: filter_not_empty. Qed.
Arguments filtermap_is_not_empty {T fT U} f F.

Canonical filtermap_pfilter {T : pointedType} U (f : T -> U) (F : {pfilter T}) := Eval hnf in 
  PFilter (filtermap f (nbhs F)) (filtermap_is_not_empty f F).

Fact filtermapi_key : unit. Proof. exact: tt. Qed.
Definition filtermapi  {T U : Type} (f : T -> set U) (F : set (set T)) : set (set U) :=
  [set P | \for x \near F, exists y, f x y /\ P y].
Canonical filtermapi_nbhs {T U} {fT : nbhsType T} (f : T -> U -> Prop) (F : fT) : nbhs_on U := Eval hnf in 
  Nbhs (filtermapi f (nbhs F)).

Notation "E `@[ x --> y ]" := (filtermapi (fun x => E) (in_nbhs (nbhs y)))
...
Lemma near_mapi {U} {fU : nbhsType U} {V : Type} (f : U -> set V)
  (F : fU) (P : set V) :
  nbhs (f `@ F) P = \for x \near F, exists y, f x y /\ P y.
Proof. by []. Qed.

Definition nearE := (@near_map, @near_mapi).

Lemma filtermapi_is_filter {T U} {fT : filterType T} (f : T -> set U) (F : fT) :
  {near F, is_totalfun f} -> is_filter (nbhs (f `@ F)).
Proof.
move=> f_totalfun; apply: Build_is_filter. (* bug *)
- by rewrite nearE; apply: nearS f_totalfun => x [[y Hy] H]; exists y.
- move=> P Q; rewrite !nearE => FP FQ; near=> x.
...
- move=> P Q subPQ; rewrite !nearE => FP; near=> x.
...
Lemma nbhseqE T (F G : {nbhs T}) :
  (nbhs F = nbhs G :> set _) = (forall P, nbhs F P <-> nbhs G P).
Proof. by rewrite predeqE propeqE; split => []. Qed.

Lemma nbhs_on_eqE {T} (F G : {nbhs T}) :
  (F = G) = (forall P, nbhs F P <-> nbhs G P).
Proof.
rewrite -nbhseqE; apply/propext => //=; split => [->//|].
by case: F G => [F] [G]/= eqFG; rewrite [F]eqFG.
Qed.
...
Proof.
rewrite predeqE; split; rewrite !nearE => Fx /=; near=> t.
...
Lemma cvg_id (y : set (set T)) : id @ y --> y.
Proof. by []. Qed.

Lemma cvg_app  (F G : set (set U)) (f : U -> V) :
  F --> G -> f @ F --> f @ G.
Proof. by move=> FG P /=; exact: FG. Qed.

Lemma cvgi_app (F G : set (set U)) (f : U -> set V) :
  F --> G -> f `@ F --> f `@ G.
Proof. by move=> FG P; rewrite !nearE; exact: FG. Qed.

Lemma cvg_comp (f : T -> U) (g : U -> V)
  (F : set (set T)) (G : set (set U)) (H : set (set V)) :
  f @ F --> G -> g @ G --> H -> g \o f @ F --> H.
Proof. 
move=> fFG gGH; apply: cvg_trans gGH => P; rewrite !nearE.
by move=> /fFG; rewrite nearE.
Qed.

Lemma cvgi_comp (f : T -> U) (g : U -> set V)
  (F : set (set T)) (G : set (set U)) (H : set (set V)) :
  f @ F --> G -> g `@ G --> H -> g \o f `@ F --> H.
Proof.
move=> fFG gGH; apply: cvg_trans gGH => P.
by rewrite nearE => /fFG; rewrite !nearE.
Qed.

Lemma cvg_eq_loc {F : fT} (f g : T -> U) :
  {near F, f =1 g} -> g @ F --> f @ F.
Proof. by move=> eq_fg P /=; rewrite !nearE; apply: nearS2 eq_fg => x/= <-. Qed.

Lemma cvgi_eq_loc {F : fT} (f g : T -> set U) :
  {near F, f =2 g} -> g `@ F --> f `@ F.
Proof.
move=> eq_fg P /=; rewrite !nearE; apply: nearS2 eq_fg => x eq_fg [y [fxy Py]].
...
Lemma cvg_fst {T U} (F : {filter T}) (G : {filter U}) :
  (@fst T U) @ (F, G) --> nbhs F.
Proof. by move=> P; rewrite nearE; apply: filter_prod1. Qed.

Lemma cvg_snd {T U} (F : {filter T}) (G : {filter U}) :
  nbhs ((@snd T U) @ (F, G)) --> nbhs G.
Proof. by move=> P; rewrite nearE; apply: filter_prod2. Qed.
...
by near=> x; have := PQ x.1 x.2; rewrite -surjective_pairing; apply; near: x;
   by [apply: cvg_fst|apply: cvg_snd].
Grab Existential Variables. all: end_near. Qed.
...
Definition nearE := (@near_map, @near_mapi, @near_map2).
Ltac near_simpl := rewrite /= ?nearE /=.
End NearMap.
...
Lemma cvg_pair {F : {filter T}} {G : {filter U}} {H : {filter V}}
   (f : T -> U) (g : T -> V) :
  f @ F --> G -> g @ F --> H -> (f x, g x) @[x --> F] --> (G, H).
Proof.
move=> fFG gFH /= P [[/=A B] [GA HB] ABP]; rewrite near_map; near=> x => /=.
by apply: ABP; split=> /=; near: x; [apply: fFG|apply: gFH].
Grab Existential Variables. all: end_near. Qed.

(* Lemma cvg_pair {F : fT} {G : fU} {H : fV} (f : T -> U) (g : T -> V) : *)
(*   f @ F --> G -> g @ F --> H -> (f x, g x) @[x --> F] --> (G, H). *)
(* Proof. *)
(* move=> fFG gFH /= P [[/=A B] [GA HB] ABP]; rewrite near_map; near=> x. *)
(* by apply: ABP; split=> /=; near: x; [apply: fFG|apply: gFH]. *)
...
(* Lemma cvg_comp2 {F : fT} {G : fU} {H : fV} {I : fW} *)
(*   (f : T -> U) (g : T -> V) (h : U -> V -> W) : *)
(*   f @ F --> G -> g @ F --> H -> *)
(*   h (fst x) (snd x) @[x --> (G, H)] --> I -> *)
(*   h (f x) (g x) @[x --> F] --> I. *)
(* Proof. *)
(* move=> fFG gFH hGHI P /= IP; rewrite nearE in hGHI *. *)
(* suff: \for w \near (f x, g x) @[x --> F], P (h w.1 w.2) by rewrite !nearE. *)
(* by apply: (cvg_pair fFG gFH); apply: hGHI. *)
(* Qed. *)
Lemma cvg_comp2 {F : {filter T}} {G : {filter U}} {H : {filter V}} {I : {filter W}}
  (f : T -> U) (g : T -> V) (h : U -> V -> W) :
  f @ F --> G -> g @ F --> H ->
  h (fst x) (snd x) @[x --> (G, H)] --> I ->
  h (f x) (g x) @[x --> F] --> I.
...
End NearPair.
(* Arguments cvg_comp2 {T U V W fT fU fV fW F G H I f g h} _ _ _. *)
Arguments cvg_comp2 {T U V W F G H I f g h} _ _ _.
...
Definition within D F := [set P : set T | {near F, D `<=` P}].
...
Lemma withinT D F : nbhs (within D F) D.
Proof. by rewrite /within; apply: near_all. Qed.
...
Canonical within_filterType D F := Eval hnf in 
 Filter (within D F) (within_filter _ _).

Lemma cvg_within D F : within D F --> F.
Proof. by move=> P; apply: nearS. Qed.

Definition subset_sig D F :=
  [set P : set {x | D x} | \for x \near F, forall Dx : D x, P (exist _ x Dx)].
Arguments subset_sig D F _ : clear implicits.

Lemma subset_sig_is_filter D F : is_filter (Nbhs (subset_sig D F)).
Proof.
constructor; rewrite /subset_sig.
- exact: near_all.
- by move=> P Q; apply: nearS2=> x PD QD Dx; split.
- by move=> P Q subPQ; apply: nearS => R PD Dx; apply: subPQ.
Qed.
Canonical subset_sig_filterType D F := Eval hnf in 
 Filter (subset_sig D F) (subset_sig_is_filter _ _).

Lemma subset_sig_proper D F :
  (forall P, nbhs F P -> ~ ~ exists x, D x /\ P x) ->
  is_pfilter (subset_sig D F).
Proof.
move=> DAP; apply: Build_is_pfilter'; first exact: filter_class.
rewrite /subset_sig => subFD.
...
by rewrite nearE; near=> y; apply: sBA.
Grab Existential Variables. all: end_near. Qed.

Lemma continuousP (S T : topologicalType) (f : S -> T) :
  continuous f <-> forall A, open A -> open (f @^-1` A).
Proof.
split=> fcont; first by rewrite !openE => A Aop ? /Aop /fcont; rewrite nearE.
move=> s A; rewrite open_nbhsE => - [B [[Bop Bfs] sBA]].
rewrite nearE; near=> y; apply: sBA; near: y.
by apply: neigh_nbhs; rewrite /neigh; split => //; apply: fcont.
Grab Existential Variables. all: end_near. Qed.
...
Lemma lim_cont {T : Type} {V U : topologicalType} {fT : filterType T} (F : fT)
  ...
  f @ F --> a -> (h \o f) @ F --> h a.
Proof.
by move=> h_cont fa fb; apply: (cvg_trans _ h_cont); apply: @cvg_comp fa _.
Qed.
...
Canonical prod_topologicalType := Eval hnf in 
  TopologicalType (T * U) prod_topologicalTypeMixin.

End Prod_Topology.

Definition filter_of T (x : set (set T)) :=
  fun (F : filter_on T) of phant_id x (in_filter F) => F.
Notation "[ 'filter' 'of' x ]" := (@filter_of _ x _ idfun).

Lemma lim_cont2 {T : Type} {V W U : topologicalType} {fT : filterType T} (F : fT)
  (f : T -> V) (g : T -> W) (h : V -> W -> U) (a : V) (b : W) :
  h z.1 z.2 @[z --> (a, b)] --> h a b ->
  f @ F --> a -> g @ F --> b -> (h (f x) (g x))@[x --> F] --> h a b.
Proof.
(* suff: False by []. *)
...
(* Print Canonical Projections. *)
(* unify (in_nbhs ?x) (uh @ ab). *)

move=> hc fa fb; apply: cvg_trans hc.
by apply: (cvg_comp2 fa fb).
...
(* move: (fun z : V * W => h z.1 z.2) => uh. *)
(* move: (a, b) => ab. *)
(* rewrite {T fT F f g h a b fa fb}. *)
(* (* pose x := [filter of uh @ ab]. *) *)
(* (* rewrite -[uh @ ab]/(in_filter x). *) *)
(* move=> /(_ [filterType U of filter_on U])/=. *)

(* (* rewrite /cvg_to/=. *) *)
(* (* rewrite -[in_nbhs (nbhs (uh @ ab))]/(uh @ ab). *) *)
(* (* evar (fU : filterType U). *) *)
(* (* move=> /(_ ?fU). *) *)
(* (* evar (s : Filter.sort [filterType U of filter_on U]). *) *)
(* move: (@cvg_to) => cvgto /=. *)
(* set daf := default_arrow_filter _. *)

(* unify (in_bhs (nbhs (uh @ ab)) (nbhs ?s). *)
(* apply. *)
...
(* move=> /(_ _ x). *)
(* pose X := [filterType U of (filter_on U)]. *)
(* move=> /(_ X). *)
(* apply. *)
...
(* pose nbhsU := [filterType U of filter_on U]. *)
(* move=> /(_ nbhsU). *)

(* Set Printing Coercions. *)
(* apply. *)
(* have :  (Filter.sort U) = (filter_on U). *)
(*   reflexivity. *)
(* unify (Filter.sort U) (filter_on U). *)
(* pose nbhsU := [filterType U of filter_on U]. *)
(* move=> /(_ nbhsU). *)
(* apply. *)
(* apply. *)
(* (* nbhs ?s *) *)
(* (* uh @ ab *) *)
(* move=> /(_ _ _ (@cvg_refl _ (nbhs (uh @ ab)))). *)
(* move=> XX. *)
(* Set Printing All. *)
(* apply XX. *)
(* apply. *)
...
Canonical pointedType := Eval hnf in  [pointedType of type].

Definition nbhsMixin : Nbhs.mixin_of type type :=
   NbhsMixin_ (fun p => [set A | exists B, [/\ op B, B p & B `<=` A]]).
Canonical nbhsType := Eval hnf in  NbhsType type type nbhsMixin.
...
Qed.
Canonical filterType := Eval hnf in FilterType type type filterMixin.

Lemma pfilterMixin : PFilter.mixin_of [filterType type of type].
Proof. by constructor => x; move=> [O [Oop Op]] /(_ _ Op). Qed.
Canonical pfilterType := Eval hnf in PFilterType type type pfilterMixin.
...
Canonical topologicalType := Eval hnf in TopologicalType type topologyMixin.
...
Definition nbhsMixin : Nbhs.mixin_of type type := NbhsMixin_ nbhs_of.
Canonical nbhsType := Eval hnf in NbhsType type type nbhsMixin.
...
Definition mx_filter (mx : 'M[fT]_(m, n)) : set (set 'M[T]_(m,n)) :=
  nbhs_from [set P | forall i j, (nbhs (mx i j)) (P i j)]
              (fun P => [set my : 'M[T]_(m, n) | forall i j, P i j (my i j)]).

Definition mx_nbhs_mixin := NbhsMixin_ mx_filter.

Canonical mx_nbhs := Eval hnf in NbhsType 'M[T]_(m,n) 'M[fT]_(m,n) mx_nbhs_mixin.
End MatrixNbhsType.
...
Proof.
constructor=> /= M; apply: nbhs_from_filter => [|A B AM BM].
...
Canonical mx_filtered := Eval hnf in FilterType 'M[T]_(m,n) 'M[fT]_(m,n) mx_filter_mixin.
End MatrixFilter.
...
Canonical mx_pfiltered := Eval hnf in PFilterType 'M[T]_(m,n) 'M[fT]_(m,n) mx_pfilter_mixin.
End MatrixPFilter.
...
Canonical matrix_topologicalType := Eval hnf in 
...
Canonical weak_pointedType := Eval hnf in [pointedType of fweak].
Definition weak_nbhsMixin := OfOpen.nbhsMixin wopen.
Canonical weak_nbhsType := Eval hnf in NbhsType fweak fweak weak_nbhsMixin. 
Definition weak_filterMixin := OfOpen.filterMixin wopT wopI.
Canonical weak_filterType := Eval hnf in FilterType fweak fweak weak_filterMixin. 
Definition weak_pfilterMixin := OfOpen.pfilterMixin wopT wopI.
Canonical weak_pfilterType := Eval hnf in PFilterType fweak fweak weak_pfilterMixin. 
Definition weak_topologyMixin := OfOpen.topologyMixin wop_bigU.
Canonical weak_topologyType := Eval hnf in TopologicalType fweak weak_topologyMixin.

Lemma weak_continuous : continuous (f : fweak -> T).
Proof. by apply/continuousP => A ?; exists A. Qed.

Lemma cvg_image (F : {filter S}) (s : fweak) :
  f @` setT = setT ->
...
Definition eventually := nbhs_from setT (fun N => [set n | (N <= n)%N]).
Notation "'\oo'" := eventually : classical_set_scope.

Canonical eventually_filter_source X := Eval hnf in 
   @Nbhs.Source X _ nat (NbhsMixin_ (fun f => f @ \oo)).

Definition eventually_filter : is_pfilter eventually.
Proof.
apply: nbhs_from_proper; last by move=> i _; exists i.
apply: nbhs_fromT_filter; first by exists 0%N.
...
Lemma nbhsE' (T : topologicalType) (x : T) :
  nbhs x = nbhs' x `&` at_point x :> set _.
Proof.
rewrite predeqE => A; split=> [x_A|[x_A Ax]].
  by split; [by near=> y => _; near: y|exact: nbhs_singleton].
by near=> y; have [->//|] := eqVneq y x; near: y.
Grab Existential Variables. all: end_near. Qed.

Definition nbhs'_is_filter {T : topologicalType} (x : T) :
  is_filter (nbhs (nbhs' x)).
Proof. exact: within_filter. Qed.
Typeclasses Opaque nbhs'.

Canonical nbhs'_filter_on (T : topologicalType)  (x : T) := Eval hnf in 
  Filter (nbhs' x) (nbhs'_is_filter _).
...
rewrite predeqE => A; split=> Acl p; last first.
  by move=> clAp; apply: Acl => /clAp [? []].
move=> NAp; apply: Acl => Q Qx; apply: contrapNT NAp => NAQ.
by near=> x; apply: contrapNN NAQ => Ax; exists x; split=> //; near: x.
Grab Existential Variables. all: end_near. Qed.

Lemma openC (D : set T) : closed D -> open (~` D).
Proof. by rewrite closedE openE => Dcl t nDt; apply: contrapT => /Dcl. Qed.
...
have NDfx : ~ D (f x).
  by move: fxD; rewrite open_nbhsE => - [A [[? ?]]]; apply.
by apply: f_cont fxD; rewrite in_setE.
Qed.

Lemma closed_cvg_loc {T} {U : topologicalType} {F : {pfilter T}}
  (f : T -> U) (D : U -> Prop) :
  forall y, f @ F --> y -> nbhs F (f @^-1` D) -> closed D -> D y.
Proof.
move=> y Ffy/= Df; apply => A /Ffy /=; rewrite nearE/= => Af.
by near F => x; exists (f x); split; near: x.
Grab Existential Variables. all: end_near. Qed.

Lemma closed_cvg {T} {U : topologicalType} {F : {pfilter T}}
  (f : T -> U) (D : U -> Prop) :
  forall y, f @ F --> y -> (forall x, D (f x)) -> closed D -> D y.
Proof. by move=> y fy FDf; apply: (closed_cvg_loc fy); apply: nearW. Qed.
...
Lemma cluster_cvgE (fT : pfilterType T) {F : fT} :
  cluster (nbhs F) = [set p | exists2 G : {pfilter T}, G --> p & G --> F].
Proof.
rewrite predeqE => p.
split=> [clFp|[G cvGp sFG] A B]; last first.
  by move=> /sFG GA /cvGp GB; near G => x; exists x; split; near: x.
pose Gbody := nbhs_from (\bigcup_(A in nbhs F) [set A `&` B | B in nbhs p]) id.
have Gpf : is_pfilter Gbody.
  apply: nbhs_from_proper => /=; last first.
    by move=> _ [A FA [B p_B <-]]; have := clFp _ _ FA p_B.
  apply: nbhs_from_filter.
    exists setT, setT; first exact: nearT.
    by exists setT; [apply: nearT|rewrite setIT].
  move=> _ _ [A1 FA1 [B1 p_B1 <-]] [A2 FA2 [B2 p_B2 <-]].
  exists (A1 `&` B1 `&` (A2 `&` B2)) => //.
  exists (A1 `&` A2); first exact: nearI.
  by exists (B1 `&` B2); [apply: nearI|rewrite setIACA].
pose G := PFilterPack Gbody Gpf.
exists G.
  move=> A p_A; exists A => //; exists setT; first exact: nearT.
  by exists A => //; rewrite setIC setIT.
move=> A FA; do 2!exists A => //; exists setT; first exact: nearT.
by rewrite setIT.
Grab Existential Variables. all: end_near. Qed.

Definition compact A := forall (fT : pfilterType T) (F : fT),
  nbhs F A -> A `&` cluster (nbhs F) !=set0.

Lemma compact0 : compact set0.
Proof. by move=> fT F /near_ex []. Qed.

Lemma subclosed_compact (A B : set T) :
  closed A -> compact B -> A `<=` B -> compact A.
Proof.
move=> Acl Bco sAB fT F FA.
have [|p [Bp Fp]] := Bco _ F; first exact: nearS FA.
by exists p; split=> //; apply: Acl=> C Cp; apply: Fp.
Qed.

Definition hausdorff := forall p q : T, cluster (nbhs p) q -> p = q.

Definition within_nbhs_proper (A : set T) p :
  (closure A p) -> is_pfilter (within A p).
Proof.
move=> clAp; apply: Build_is_pfilter; last exact: filter_class.
by move=> P /clAp [q [Aq AqsoBq]]; exists q; apply: AqsoBq.
Qed.

Lemma compact_closed (A : set T) : hausdorff -> compact A -> closed A.
Proof.
move=> hT Aco p clAp; have := @withinT _ _ A p.
case: pfilterP => [|w eqw]; first exact: within_nbhs_proper.
move=> /(Aco _ w)[q [Aq clsAp_q]]; rewrite (hT p q) //.
by apply: cvg_cluster clsAp_q; rewrite eqw; apply: cvg_within.
Qed.

End Compact.
Arguments hausdorff : clear implicits.

Arguments pfilterP {_ _}.
Lemma continuous_compact (T U : topologicalType) (f : T -> U) A :
  {in A, continuous f} -> compact A -> compact (f @` A).
Proof.
move=> fcont Aco fT F FfA.
have [|G eqG] := pfilterP (nbhs_from (nbhs F) (fun C => A `&` f @^-1` C)).
  apply: (nbhs_from_proper (nbhs_from_filter _ _)); first by exists (f @` A).
    move=> C1 C2 F1 F2; exists (C1 `&` C2); first exact: nearI.
    by move=> ?[?[]]; split; split.
  by move=> C /(nearI FfA) /near_ex [_ [[p ? <-]]]; eexists p.
case: (Aco _ G); first by rewrite eqG; exists (f @` A) => // ? [].
move=> p [Ap clsGp]; exists (f p); split; first exact/imageP.
move=> B C FB /fcont; rewrite in_setE /= => /(_ Ap) p_Cf.
have : G (A `&` f @^-1` B) by rewrite eqG; exists B.
by move=> /clsGp /(_ p_Cf) [q [[]]]; exists (f q).
Qed.

Section Tychonoff.

Definition filter_is_ultra T (F : set (set T)) :=
  forall G : pfilter_on T, F `<=` G -> G = F :> set _.

Record is_ufilter T (F : set (set T)) := {
  pfilter_ufilter :> is_pfilter F ;
  _ : filter_is_ultra F
}.

Structure ufilter_on T := UFilterPack {
  in_ufilter :> set (set T);
  _ : is_ufilter in_ufilter
}.

Definition ufilter_class T (F : ufilter_on T) : is_ufilter F :=
  let: UFilterPack _ class := F in class.
Arguments UFilterPack {T} _ _.
Canonical ufilter_nbhs_on T (F : ufilter_on T) := Eval hnf in Nbhs F.
Coercion ufilter_nbhs_on : ufilter_on >-> nbhs_on.
Canonical ufilter_filter_on T (F : ufilter_on T) :=
  Eval hnf in Filter F (ufilter_class F).
Coercion ufilter_filter_on : ufilter_on >-> filter_on.
Canonical ufilter_pfilter_on T (F : ufilter_on T) :=
  Eval hnf in PFilterPack F (ufilter_class F).
Coercion ufilter_pfilter_on : ufilter_on >-> pfilter_on.

Definition UFilter_of {T} (F : set (set T)) (ultraF : filter_is_ultra F) :=
  fun (fF : pfilter_on T) of phant_id F (fF : set (set T)) =>
  fun (fFF : is_pfilter _) of phant_id (PFilterPack _ fFF) fF =>
  UFilterPack F (Build_is_ufilter fFF ultraF).
Notation UFilter F ultraF := (@UFilter_of _ F ultraF _ id _ id).

Lemma ultra_pointfilter (T : pointedType) (x : T) :
  filter_is_ultra (pointfilter x).
Proof.
rewrite /pointfilter/= => G pG; rewrite predeqE => A; split => [|/pG//].
rewrite -nbhs_setE => GA; have Gp : G [set x] by apply: pG.
by near G => y; suff <- : y = x; near: y.
Grab Existential Variables. all: end_near. Qed.

Lemma ufilter_is_pfilter (T : Type) (pT : ufilter_on T) :
  filter_is_proper (Nbhs (in_ufilter pT)).
Proof. by case: pT => ? [[]]. Qed.

Lemma ufilter_is_filter (T : Type) (pT : ufilter_on T) :
  is_filter (Nbhs (in_pfilter pT)).
Proof. exact: pfilter_is_filter. Qed.

Canonical ufilter_on_eqType T := Eval hnf in EqType (ufilter_on T) gen_eqMixin.
Canonical ufilter_on_choiceType T := Eval hnf in 
  ChoiceType (ufilter_on T) gen_choiceMixin.
Canonical pointfilter_ufilter (T : pointedType) (x : T) := Eval hnf in 
  UFilter (pointfilter x) (@ultra_pointfilter _ x). 
Canonical ufilter_on_PointedType (T : pointedType) := Eval hnf in 
  PointedType (ufilter_on T) (pointfilter_ufilter point).
Definition ufilter_on_nbhs_mixin (T : pointedType) :=
  NbhsMixin_ (@in_ufilter T).
Canonical ufilter_on_nbhsType (T : pointedType) := Eval hnf in
...
Canonical ufilter_on_filterType (T : pointedType) := Eval hnf in
  FilterType T (ufilter_on T) (@ufilter_on_filter_mixin T).
Definition ufilter_on_pfilter_mixin (T : pointedType)  :=
  PFilterMixin (@ufilter_is_pfilter T).
Canonical ufilter_on_pfilterType (T : pointedType) := Eval hnf in
...
Lemma ultra_cvg_clusterE (T : topologicalType) (F : ufilter_on T) :
  cluster (nbhs F) = [set p | nbhs F --> p].
Proof.
rewrite predeqE => p; split; last first.
  by move=> cvFp; rewrite cluster_cvgE; exists (F : pfilter_on T).
rewrite cluster_cvgE => - [G cvGp /max_filter eqGF].
by rewrite eqGF in cvGp.
Qed.

Lemma mk_ufilter_subdef T (F : {pfilter T}) :
  exists G : ufilter_on T, F `<=` G.
Proof.
...
Lemma compact_ultra (T : topologicalType) :
  compact = [set A | forall F : ufilter_on T,
                      F A -> A `&` [set p | F --> p] !=set0].
Proof.
...
Lemma filter_image (T U : Type) (f : T -> U) (F : {filter T}) :
  f @` setT = setT -> is_filter [set f @` A | A in F].
Proof.
set X := [set _ | _ in _]; move=> fsurj; split.
- by exists setT => //; exact: (nearT F).
- move=> P Q [A FA]; rewrite -[[eta P]]/P => <-.
  move=> [B FB]; rewrite -[[eta Q]]/Q => <-.
 ...
 (* suff: nbhs F FAB by []. *)
  (* suff: \for x \near F, FAB x by []. *)
  apply: nearS sAB _. apply: nearI.
- move=> A B sAB [C FC fC_eqA].
  exists (f @^-1` B); last by rewrite image_preimage.
  suff: nbhs F C -> nbhs F (f @^-1` B) by apply.
  by apply: nearS => p Cp; apply: sAB; rewrite -fC_eqA; exists p.
Grab Existential Variables. all: by end_near. Qed.

Lemma proper_image (T U : Type) (f : T -> U) (F : pfilter_on T) :
  f @` setT = setT -> is_pfilter [set f @` A | A in F].
Proof.
move=> fsurj; apply Build_is_pfilter; last exact: filter_image.
move=> _ [A FA <-].
(* have := (@near_const _ (pfilter_on T) F). *)

(* have : nbhs (F : {filter T}) A := FA. *)
(* have /near_ex [p Ap] := FA; exists (f p); exists p. *)
(* Qed. *)
Admitted.

Lemma in_ultra_setVsetC T (F : ufilter_on T) (A : set T) :
  F A \/ F (~` A).
Proof.
case: (pselect (F (~` A))) => [|nFnA]; first by right.
left; suff : is_pfilter (nbhs_from (F `|` [set A `&` B | B in F]) id).
...
Lemma finI_filter (I : choiceType) T (D : set I) (f : I -> set T) :
  finI D f -> is_pfilter (nbhs_from (finI_from D f) id).
Proof.
move=> finIf; apply: (nbhs_from_proper (nbhs_from_filter _ _)).
...
Definition nbhs_ {T T'} (ball : T -> R -> set T') (x : T) :=
   @nbhs_from R _ [set x | 0 < x] (ball x).

Lemma nbhs_E {T T'} (ball : T -> R -> set T') x :
  nbhs_ ball x = @nbhs_from R _ [set x : R | 0 < x] (ball x).
Proof. by []. Qed.
...
Next Obligation.
rewrite (Uniform.ax4 m) nbhs_E; apply nbhs_from_proper; last first.
  move=> e egt0; exists p; suff : Uniform.ball m p (PosNum egt0)%:num p by [].
  exact: Uniform.ax1.
apply: nbhs_from_filter; first by exists 1%:pos%:num.
...
Lemma nbhs_from_ballE {M : uniformType} x :
  @nbhs_from R _ [set x : R | 0 < x] (@ball M x) = nbhs x.
Proof. by rewrite -nbhs_ballE. Qed.

Module Export NbhsBall.
Definition nbhs_simpl := (nbhs_simpl,@nbhs_from_ballE,@nbhs_ballE).
End NbhsBall.
...
Definition entourages : set (set (M * M)):=
  nbhs_from [set eps : R | eps > 0]
              (fun eps => [set xy | ball xy.1 eps xy.2]).

Global Instance entourages_filter : is_pfilter entourages.
Proof.
apply nbhs_from_proper; last by exists (point,point); apply: ballxx.
apply: nbhs_from_filter; first by exists 1; rewrite ltr01.
...
Lemma cvg_close {F} {FF : is_pfilter F} (x y : M) :
  F --> x -> F --> y -> close x y.
Proof.
...
Lemma cvg_ballP {F} {FF : is_filter F} (y : M) :
  F --> y <-> forall eps : R, 0 < eps -> \for y' \near F, ball y eps y'.
Proof. by rewrite -nbhs_fromP !nbhs_simpl /=. Qed.
Definition cvg_nbhs := @cvg_ballP.

Lemma cvg_ballPpos {F} {FF : is_filter F} (y : M) :
  F --> y <-> forall eps : {posnum R}, \for y' \near F, ball y eps%:num y'.
Proof.
by split => [/cvg_ballP|] pos; [case|apply/cvg_ballP=> _/posnumP[eps] //].
Qed.

Lemma cvg_ball {F} {FF : is_filter F} (y : M) :
  F --> y -> forall eps : R, 0 < eps -> \for y' \near F, ball y eps y'.
Proof. by move/cvg_ballP. Qed.

Lemma app_cvg_nbhs T {F} {FF : is_filter F} (f : T -> M) y :
  f @ F --> y <-> forall eps : R, 0 < eps -> \for x \near F, ball y eps (f x).
Proof. exact: cvg_ballP. Qed.

Lemma cvgi_ballP T {F} {FF : is_filter F} (f : T -> M -> Prop) y :
  f `@ F --> y <->
  forall eps : R, 0 < eps -> \for x \near F, exists z, f x z /\ ball y eps z.
Proof.
...
Definition cvgi_nbhs := @cvgi_ballP.

Lemma cvgi_ball T {F} {FF : is_filter F} (f : T -> M -> Prop) y :
  f `@ F --> y ->
  forall eps : R, 0 < eps -> F [set x | exists z, f x z /\ ball y eps z].
Proof. by move/cvgi_ballP. Qed.

Lemma cvgi_close T {F} {FF: is_pfilter F} (f : T -> set M) (l l' : M) :
...
Grab Existential Variables. all: end_near. Qed.
Definition cvgi_nbhs_close := @cvgi_close.

Lemma cvg_const {T} {F : {filter T}}
...
Hint Resolve nbhs_ball : core.
Arguments cvg_const {M T F FF} a.
...
Canonical R_pointedType := Eval hnf in PointedType R 0.
...
Definition matrix_uniformType_mixin := Eval hnf in
  Uniform.Mixin mx_ball_center mx_ball_sym mx_ball_triangle mx_nbhs.

Canonical matrix_uniformType := Eval hnf in
...
Canonical prod_uniformType (U V : uniformType) := Eval hnf in
...
Lemma cvg_ball2P {F : {filter U}} {G : {filter V}}
  {FF : is_filter F} {FG : is_filter G} (y : U) (z : V):
  (F, G) --> (y, z) <->
  forall eps : R, eps > 0 -> \for y' \near F & z' \near G,
...
Canonical generic_source_filter := Eval hnf in @Filter.Source _ _ _ (nbhs_ fct_ball).
Canonical fct_topologicalType := Eval hnf in
  TopologicalType (T -> U) fct_topologicalTypeMixin.
Canonical fct_uniformType := Eval hnf in UniformType (T -> U) fct_uniformType_mixin.
...
Canonical matrix_completeType := Eval hnf in CompleteType 'M[T]_(m, n) mx_complete.
...
Canonical fun_completeType := Eval hnf in CompleteType (T -> U) fun_complete.
