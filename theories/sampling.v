...
rewrite bernoulliE// diracE/= mem_set// mule1// diracE/= memNset//.
...
rewrite bernoulliE// diracE/= memNset// mule0// diracE/= mem_set// add0e mule1.
...
Proof.
rewrite unlock -(@ge0_integral_distribution _ _ _ _ _ _ X
  (EFin \o GRing.natmul 1))//; last by move=> y/=.
rewrite /bernoulli/=.
rewrite (@eq_measure_integral _ _ _ _ (bernoulli p)); last first.
   by move=> A mA _ /=; congr (_ _); exact: bernoulliP.
by rewrite integral_bernoulli//= -!EFinM -EFinD mulr0 addr0 mulr1.
Qed.
...
rewrite unlock TAB integral_setU_EFin -?TAB//.
under eq_integral.
  move=> x /=.
...
rewrite -(@fineK _ mu)//; last by rewrite /mu expectation_bernoulli_trial.
