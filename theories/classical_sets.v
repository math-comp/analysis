...
Section bigcup_mkset.
Variables (T : choiceType) (U : Type).

Lemma bigcup_mkset_cond (s : seq T) (f : T -> set U) (P : pred T) :
...
Lemma bigcup_mkset (s : seq T) (f : T -> set U) :
  \bigcup_(t in [set x | x \in s]) (f t) = \big[setU/set0]_(t <- s) (f t).
Proof.
rewrite -(bigcup_mkset_cond s f xpredT); congr (\bigcup_(t in mkset _) _).
