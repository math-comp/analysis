...
From mathcomp Require Import interval_inference archimedean rat ring lra.
...
(* Definition mR R := [the measurableType (R.-ocitv.-measurable).-sigma of
                 salgebraType (R.-ocitv.-measurable)]. *)
Notation munit := (unit : measurableType _).
Notation mbool := (bool : measurableType _).
Notation mnat := (nat : measurableType _).
End Notations.
...
Lemma integral_bernoulli {R : realType} (p : {nonneg R}) (p1 : (p%:num <= 1)%R)
  ...
  p%:num%:E * f true + p%:num.~%:E * f false.
Proof.
move=> f0.
...
- rewrite [X in measurable_fun _ X](_ : _ =
    (fun x => if 0 <= x <= 1 then x.~%:E else 1%E))//.
...
Lemma integral_bernoulli_trunc {R : realType} (p : R) (f : bool -> \bar R) :
  ...
  \int[bernoulli_trunc p]_y (f y) = p%:E * f true + p.~%:E * f false.
Proof.
...
rewrite (_ : f = fun x => if 0 <= x <= 1 then (\sum_(m < n.+1)
      ...
      | inl l0p =>
          ...
          | inr _ => (x ^+ m * x.~ ^+ (n - m) *+ 'C(n, m))%:E * \d_(nat_of_ord m) Ys
          end
      | inr _ => (x ^+ m * x.~ ^+ (n - m) *+ 'C(n, m))%:E * \d_(nat_of_ord m) Ys
      end)%E
   else \d_0%N Ys)//.
  ...
  rewrite /mscale/= [X in measurable_fun _ X](_ : _ = (fun x =>
    ((x ^+ m * x.~ ^+ (n - m) *+ 'C(n, m))%:E * \d_(nat_of_ord m) Ys)%E)); last first.
...
Definition ubeta_nat_pdf (t : R) := t ^+ a.-1 * t.~ ^+ b.-1.
...
rewrite big_mkord (bigD1 floor_f)//= ifT; last first.
  rewrite lee_fin lte_fin; apply/andP; split.
    by rewrite natr_absz (@ger0_norm _ (floor `|f t|)) ?floor_ge0// floor_le_tmp.
...
apply: measurable_fun_pair => //=; exact: measurableT_comp.
Qed.
...
Lemma letin_sample_bernoulli d d' (T : measurableType d)
  ...
  r%:num%:E * u (x, true) y + (r%:num.~)%:E * u (x, false) y.
Proof.
rewrite letinE/=.
rewrite ge0_integral_measure_sum// 2!big_ord_recl/= big_ord0 adde0/=.
by rewrite !ge0_integral_mscale//= !integral_dirac//= !diracT 2!mul1e.
...
Definition sample_and_branch : R.-sfker T ~> R :=
  letin
    (sample_cst [the probability _ _ of bernoulli p27]) (* T -> B *)
