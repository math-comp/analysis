...
Local Lemma refine_subset : forall n U e, @refine_apx n U e `<=` U.
Proof.
...
Local Fixpoint branch_apx (b : T) n :=
  if n is S m then refine_apx (branch_apx b m) (b m) else [set: X].
...
case/refine_separates => n sep; have bry : branch_apx b n.+1 y.
...
have zcov' : forall n (U : set X), exists e, U z -> @refine_apx n U e z.
  ...
  by rewrite {1}(@refine_cover n U); case => e _ ?; exists e.
...
suff : forall n, branch_apx x n = branch_apx y n.
  ...
  by rewrite brE -/(branch_apx y n.+1); apply: tree_map_apx.
elim => // m /= brE.
...
move=> M' _ Mcov; exists 
  ((fun x => closure (fs x)) @` [set` M'] `|` [set [set t0];[set t1]]).
move=> _; split.
- rewrite finite_setU; split; first by exact/finite_image/finite_fset.
  exact: finite_set2.
- move=> A []; first (case=> z M'z <-; exists z; split).
...
case: (pselect (exists (p : T), p != point)).
  by case => p ppt; apply: cantor_surj_twop; exact: ppt.
move=> /forallNP xpt; have : set_surj [set: cantor_space] [set: T] (cst point).
