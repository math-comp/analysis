...
- intros H P [eps LP].
  ...
  apply: filter_imp.
  intros u.
  rewrite /= AbsRing_ballE in LP => /boolp.asboolP.
  apply LP.
- intros H eps.
  apply (H (fun y => Rabs (y - l) < eps)).
  now exists eps => ?; rewrite AbsRing_ballE boolp.asboolE.
- intros H P [M LP].
  unfold filtermap.
  generalize (H M).
  apply: filter_imp.
  intros u => /Rstruct.RltP.
  apply LP.
- intros H M.
  apply (H (fun y => M < y)).
  now exists M => ? /Rstruct.RltP.
- intros H P [M LP].
  unfold filtermap.
  generalize (H M).
  apply: filter_imp.
...
Proof.
  ...
  apply cond_pos.
  intros y [H1 H2].
  now apply (Hd y) => //; rewrite AbsRing_ballE boolp.asboolE.
Qed.
...
Proof.
  ...
  intros y Hy Hxy.
  apply (H' y).
  now split => //; rewrite AbsRing_ballE boolp.asboolE in Hy.
Qed.
...
(* was ring_simplify.*)
  rewrite -Rstruct.RplusE; ring_simplify.
  rewrite -Rstruct.RinvE; last by rewrite /plus /= -S_INR; apply/eqtype.eqP/not_0_INR.
  by apply RinvN_pos.
...
Proof.
  ...
  exists d => y /= Hy Hy'.
  apply Hd.
  rewrite /AbsRing_ball /abs /minus /plus /opp /=.
  rewrite -!RabsE.
...
Proof.
  ...
  rewrite (AbsRing_ballE b (mkposreal _ H)) boolp.asboolE in Hy.
  eapply Rle_lt_trans, Hy.
  rewrite /R_dist Rabs_minus_sym. (* was rewrite -abs_opp opp_minus.*)
  apply Rle_abs.
...
{ assert (Hfb' : Rbar_locally' b (fun x => y < f x)).
  ...
  - destruct Hfb' as [eps He].
    ...
    apply He.
    rewrite AbsRing_ballE boolp.asboolE.
  ...
  - destruct Hfb' as [M HM].
    ...
    intros Hu.
    apply Rle_not_lt with (1 := H2).
    now apply HM; apply/Rstruct.RltP.
  - now destruct a. }
...
{ assert (Hfa' : Rbar_locally' a (fun x => Rbar_lt x b /\ f x < y)).
  ...
  - destruct Hfa' as [eps He].
    ...
    assert (H : Rbar_lt (a + eps / 2) b /\ (f (a + eps / 2) < y)).
      ...
      replace (a + eps / 2 (*+*) - a) with (eps / 2) by ring.
      rewrite Rabs_pos_eq.
      apply/Rstruct.RltP/Rlt_eps2_eps.
      apply cond_pos.
      apply Rlt_le.
      apply is_pos_div_2.
  ...
  - destruct Hfa' as [M HM].
    ...
    assert (H : Rbar_lt (M - 1) b /\ f (M - 1) < y).
      apply HM.
      apply/Rstruct.RltP/Rminus_lt_0.
...
apply Hr; rewrite -Rstruct.RminusE; apply/Rstruct.RltP.
...
apply Rplus_lt_compat.
apply Hr.
rewrite -Rstruct.RminusE; apply/Rstruct.RltP.
replace (u - p) with (u - x + (x - p)) by ring.
apply Rle_lt_trans with (1 := Rabs_triang _ _).
rewrite (double_var r).
apply Rplus_lt_compat with (2 := Hxp).
...
Proof.
(*intros x y eps; exists eps => y0; rewrite /ball /= /prod_ball /= !ballE; tauto.
Qed.*) Admitted.
...
Proof.
(*by intros x y c eps; exists eps; rewrite Rminus_eq_0 Rabs_R0.
Qed.*) Admitted.
...
by apply: (@filterlim_ext_loc _ _ _ _ _ f g).
Qed.*) Admitted.
...
(* TODO: rewrite {1}/ball /= /prod_ball /= in Hdel.
...
Proof.
  intros Hf Hg.
(*TODO: apply filterlim_locally => eps.
...
(*      by apply Rlt_0_1.
    ...
    contradict Hy.
    rewrite /ball /= AbsRing_ballE boolp.asboolE.
    apply Rle_not_lt.
    ...
    by apply Hd; rewrite /ball /= AbsRing_ballE boolp.asboolE.
  ...
  apply (Hdelta x).
    eapply ball_le; last first.
      rewrite /ball /= AbsRing_ballE boolp.asboolE; apply Htx.
